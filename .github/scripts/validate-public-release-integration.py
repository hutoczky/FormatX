...
def validate_known_issues() -> None:
    ...
    require_tokens(page, "Known-issues page", [
        'data-fx-public-page="known-issues"',
        'data-issues-summary',
        'data-issue-controls',
        'data-issue-search',
        'data-issue-filter="platform"',
        'data-issue-filter="severity"',
        'data-issue-filter="status"',
        'data-issue-results',
        'data-issues-root',
        'hreflang="hu"',
        'hreflang="en"',
    ...
    require_tokens(controller, "Known-issues controller", [
        "configureIssueControls",
        "applyIssueFilters",
        "renderIssueSummary",
        "data.issue_records" if False else "data.items",
        "fx-issue-card__id",
        "aria-busy",
    ])
    ...
    for item in data.get("items", []):
        ...
        require(item.get("severity") in data.get("severity_labels", {}), f"Unknown severity: {item.get('id')}")
        require(item.get("fix_status") in data.get("fix_labels", {}), f"Unknown fix status: {item.get('id')}")
...
def validate_public_shell() -> None:
    ...
    require("fetch(" not in shell and "XMLHttpRequest" not in shell and "WebSocket" not in shell,
            "Public shell must not send data or depend on remote content")
    ...
    for source, label in [(production, "Production wrapper"), (preview, "Preview wrapper")]:
        require_tokens(source, label, [
            "formatx-public-shell.js", "formatx-content-standard.css?v=20260731-content-2",
            "public-shell-1", "Cache-Control', 'no-store",
        ])
    ...
    pages = [
        "method.html", "verification.html", "test-matrix.html", "known-issues.html",
        "security.html", "decision-log.html", "downloads/index.html", "license.html",
        "terms.html", "privacy.html", "support.html",
    ]
    for page in pages:
        content = read(SCIFI / page)
        require('<link rel="canonical"' in content, f"Canonical URL missing from {page}")
        require('<meta name="description"' in content, f"Meta description missing from {page}")
...
def validate_release_sync() -> None:
    ...
    require_tokens(release_script, "Release metadata controller", [
        "ready-v3", "isAllowedReleaseUrl", "formatBytes", "integrityLabel",
        "data-release-integrity", "data-release-source-updated", "current-release.json",
    ])
    ...
    require(parsed.scheme == "https" and parsed.netloc == "github.com" and parsed.path.startswith("/hutoczky/FormatX-Updates/releases/"),
            "Current release URL is not an official FormatX-Updates release URL")
    windows = release.get("channels", {}).get("windows", {})
    if windows.get("available") is True:
        download = urlparse(str(windows.get("download_url") or ""))
        require(download.scheme == "https" and download.netloc == "github.com" and download.path.startswith("/hutoczky/FormatX-Updates/releases/download/"),
                "Windows download is not an official release asset")
    schema = release.get("schema_version")
    require(schema in {1, 2}, f"Unsupported current-release schema: {schema}")
    if schema == 2:
        require(isinstance(release.get("source_release_id"), int), "Schema 2 release lacks source_release_id")
...
def validate_worker_ownership() -> None:
    ...
    require(preview_config.get("main") == "content-preview-entry.js",
            "Preview Worker does not use content-preview-entry.js")
...
def main() -> int:
    ...
    if ERRORS:
        ...
        return 1
    print("FormatX public pages, known-issues registry, release sync and Worker ownership are valid.")
    return 0
