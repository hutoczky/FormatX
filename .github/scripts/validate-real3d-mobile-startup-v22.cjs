...
for (const token of [
  ...
  'schedule(3)'
]) assert.ok(renderer.includes(token), `missing r317 mobile startup contract: ${token}`);
...
assert.match(interactionStability, /r250|r317|booting-v69/);
...
console.log('PASS: mobile startup selects the r317 event-driven crystal with r250 WebGL fallback and stable reference controls/text flow.');
