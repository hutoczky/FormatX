...
(async()=>{
  ...
  async function captureMobileNativePreflight(){
    ...
    try{
      ...
      const state=await page.evaluate(()=>{
        ...
        return {
          visual:root.dataset.fxNativeMagVisualR1559||root.dataset.fxNativeMagVisualR1558||root.dataset.fxNativeMagVisualR1557||root.dataset.fxNativeMagVisualR1556||root.dataset.fxNativeMagVisualR1555||root.dataset.fxNativeMagVisualR1553||root.dataset.fxNativeMagVisualR1500||'',
  ...
  try{
    ...
    // hides the permanent mobile MAG evidence.
    {
      ...
      const stage=page.locator('#hero .fx-crystal-organism-r326-stage').first();
      await captureLocatorClip(page,stage,'09-mobile-native-mag.png');
      if(state.visual!=='photographic-smoky-obsidian-irregular-monolith-antialiased-clean-surface-visible-dark-glass')errors.push('R1559 visual marker missing: '+state.visual);
