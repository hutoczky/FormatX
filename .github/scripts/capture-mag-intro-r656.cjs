...
(async()=>{
  ...
  async function captureMobileNativePreflight(){
    ...
    try{
      ...
      const state=await page.evaluate(()=>{
        ...
        return {
          visual:root.dataset.fxNativeMagVisualR1500||root.dataset.fxNativeMagVisualR1490||root.dataset.fxNativeMagVisualR1480||root.dataset.fxNativeMagVisualR1470||'',
  ...
  try{
    ...
    // hides the permanent mobile MAG evidence.
    {
      ...
      const stage=page.locator('#hero .fx-crystal-organism-r326-stage').first();
      await captureLocatorClip(page,stage,'09-mobile-native-mag.png');
      if(state.visual!=='photoreal-asymmetric-obsidian-mineral-local-optical-lens-eight-tendrils')errors.push('R1500 visual marker missing: '+state.visual);
