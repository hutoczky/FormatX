...
(async()=>{
  ...
  async function captureMobileNativePreflight(){
    ...
    try{
      ...
      const state=await page.evaluate(()=>{
        ...
        return {
          visual:root.dataset.fxNativeMagVisualR1557||root.dataset.fxNativeMagVisualR1556||root.dataset.fxNativeMagVisualR1555||root.dataset.fxNativeMagVisualR1553||root.dataset.fxNativeMagVisualR1500||'',
  ...
  try{
    ...
    // hides the permanent mobile MAG evidence.
    {
      ...
      const stage=page.locator('#hero .fx-crystal-organism-r326-stage').first();
      await captureLocatorClip(page,stage,'09-mobile-native-mag.png');
      if(state.visual!=='solid-black-volcanic-glass-broad-mineral-planes-neutral-studio-reflections-no-eye-no-hud')errors.push('R1557 visual marker missing: '+state.visual);
    ...
    if(!SKIP_HANDOFF){
      ...
      // native MAG, full-page chamber, controls and copy together at 390x844.
      {
        ...
        const stage=page.locator('#hero .fx-crystal-organism-r326-stage').first();
        await captureLocatorClip(page,stage,'09-mobile-native-mag.png');
        if(state.visual!=='solid-black-volcanic-glass-broad-mineral-planes-neutral-studio-reflections-no-eye-no-hud'){
          errors.push('R1557 native crystal visual marker missing: '+state.visual);
        }
