...
(async()=>{
  ...
  async function captureMobileNativePreflight(){
    ...
    try{
      ...
      const state=await page.evaluate(()=>{
        ...
        return {
          visual:root.dataset.fxNativeMagVisualR1558||root.dataset.fxNativeMagVisualR1557||root.dataset.fxNativeMagVisualR1556||root.dataset.fxNativeMagVisualR1555||root.dataset.fxNativeMagVisualR1553||root.dataset.fxNativeMagVisualR1500||'',
  ...
  try{
    ...
    // hides the permanent mobile MAG evidence.
    {
      ...
      const stage=page.locator('#hero .fx-crystal-organism-r326-stage').first();
      await captureLocatorClip(page,stage,'09-mobile-native-mag.png');
      if(state.visual!=='smoky-obsidian-continuous-asymmetric-crystal-visible-studio-planes-clean-silhouette-no-eye')errors.push('R1558 visual marker missing: '+state.visual);
    ...
    if(!SKIP_HANDOFF){
      ...
      // native MAG, full-page chamber, controls and copy together at 390x844.
      {
        ...
        const stage=page.locator('#hero .fx-crystal-organism-r326-stage').first();
        await captureLocatorClip(page,stage,'09-mobile-native-mag.png');
        if(state.visual!=='smoky-obsidian-continuous-asymmetric-crystal-visible-studio-planes-clean-silhouette-no-eye'){
          errors.push('R1558 native crystal visual marker missing: '+state.visual);
        }
