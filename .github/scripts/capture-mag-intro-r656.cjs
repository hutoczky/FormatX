...
(async()=>{
  ...
  async function captureMobileNativePreflight(){
    ...
    try{
      ...
      u.searchParams.set('mobileproof','r1490-preflight');
      ...
      if(!ready)throw new Error('R1490 mobile preflight MAG did not become ready');
      ...
      const state=await page.evaluate(()=>{
        ...
        return {
          visual:root.dataset.fxNativeMagVisualR1490||root.dataset.fxNativeMagVisualR1480||root.dataset.fxNativeMagVisualR1470||'',
      ...
      if(state.shape!=='crystal')errors.push('R1490 preflight shape is not crystal: '+state.shape);
      if(/blur\((?!0(?:px)?\))/i.test(state.filter||''))errors.push('R1490 preflight still blurred: '+state.filter);
      if(state.ring.display!=='none'&&state.ring.visibility!=='hidden'&&state.ring.opacity>.01)errors.push('R1490 preflight hero ring visible: '+JSON.stringify(state.ring));
      if(state.overflow>2)errors.push('R1490 preflight overflow: '+state.overflow);
  ...
  try{
    ...
    // hides the permanent mobile MAG evidence.
    {
      ...
      const u=new URL(BASE);
      u.searchParams.set('mobileproof','r1490');
      await page.goto(u.href,{waitUntil:'domcontentloaded',timeout:30000});
      ...
      if(!mobileReady)throw new Error('R1490 early mobile native MAG did not become ready');
      ...
      const stage=page.locator('#hero .fx-crystal-organism-r326-stage').first();
      await captureLocatorClip(page,stage,'09-mobile-native-mag.png');
      if(state.visual!=='single-body-asymmetric-obsidian-crystal-no-detached-armor-optical-lens-eight-tendrils')errors.push('R1490 visual marker missing: '+state.visual);
      if(state.shape!=='crystal')errors.push('R1490 mobile MAG shape is not crystal: '+state.shape);
      if(/blur\((?!0(?:px)?\))/i.test(state.canvasFilter||''))errors.push('R1490 mobile MAG still has blur: '+state.canvasFilter);
      if(state.heroRing.present&&state.heroRing.display!=='none'&&state.heroRing.visibility!=='hidden'&&state.heroRing.opacity>.01)errors.push('R1490 legacy hero ring visible: '+JSON.stringify(state.heroRing));
      if(state.overflow>2)errors.push('R1490 horizontal overflow: '+state.overflow);
    ...
    for(const [seconds,name] of SHOTS){
      ...
      if(!overlayReady){
        ...
        throw new Error('R1490 intro overlay missing '+JSON.stringify({debug,errors}));
      }
      ...
      if(!frameReady)throw new Error('R1490 deterministic intro frame did not become ready at '+seconds+'s');
