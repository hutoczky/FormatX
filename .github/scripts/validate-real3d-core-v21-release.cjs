...
/* r326 is the user-visible canonical production MAG. r442 is the current
   physical/render-budget contract; old r424 labels remain lineage only. */
for(const token of [
  ...
  'const IDLE_ENERGY = mobile ? .36 : .32','vec3 filmic','float irisBand','fxCoreOpticsR424',
  ...
  'if(!mobile){',"fxCoreIdleRenderR441='zero-frame'"
])assert.ok(bootstrapRenderer.includes(token),`missing current r442 WebGL release contract: ${token}`);
assert.doesNotMatch(bootstrapRenderer,/setInterval\s*\(/);
assert.equal((bootstrapRenderer.match(/requestAnimationFrame\(frame\)/g)||[]).length,2,'release r326 must expose one burst entry and one bounded continuation only');
...
])assert.ok(currentLoader.includes(token),`missing current r442 release loader contract: ${token}`);
assert.ok(currentLoader.indexOf("fxCurrentMagStartupR442='styles-ready-before-renderer'")<currentLoader.indexOf("await addScript(RENDERER,'data-fx-current-r326-r422')"),'release renderer must start after critical styles');

for(const token of [
  "VERSION='r267-closed-volume-crystal'",
  'frontCenter=[0,.015,.58]','backCenter=[0,-.008,-.36]',
  'innerFront','outerFront','innerBack','outerBack',
  'add(outerFront[i],outerBack[i],outerBack[j]','add(outerFront[i],outerBack[j],outerFront[j]',
  "getContext('webgl2'","getContext('webgl'",'gl.enable(gl.DEPTH_TEST)','gl.depthFunc(gl.LEQUAL)',
  'gl.drawArrays(gl.TRIANGLES,0,mesh.count)','closed-front-back-sidewalls','physical-z-depth-r267',
  'normal-based-two-light-soft-fresnel','event-driven-no-idle-raf','ResizeObserver','IntersectionObserver','pointerdown','pointermove'
...
for(const token of [
  'function r326Primary()',"fxCoreMobileAwardRevision==='new-crystal-organism-r326'",
  "fxCoreCrystalRevision==='r326-four-direction-living-facet-organism'","fxCrystalRendererRequest='r326-primary-preserved'",
  "fxCrystalRendererOwnershipR420='r326-exclusive-primary'","renderer:'single-webgl-crystal-organism-r326'",
  ...
  "fxCrystalRendererRequest='closed-volume-r267-fallback'","fxCrystalRendererOwnershipR420='r267-legacy-fallback'"
])assert.ok(portal.includes(token),`missing r326-primary / r267-fallback portal release ownership: ${token}`);

for(const token of [
  '.fx-core-r267-volume-canvas','opacity: .985 !important','brightness(.94)','contrast(.87)','saturate(.96)','blur(.26px)',
  'opacity: .98 !important','brightness(.92)','contrast(.85)','saturate(.95)','blur(.32px)','r268-mobile-optics-softened-bloom-and-rim'
...
for(const token of [
  'production-r418-final-mobile-header-owner','production-r418-final-mobile-header-owner-non-occluding',
  'position:relative!important','top:auto!important','position:absolute!important','right:122px!important','right:70px!important','right:12px!important',
...
for(const token of ['production-r440-direct-r326-restrained-bloom-native-soft-edge-mobile-review','production-r443-cross-device-decorative-overflow-clipped-no-information-loss']){
  assert.ok(currentMagCss.includes(token),`missing current MAG release compatibility contract: ${token}`);
}
...
for(const token of ['production-r442-final-mobile-mag-optics-owner-restrained-native-no-blur','brightness(.90)','contrast(.89)','saturate(.95)']){
  assert.ok(baseOpticsCss.includes(token),`missing r442 base release optics: ${token}`);
}
...
])assert.ok(balanceCompact.includes(token.replace(/\s+/g,'')),`missing r444 balanced release optics: ${token}`);
...
for(const token of ['formatx-mobile-mag-balance-r444.css?v=20260830-r444-readable-midlight-soft-edge','function ensureOpticsStyle()','data-fx-mobile-mag-balance-r444','ensureOpticsStyle();']){
  assert.ok(surfaceSheen.includes(token),`missing r444 release optics mount: ${token}`);
}
...
for(const source of [bootstrap,wrapper,bootstrapRenderer,volumeRenderer,portal,layout,bridge,currentLoader,surfaceSheen])new Function(source);
console.log('PASS: release validates r326 as authoritative production WebGL MAG with r442 18x36/two-pass/720k idle-zero rendering, r444 readable soft-edge phone optics, r267 fallback isolation, non-occluding header and bidirectional site↔MAG coupling.');
