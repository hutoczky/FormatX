...
for (const token of [
  'gl.enable(gl.DEPTH_TEST)','gl.drawArrays(gl.TRIANGLES','gl.POINTS','function outlineRadius(','function surfacePoint(','function buildShell(','function starContour(','function radialFacets(','reference-four-point-crystal-v1','sharp-four-tip-concave-crystal-v53','clean-faceted-refractive-glass-v53','moving-white-nucleus-concentric-spectral-rings-v53','single-context-adaptive-60-plus-fps','webglcontextlost'
]) assert.ok(desktopRenderer.includes(token), `missing desktop release renderer contract: ${token}`);

for (const token of [
  'award-four-point-concave-crystal-v56','function outlinePoint(','function surfacePoint(','function buildShell(','function starContour(','function radialFacets(','gl.enable(gl.DEPTH_TEST)','gl.drawArrays(gl.TRIANGLES','gl.POINTS','quadratic-bezier-cusped-outline-v56','bright-faceted-fresnel-glass-v56','white-reactor-nucleus-concentric-cyan-violet-rings-v56','single-context-adaptive-60-plus-fps','webglcontextlost'
]) assert.ok(mobileRenderer.includes(token), `missing mobile release renderer contract: ${token}`);

assert.match(desktopRenderer, /budget=mobile\?1100000:2500000/);
assert.match(mobileRenderer, /budget=1150000/);
assert.doesNotMatch(desktopRenderer + mobileRenderer, /new\s+Image\s*\(|drawImage\s*\(|createImageBitmap\s*\(|backgroundImage/i);
...
console.log('PASS: release MAG uses native WebGL2 v53 on desktop and sharp concave award v56 on mobile.');
