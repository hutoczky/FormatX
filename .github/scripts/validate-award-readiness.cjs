...
assert.equal(contract.layout_contract?.mobile?.controller, 'seamless-v7', 'public contract mobile seamless controller is stale');
assert.equal(contract.layout_contract?.mobile?.automatic_scroll_loop, true, 'public contract must expose the mobile seamless loop');
assert.equal(contract.layout_contract?.mobile?.visual_bridge, true, 'public contract mobile visual bridge regressed');
assert.equal(contract.layout_contract?.mobile?.transfer_mode, 'scrollend-or-idle', 'public contract mobile handoff mode regressed');
assert.equal(contract.layout_contract?.mobile?.native_momentum_preserved, true, 'public contract mobile momentum preservation regressed');
assert.equal(contract.layout_contract?.mobile?.finite_document, false, 'public contract must not describe mobile as finite');
...
assert.equal(scrollPolicy.mobile?.controller, 'seamless-v7', 'canonical mobile seamless-v7 controller regressed');
assert.equal(scrollPolicy.mobile?.automatic_loop, true, 'mobile automatic loop must remain enabled');
assert.equal(scrollPolicy.mobile?.visual_bridge, true, 'mobile visual bridge must remain enabled');
assert.equal(scrollPolicy.mobile?.automatic_page_position_changes, true, 'mobile boundary handoff positioning must remain enabled');
...
assert.equal(scrollPolicy.mobile?.native_momentum_preserved, true, 'mobile native momentum must remain preserved');
