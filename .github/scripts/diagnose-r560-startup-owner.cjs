...
const asEvent=e=>({name:e.name,cat:e.cat||'',ph:e.ph,startMs:Number(e.ts||0)/1000,durationMs:Number(e.dur||0)/1000,pid:e.pid,tid:e.tid,data:compactArgs(e.args)});
...
function taskRawEvidence(rawEvents,task,threadNames){
  ...
  const rawNested=inside
    .filter(e=>e.name!=='ThreadControllerImpl::RunTask'||e.ph!=='E')
...
(async()=>{
  ...
  const url=new URL(BASE);url.searchParams.set('r708_trace',Date.now().toString());
  ...
  const report={auditedSha:process.env.AUDITED_SHA||'',traceCategories:TRACE_CATEGORIES,runtime,errors,rendererMainThread:rendererKey||null,rendererThreadName:threadNames.get(rendererKey)||'',rendererTaskDetails,cpuProfile,topEvents:events.slice(0,160)};
  fs.writeFileSync(path.join(OUT,'report.json'),JSON.stringify(report,null,2)+'\n');
  console.log('R708_STARTUP_OWNER '+JSON.stringify({state:runtime.state,paints:runtime.paints,navigation:runtime.navigation,rendererMainThread:report.rendererMainThread,rendererThreadName:report.rendererThreadName,rendererTaskDetails:rendererTaskDetails.slice(0,4),cpuProfile,topEvents:events.slice(0,45),resources:runtime.resources.filter(r=>r.initiatorType==='script'||r.initiatorType==='link').slice(0,90)},null,2));
