...
adaptive_voice = read("docs/scifi-ui/scripts/organism-natural-voice.js")
...
require("loader version is v22", "safe-ready-v22" in loader and "safe-degraded-v22" in loader, results)
require("loader includes Organism voice v3", "organism-voice.js?v=20260730-organism-voice-3" in loader, results)
require("loader includes adaptive voice v2", "organism-natural-voice.js?v=20260730-natural-voice-2" in loader, results)
...
require("adaptive voice does not block system voices", "level === 'legacy' || level === 'standard'" not in adaptive_voice, results)
require("adaptive voice resumes browser speech", "synth.resume()" in adaptive_voice, results)
require("adaptive voice has start watchdog", "watchdog = window.setTimeout" in adaptive_voice, results)
require("adaptive voice has browser default fallback", "browser-default" in adaptive_voice, results)
...
for worker_source, label in ((root_worker, "preview Worker"), (production_entry, "production Worker")):
    ...
    require(f"{label} serves adaptive Organism voice", "organism-natural-voice.js" in worker_source, results)
...
require("preview routing includes adaptive Organism voice", "/scifi-ui/scripts/organism-natural-voice.js" in preview_config, results)
