...
async function verify(browser, spec) {
  ...
  await page.addInitScript(() => {
    ...
    const observeCover = () => {
      ...
      if (overlay instanceof HTMLElement) {
        ...
        const hidden = overlay.hidden && (style.display === 'none' || style.visibility === 'hidden');
        if (!hidden && overlay.dataset.fxPreloaderR531 === 'active' && evidence.introVisibleAt == null) evidence.introVisibleAt = performance.now();
        if (hidden && evidence.completeAt != null) {
          evidence.introActuallyHiddenAt = performance.now();
          return;
        }
      }
      requestAnimationFrame(observeCover);
    };
    requestAnimationFrame(observeCover);
  ...
  try {
    ...
    await page.waitForFunction(() => Number.isFinite(window.__fxR544PreloaderEvidence?.introActuallyHiddenAt), null, { timeout: 3000 });
    state = await page.evaluate(() => {
      ...
      return {
        ...
        lifecycle:window.FormatXIntroTiming||{},
        magTiming:{workerRequestedAt:Number(root.dataset.fxMagWorkerRequestedAtR721||NaN),workerInitEnteredAt:Number(root.dataset.fxMagWorkerInitEnteredAtR721||NaN),firstUsefulFrameAt:Number(root.dataset.fxMagFirstFrameAtR721||NaN)},
        ...
        canvasCount:document.querySelectorAll('#hero .fx-crystal-organism-r326-canvas').length, magRequest:root.dataset.fxCurrentMagRequestR530||'', crystal:root.dataset.fxCrystalOrganismR326||'', renderer:root.dataset.fxCoreRenderer||'', shaderCompile:root.dataset.fxCoreShaderCompileR550||'',
    ...
    const rawCompleteAt=state.evidence.completeAt, completeAt=rawCompleteAt==null?NaN:Number(rawCompleteAt), source=String(state.evidence.source||state.release||''), duration=completeAt-state.bootAt, active=state.evidence.activeContract;
    const t=state.lifecycle;
    state.releaseTiming={...t,introVisibleAt:state.evidence.introVisibleAt,introActuallyHiddenAt:state.evidence.introActuallyHiddenAt,releaseRequestDelay:t.releaseRequestedAt-t.bootAt,releaseExecutionLag:t.releaseCallbackEnteredAt-t.releaseRequestedAt,releaseCommitLag:state.evidence.introActuallyHiddenAt-t.releaseCallbackEnteredAt,releaseQueueLag:t.releaseCallbackEnteredAt-t.releaseDueAt,logicalIntroDuration:duration,observedHiddenDuration:state.evidence.introActuallyHiddenAt-t.bootAt};
    try {
      assert.notEqual(rawCompleteAt,null,`${spec.name}: preloader completion event was not measured`); assert.ok(Number.isFinite(completeAt),`${spec.name}: invalid preloader completion timestamp`); assert.ok(Number.isFinite(state.bootAt)); assert.equal(state.timing,spec.timing); assert.equal(state.clock,'navigation-script-boot-single-deadline'); assert.equal(state.preloader,'done'); assert.equal(state.overlayHidden,true); assert.equal(state.heroVisible,true); assert.equal(state.pauseCount,0); assert.ok(state.overflow<=2); assert.ok(!/runtime-error|promise-error/.test(source));
...
(async()=>{
  ...
  const specs=[{name:'mobile-390x844',viewport:{width:390,height:844},mobile:true,min:1180,max:1450,visualMax:1360,timing:'mobile-1180-1450'}, {name:'desktop-1440x900',viewport:{width:1440,height:900},mobile:false,min:1350,max:1650,visualMax:1640,timing:'desktop-1350-1650'}];
  const results=[],failures=[];
  for(const spec of specs){
    try{results.push(await verifyFresh(spec));}
    catch(error){failures.push({error:String(error?.stack||error),evidence:error?.fxEvidence||null});}
  }
  if(failures.length){
    const error=new Error(`${failures.length} bounded intro profile(s) failed`);
    error.fxEvidence={failures,passed:results};
    throw error;
  }
