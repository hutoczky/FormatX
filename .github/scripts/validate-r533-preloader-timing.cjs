...
function testUrl(name) { const url = new URL(BASE); url.searchParams.set('r544_preloader', `${name}-${Date.now()}`); return url.href; }
...
async function verify(browser, spec) {
  ...
  await page.addInitScript(() => {
    window.__fxR544PreloaderEvidence = { completeAt: null, source: null, activeContract: null, magReadyAt: null };
    const capture = () => {
      const evidence = window.__fxR544PreloaderEvidence; if (!evidence) return;
    ...
    document.addEventListener('formatx:preloadercomplete', event => { capture(); window.__fxR544PreloaderEvidence.completeAt = performance.now(); window.__fxR544PreloaderEvidence.source = String(event?.detail?.source || ''); observer.disconnect(); }, { once: true, capture: true });
  });
  try {
    ...
    const state = await page.evaluate(() => { const root=document.documentElement, overlay=document.getElementById('formatx-event-horizon'), hero=document.getElementById('hero'), hs=hero?getComputedStyle(hero):null; return { evidence:window.__fxR544PreloaderEvidence||{}, timing:root.dataset.fxPreloaderTimingR533||'', clock:root.dataset.fxPreloaderClockR544||'', bootAt:Number(root.dataset.fxPreloaderBootR533||NaN), lateSkip:root.dataset.fxPreloaderLateSkipR533==='true', preloader:root.dataset.fxPreloaderR531||'', release:root.dataset.fxPreloaderReleaseR531||'', overlayHidden:overlay?overlay.hidden:true, overlayDisplay:overlay?getComputedStyle(overlay).display:'none', heroVisible:Boolean(hero&&hs&&hs.display!=='none'&&hs.visibility!=='hidden'&&hero.getBoundingClientRect().height>0), pauseCount:document.querySelectorAll('.fx-reference-pause').length, overflow:Math.max(document.documentElement.scrollWidth,document.body.scrollWidth)-innerWidth }; });
...
(async()=>{
  ...
  const report={ auditedSha:process.env.AUDITED_SHA||'', contract:'r544-fresh-profile-extended-static-intro-mag-behind-compositor-bounded', mobile, desktop };
  fs.writeFileSync(path.join(OUT,'report.json'),JSON.stringify(report,null,2)+'\n'); console.log('R544_PRELOADER_TIMING_PASS'); console.log(JSON.stringify(report,null,2));
