...
assert.match(runtime, /loading="eager"/, 'post-intent showcase images must load immediately after deferred mount');
assert.doesNotMatch(runtime, /loading="lazy"/, 'showcase must not add a second lazy gate after user-intent deferral');
