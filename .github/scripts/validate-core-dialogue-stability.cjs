...
assert.match(scrollBootstrap, /mobile-native-document-v1/);
assert.match(scrollBootstrap, /automaticPagePositionChanges: false/);
assert.doesNotMatch(scrollBootstrap, /scrollTo\s*\(|scrollIntoView\s*\(|cloneNode\s*\(|preventDefault\s*\(/);
assert.match(desktopScroll, /const VERSION = 'seamless-v7'/);
assert.match(desktopScroll, /root\.dataset\.fxInfiniteInput = 'native'/);
assert.match(desktopScroll, /visualBridge: true/);
assert.match(desktopScroll, /sourceHero\.cloneNode\(true\)/);
assert.match(desktopScroll, /window\.scrollTo\(/);
assert.doesNotMatch(desktopScroll, /addEventListener\(['"](?:wheel|touchmove)['"]/);

assert.equal(scrollPolicy.mobile.controller, 'mobile-native-document-v1');
assert.equal(scrollPolicy.mobile.automatic_loop, false);
assert.equal(scrollPolicy.mobile.visual_bridge, false);
assert.equal(scrollPolicy.mobile.automatic_page_position_changes, false);
...
console.log('PASS: core visual calibration and dialogue stability remain intact with mobile native-document and desktop seamless-v7 scrolling.');