(() => {
  ...
  class FormatXGenesisThree {
    ...
    makeCore(){
      ...
      const diamondMat=new T.MeshBasicMaterial({
        color:0x2b7d8d,transparent:true,opacity:.032,
      ...
      const diamondEdge=new T.LineSegments(
        ...
        new T.LineBasicMaterial({
          color:0x69c8d7,transparent:true,opacity:.13,
          depthWrite:false,blending:T.AdditiveBlending
        })
      );
    ...
    updateCore(t,time){
      let sc=.001;
      if(t<2.55)sc=.43+ease(t/2.55)*.17;
      else if(t<3.35)sc=mix(.60,.76,smooth((t-2.55)/.80));
      else if(t<5.65)sc=.76+Math.sin((t-3.35)*1.02)*.004;
      else if(t<8.95)sc=mix(.76,.60,smooth((t-5.65)/3.30));
      else sc=.57;
      ...
      const pulse=.977+.023*Math.sin(time*.0050);

      this.coreShell.material.opacity=early*(.66+.10*eyeHold);
      this.coreGlass.material.opacity=early*(.035+.045*eyeHold);
      this.coreEdges.material.opacity=early*(.030+.045*eyeHold);

      this.irisGroup.scale.setScalar((.06+eyeHold*.96)*pulse);
      this.irisRays.rotation.z=time*.000072;
      this.glowSprite.material.opacity=(.002+eyeHold*.13)*pulse;
      this.glowSprite.scale.setScalar(1.08+eyeHold*.28);
      this.coreInner.material.opacity=.005+eyeHold*.20;
      this.coreLight.intensity=eyeHold*(3.6+Math.sin(time*.005)*.34);
    ...
    updateOrganic(t,time){
      ...
      this.organicShellMaterial.opacity=.55*visible;
      this.organicLobeMaterial.opacity=.38*visible;
      this.organicWireMaterial.opacity=.001*visible;
      this.organicVeinMaterial.opacity=.76*visible;
      this.organicHoodMaterial.opacity=.62*visible;
      if(this.organicFoldMaterial)this.organicFoldMaterial.opacity=.94*visible;
    ...
    updateCamera(t,time){
      ...
      if(t<2.55){
        ...
        z=mix(5.25,5.55,k);
        x=Math.sin(time*.00031)*.095*(1-k*.35);
        y=.020+Math.sin(time*.00027)*.034;
      }else if(t<3.40){
        const k=smooth((t-2.55)/.85);
        z=mix(5.55,4.02,k);
        x=mix(.038,0,k);y=mix(.028,0,k);
      }else if(t<5.55){
        z=4.02+Math.sin(time*.00024)*.013;
        x=Math.sin(time*.00018)*.009;
        ...
        z=mix(4.62,4.76,smooth((t-9.10)/.60));
        y=.007;
      }
      if(this.width<this.height)z+=1.00;
  ...
  async function attach(canvas,getTarget){
    ...
    try{
      ...
      if(!visualProof && isSoftwareWebGL()){
        document.documentElement.dataset.fxMagBirthR720='software-webgl-r649-fallback';
        return null;
      }
      loader ||= loadThree();
      ...
      return {
        ...
        engine,
        revision:'r720-organic-reference-fused-three-genesis'
      };
    }catch(error){
      console.error('FormatX R720 genesis renderer failed:',error);
      document.documentElement.dataset.fxMagBirthR720='fallback-r649';
      return null;
    }
  }
  ...
  window.FormatXMagGenesisThreeR720={
    attach,
    revision:'r720-organic-reference-fused-three-genesis-dna-cellular-living-architecture'
  };
})();