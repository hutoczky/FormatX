(() => {
  ...
  class FormatXGenesisThree {
      ...
      for(let pair=0;pair<foldPairs;pair++){
        ...
        for(const side of [-1,1]){
          ...
          const fold=new T.Mesh(
            new T.TubeGeometry(curve,40,.034+r()*.009,7,false),
    ...
    updateCore(t,time){
      ...
      else if(t<3.30) sc=mix(.72,.41,smooth((t-2.70)/.60));
      else if(t<5.70) sc=.41;
      else if(t<9.55) sc=mix(.41,.35,smooth((t-5.70)/3.85));
      else sc=.35;
      ...
      this.irisGroup.scale.setScalar((.010+eyeHold*.94)*pulse);
    ...
    updateOrganic(t,time){
      ...
      this.organicVeinMaterial.opacity=.90*visible;
      this.organicHoodMaterial.opacity=.16*visible;
      if(this.organicFoldMaterial)this.organicFoldMaterial.opacity=.96*visible;
      if(this.organicFoldGlowMaterial)this.organicFoldGlowMaterial.opacity=.22*visible;
    ...
    updateTentacles(t,time){
      ...
      this.tentacleNodeMaterial.opacity=.78*grow;
      if(this.tentacleGlowMaterial)this.tentacleGlowMaterial.opacity=.16*grow;
  ...
  async function attach(canvas,getTarget){
    ...
    try{
      ...
      if(!visualProof && isSoftwareWebGL()){
        document.documentElement.dataset.fxMagBirthR980='software-webgl-r649-fallback';
        return null;
      }
      loader ||= loadThree();
      ...
      return {
        ...
        engine,
        revision:'r980-neural-surface-three-genesis'
      };
    }catch(error){
      console.error('FormatX R980 genesis renderer failed:',error);
      document.documentElement.dataset.fxMagBirthR980='fallback-r649';
      return null;
    }
  }
  ...
  window.FormatXMagGenesisThreeR980={
    attach,
    revision:'r980-neural-surface-three-genesis-dna-cellular-living-architecture'
  };
})();