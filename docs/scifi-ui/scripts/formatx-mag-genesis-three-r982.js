(() => {
  ...
  class FormatXGenesisThree {
    ...
    makeCore(){
      ...
      const shellGlass=new T.MeshPhysicalMaterial({
        ...
        emissive:0x0b3340,emissiveIntensity:.14,
    ...
    makeOrganicLayer(){
      ...
      this.organicLobes=[];
      const lobeGeo=new T.IcosahedronGeometry(.190,3);
      const count=54;
      for(let i=0;i<count;i++){
        const phi=Math.acos(1-2*(i+.5)/count);
        const theta=Math.PI*(1+Math.sqrt(5))*i;
        const rr=1.30+(r()-.5)*.13;
      ...
      for(let pair=0;pair<foldPairs;pair++){
        ...
        for(const side of [-1,1]){
          ...
          const fold=new T.Mesh(
            new T.TubeGeometry(curve,40,.020+r()*.006,7,false),
            this.organicFoldMaterial
          );
          const glow=new T.Mesh(
            new T.TubeGeometry(curve,40,.0034,5,false),
      ...
      for(let i=0;i<40;i++){
        ...
        const vein=new T.Mesh(
          new T.TubeGeometry(curve,26,.008+r()*.004,6,false),
    ...
    makeTentacles(){
      ...
      this.tentacleMaterial=new T.MeshStandardMaterial({
        color:0x08131a,metalness:.44,roughness:.34,
        emissive:0x07313b,emissiveIntensity:.34,
        transparent:true,opacity:0
      });
      ...
      this.tentacleNodeMaterial=new T.PointsMaterial({
        color:0x9bedf7,size:.055,transparent:true,opacity:0,
        depthWrite:false,blending:T.AdditiveBlending,sizeAttenuation:true
      });
    ...
    updateCore(t,time){
      let sc;
      if(t<2.70) sc=.46+ease(t/2.70)*.08;
      else if(t<3.30) sc=mix(.54,.57,smooth((t-2.70)/.60));
      else if(t<5.70) sc=.57;
      else if(t<9.55) sc=mix(.57,.50,smooth((t-5.70)/3.85));
      else sc=.50;
      ...
      const surfaceZ=cellular*.82;

      this.coreGroup.position.set(tx,ty,surfaceZ);
      this.organicGroup.position.set(tx,ty,0);
      this.cellGroup.position.set(tx,ty,0);
      // Armor is placed on the front face of the living sphere.
      this.mechanicalGroup.position.set(tx,ty,.72);
      ...
      const mature=smooth((t-5.85)/.90);
      const pulse=.982+.018*Math.sin(time*.0046);

      // Stage 1 stays mostly hidden behind foreground DNA.
      this.coreShell.material.opacity=.34*stage1 + .44*(1-stage1);
      this.coreGlass.material.opacity=.060*stage1 + .035*(1-stage1);
      this.coreEdges.material.opacity=.070*stage1 + .038*(1-stage1);
      if(this.corePetalMaterial)this.corePetalMaterial.opacity=.026*stage1+.052*(1-stage1);

      // Large embedded optical iris, matching the dominant reference focal point.
      this.irisGroup.scale.setScalar((.010+eyeHold*1.48+mature*.08)*pulse);
      this.irisRays.rotation.z=time*.000060;
      if(this.irisCorona)this.irisCorona.material.opacity=.98*eyeHold+.10*mature;
      this.glowSprite.material.opacity=(.001+eyeHold*.060+mature*.010)*pulse;
      this.glowSprite.scale.setScalar(1.00+eyeHold*.22);
      this.coreInner.material.opacity=.002+eyeHold*.090+mature*.012;
      this.coreLight.intensity=eyeHold*(5.6+Math.sin(time*.0046)*.30)+mature*.8;

      if(this.coreLabel)this.coreLabel.material.opacity=.060*stage1;
      if(this.coreContours){
        this.coreContours.forEach((m,i)=>{
          m.material.opacity=(.055-i*.018)*stage1+.014*(1-stage1);
    ...
    updateOrganic(t,time){
      ...
      this.organicWireMaterial.opacity=.0004*visible;
      this.organicVeinMaterial.opacity=.72*visible;
      this.organicHoodMaterial.opacity=.10*visible;
      if(this.organicFoldMaterial)this.organicFoldMaterial.opacity=.58*visible;
      if(this.organicFoldGlowMaterial)this.organicFoldGlowMaterial.opacity=.095*visible;

      if(this.organicHoodGroup){
        this.organicHoodGroup.scale.setScalar(.54);
    ...
    updateMechanical(t,time){
      const grow=smooth((t-5.95)/1.10);
      this.mechanicalGroup.visible=grow>.002;
      this.mechanicalGroup.scale.setScalar(.001+grow*.88);

      this.mechMaterial.opacity=.11*grow;
      this.mechMidMaterial.opacity=.18*grow;
      this.silverMaterial.opacity=.94*grow;
      this.mechEdgeMaterial.opacity=.030*grow;
      this.mechInnerMaterial.opacity=.24*grow;
      if(this.seamMaterial)this.seamMaterial.opacity=.34*grow;
      if(this.mechEyeCorona)this.mechEyeCorona.material.opacity=.48*grow;
      this.mechInnerRing.rotation.z=time*.00012;
      if(this.mechLight)this.mechLight.intensity=4.2*grow;

      if(this.mechBody){
        this.mechBody.rotation.y=Math.sin(time*.00018)*.006*grow;
        this.mechBody.rotation.x=Math.sin(time*.00015)*.004*grow;
      }
      if(this.mechCradle)this.mechCradle.rotation.z=Math.sin(time*.00016)*.005*grow;
      this.silverParts.forEach((p,i)=>{
        p.rotation.y=Math.sin(time*.00015+i)*.005*grow;
      });
    }
    ...
    updateTentacles(t,time){
      ...
      this.tentacleMaterial.opacity=.86*grow*flashFade;
      this.tentacleEdgeMaterial.opacity=.010*grow;
      this.tentacleNodeMaterial.opacity=.92*grow;
      if(this.tentacleGlowMaterial)this.tentacleGlowMaterial.opacity=.26*grow;
    ...
    updateCamera(t,time){
      ...
      if(t<2.70){
        ...
        z=5.18+Math.sin(time*.00017)*.009;
        y=.004;
      }
  ...
  async function attach(canvas,getTarget){
    ...
    try{
      ...
      if(!visualProof && isSoftwareWebGL()){
        document.documentElement.dataset.fxMagBirthR982='software-webgl-r649-fallback';
        return null;
      }
      loader ||= loadThree();
      ...
      return {
        ...
        engine,
        revision:'r982-reference-balance-three-genesis'
      };
    }catch(error){
      console.error('FormatX R982 genesis renderer failed:',error);
      document.documentElement.dataset.fxMagBirthR982='fallback-r649';
      return null;
    }
  }
  ...
  window.FormatXMagGenesisThreeR982={
    attach,
    revision:'r982-reference-balance-three-genesis-dna-cellular-living-architecture'
  };
})();