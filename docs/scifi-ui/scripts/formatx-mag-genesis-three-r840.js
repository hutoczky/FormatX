(() => {
  ...
  class FormatXGenesisThree {
    ...
    updateCore(t,time){
      ...
      this.coreShell.material.opacity=early*(.72+.10*eyeHold);
      this.coreGlass.material.opacity=early*(.040+.045*eyeHold);
      this.coreEdges.material.opacity=early*(.025+.035*eyeHold);
      if(this.corePetalMaterial)this.corePetalMaterial.opacity=early*(.32+.08*eyeHold);
      ...
      this.glowSprite.material.opacity=(.001+eyeHold*.055)*pulse;
      this.glowSprite.scale.setScalar(1.02+eyeHold*.18);
      this.coreInner.material.opacity=.003+eyeHold*.075;
      this.coreLight.intensity=eyeHold*(2.35+Math.sin(time*.0048)*.20);
    ...
    updateOrganic(t,time){
      ...
      this.organicLobeMaterial.opacity=.50*visible;
      this.organicWireMaterial.opacity=.001*visible;
      this.organicVeinMaterial.opacity=.88*visible;
    ...
    updateCells(t,time){
      ...
      this.cellMaterial.opacity=.055*visible;
      this.cellEdgeMaterial.opacity=.0015*visible;
      this.cellVeinMaterial.opacity=.16*visible;
    ...
    updateTentacles(t,time){
      ...
      this.tentacleNodeMaterial.opacity=.48*grow;
      if(this.tentacleGlowMaterial)this.tentacleGlowMaterial.opacity=.86*grow;
  ...
  async function attach(canvas,getTarget){
    ...
    try{
      ...
      if(!visualProof && isSoftwareWebGL()){
        document.documentElement.dataset.fxMagBirthR840='software-webgl-r649-fallback';
        return null;
      }
      loader ||= loadThree();
      ...
      return {
        ...
        engine,
        revision:'r840-biotic-volume-locked-three-genesis'
      };
    }catch(error){
      console.error('FormatX R840 genesis renderer failed:',error);
      document.documentElement.dataset.fxMagBirthR840='fallback-r649';
      return null;
    }
  }
  ...
  window.FormatXMagGenesisThreeR840={
    attach,
    revision:'r840-biotic-volume-locked-three-genesis-dna-cellular-living-architecture'
  };
})();