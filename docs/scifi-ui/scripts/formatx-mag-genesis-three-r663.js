(() => {
  ...
  class FormatXGenesisThree {
    ...
    makeCore(){
      ...
      this.coreLabel.scale.set(.78,.195,1);
      this.coreLabel.position.set(0,-.05,.34);
    ...
    updateCore(t,time){
      ...
      else if(t>=2.55 && t<3.35)sc=mix(1.24,.78,smooth((t-2.55)/.80));
      else if(t>=3.35 && t<5.65)sc=.78+Math.sin((t-3.35)*1.05)*.008;
      else if(t>=5.65 && t<8.95)sc=mix(.78,.94,smooth((t-5.65)/3.30));
      ...
      this.irisGroup.scale.setScalar(
        (phase1*.48 + phase2Boost*1.02 + finalBoost*1.18 + .10)*pulse
      );
      this.irisRays.rotation.z=time*.00010;
      this.glowSprite.material.opacity=(.04+.08*phase1+.62*phase2Boost+.58*finalBoost)*pulse;
      this.glowSprite.scale.setScalar(1.42+phase1*.22+phase2Boost*.42+finalBoost*.36);
      this.coreInner.material.opacity=.14+.10*phase1+.68*phase2Boost+.70*finalBoost;
      this.coreEdges.material.opacity=.22+.18*eye;
      this.coreLight.intensity=eye*(2.8+phase1*1.4+phase2Boost*10+finalBoost*11+Math.sin(time*.005)*1.4);
      if(this.coreLabel)this.coreLabel.material.opacity=.30*(1-smooth((t-2.60)/.50))+.055*finalBoost;
    }
    ...
    render(r,time){
      ...
      this.renderer.toneMappingExposure=1.18+flash*.82+after*.06;
  ...
  async function attach(canvas,getTarget){
    ...
    try{
      ...
      if(!visualProof && isSoftwareWebGL()){
        document.documentElement.dataset.fxMagBirthR663='software-webgl-r649-fallback';
        return null;
      }
      loader ||= loadThree();
      ...
      return {
        ...
        engine,
        revision:'r663-surface-core-three-genesis'
      };
    }catch(error){
      console.error('FormatX R663 genesis renderer failed:',error);
      document.documentElement.dataset.fxMagBirthR663='fallback-r649';
      return null;
    }
  }
  ...
  window.FormatXMagGenesisThreeR663={
    attach,
    revision:'r663-surface-core-three-genesis-dna-cellular-living-architecture'
  };
})();