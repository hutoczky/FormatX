(() => {
  ...
  class FormatXGenesisThree {
    ...
    updateMechanical(t,time){
      const pre=smooth((t-8.90)/.46);
      const flashReveal=smooth((t-9.30)/.28);
      const grow=pre*.32+flashReveal*.68;
      this.mechanicalGroup.visible=grow>.002;
      this.mechanicalGroup.scale.setScalar(.001+grow*.99);
      this.mechMaterial.opacity=.88*grow;
      this.mechEdgeMaterial.opacity=.33*grow;
      this.plates.forEach((p,i)=>{
        p.rotation.z+=(i%2?1:-1)*.00018;
        p.rotation.x=Math.sin(time*.00031+i)*.018*grow;
      });
    }

    updateTentacles(t,time){
      const grow=smooth((t-5.42)/1.08);
      const settle=1-smooth((t-9.42)/.50)*.08;
      this.tentacleGroup.visible=grow>.002;
      this.tentacleMaterial.opacity=.72*grow*settle;
      this.tentacleEdgeMaterial.opacity=.22*grow*settle;
      this.tentacles.forEach((g,i)=>{
        const wave=1+Math.sin(time*.00113+g.userData.phase)*.026*grow;
        const sy=.001+grow*.999;
        g.scale.set(sy*wave,sy*wave,sy*wave);
        g.rotation.z=Math.sin(time*.00062+g.userData.phase)*.055*grow;
        g.rotation.x=Math.sin(time*.00051+g.userData.phase)*.035*grow;
      });
    }
    ...
    updateCamera(t,time){
      ...
      if(t<2.50){
        ...
        z=mix(5.82,6.46,k);
        x=Math.sin(time*.00042)*.27*(1-k*.34);
        y=.06+Math.sin(time*.00033)*.10;
      }else if(t<3.42){
        const k=smooth((t-2.50)/.92);
        z=mix(6.46,4.20,k);
        x=mix(.10,0,k);
        y=mix(.10,.01,k);
      }else if(t<5.50){
        z=4.20+Math.sin(time*.00031)*.042;
        x=Math.sin(time*.00023)*.045;
        y=Math.cos(time*.00027)*.032;
      }else if(t<8.85){
        const k=smooth((t-5.50)/3.35);
        z=mix(4.20,6.78,k);
        x=Math.sin(time*.00019)*.035*(1-k);
        y=mix(.01,.03,k);
      }else{
        z=mix(6.78,7.05,smooth((t-8.85)/.92));
        y=.03;
      }
  ...
  async function attach(canvas,getTarget){
    ...
    try{
      ...
      return {
        ...
        engine,
        revision:'r651-frame-matched-three-genesis'
      };
    }catch(error){
      console.error('FormatX R651 genesis renderer failed:',error);
      document.documentElement.dataset.fxMagBirthR651='fallback-r649';
      return null;
    }
  }

  window.FormatXMagGenesisThreeR651={
    attach,
    revision:'r651-frame-matched-three-genesis-dna-cellular-living-architecture'
  };
})();