...
function shapeState(){const core=renderer();const target=root.dataset.fxCoreTargetShape||root.dataset.fxCoreShapeR337||core?.shape||'';const settled=root.dataset.fxCoreShape||'';const morph=Number(core?.morph);const targetMorph=target==='sphere'?1:(target==='crystal'||target==='organism'?0:NaN);return{core,target,settled,morph,targetMorph,ready:Boolean(core)&&Number.isFinite(morph)&&Number.isFinite(targetMorph)&&settled===target&&Math.abs(morph-targetMorph)<.015};}
...
function arm(){if(armed)return;armed=true;root.dataset.fxMobileRenderGovernorR426='ready';root.dataset.fxCoreMobileIdlePolicyR426='periodic-surface-bursts-between-zero-idle';root.dataset.fxMobileRenderGovernorRevisionR433='r528-automatic-idle-flag-no-manual-pause';root.dataset.fxMobileRenderGovernorOrganismR1723='fixed-organism-morph-zero-settle';root.dataset.fxMobileSurfaceBudgetR484='full-1160ms-sweep-then-zero-idle';requestAnimationFrame(()=>idle('startup-painted-r528'));}
