(() => {
  ...
  class FormatXGenesisThree {
    ...
    makeCore(){
      ...
      const petalDefs=[
        [0,.19,.16,0,1.00,.94],
        [.19,0,.15,-Math.PI/2,.94,1.00],
        [0,-.19,.15,Math.PI,.99,.94],
        [-.19,0,.15,Math.PI/2,.94,1.00]
      ];
      ...
      const ring1=new T.Mesh(new T.TorusGeometry(.278,.018,10,80),cyan.clone());
      ring1.material.opacity=.46;
      const ring2=new T.Mesh(new T.TorusGeometry(.405,.008,8,80),cyan.clone());
      ring2.material.opacity=.19;
      const pupil=new T.Mesh(new T.CircleGeometry(.102,64),new T.MeshBasicMaterial({color:0x073441,side:T.DoubleSide}));
      const pupilRing=new T.Mesh(new T.RingGeometry(.110,.164,72),cyan.clone());
      pupilRing.material.opacity=.48;
    ...
    updateCore(t,time){
      ...
      else if(t<3.35)sc=mix(.58,.84,smooth((t-2.55)/.80));
      else if(t<5.65)sc=.84+Math.sin((t-3.35)*1.02)*.003;
      else if(t<8.95)sc=mix(.84,.48,smooth((t-5.65)/3.30));
      else sc=.46;
      ...
      this.coreShell.material.opacity=early*(.24+.06*eyeHold);
      this.coreGlass.material.opacity=early*(.025+.040*eyeHold);
      this.coreEdges.material.opacity=early*(.028+.042*eyeHold);
      if(this.corePetalMaterial)this.corePetalMaterial.opacity=early*(.84+.10*eyeHold);

      this.irisGroup.scale.setScalar((.052+eyeHold*.93)*pulse);
      this.irisRays.rotation.z=time*.000070;
      this.glowSprite.material.opacity=(.002+eyeHold*.14)*pulse;
      this.glowSprite.scale.setScalar(1.05+eyeHold*.22);
      this.coreInner.material.opacity=.005+eyeHold*.16;
      this.coreLight.intensity=eyeHold*(2.9+Math.sin(time*.005)*.28);

      if(this.coreLabel)this.coreLabel.material.opacity=.16*early*(1-smooth((t-2.62)/.32));
    }
    ...
    updateOrganic(t,time){
      ...
      this.organicLobeMaterial.opacity=.40*visible;
      this.organicWireMaterial.opacity=.001*visible;
      this.organicVeinMaterial.opacity=.74*visible;
      this.organicHoodMaterial.opacity=.46*visible;
      if(this.organicFoldMaterial)this.organicFoldMaterial.opacity=.98*visible;
      if(this.organicFoldGlowMaterial)this.organicFoldGlowMaterial.opacity=.29*visible;
    ...
    updateCamera(t,time){
      ...
      if(t<2.55){
        ...
        z=4.04+Math.sin(time*.00024)*.013;
        ...
        z=mix(6.00,6.18,smooth((t-9.10)/.60));
        y=.007;
      }
      if(this.width<this.height)z+=1.04;
  ...
  async function attach(canvas,getTarget){
    ...
    try{
      ...
      if(!visualProof && isSoftwareWebGL()){
        document.documentElement.dataset.fxMagBirthR721='software-webgl-r649-fallback';
        return null;
      }
      loader ||= loadThree();
      ...
      return {
        ...
        engine,
        revision:'r721-organic-petal-final-three-genesis'
      };
    }catch(error){
      console.error('FormatX R721 genesis renderer failed:',error);
      document.documentElement.dataset.fxMagBirthR721='fallback-r649';
      return null;
    }
  }
  ...
  window.FormatXMagGenesisThreeR721={
    attach,
    revision:'r721-organic-petal-final-three-genesis-dna-cellular-living-architecture'
  };
})();