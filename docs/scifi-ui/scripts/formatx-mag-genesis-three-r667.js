(() => {
  ...
  class FormatXGenesisThree {
    ...
    makeMechanicalLayer(){
      ...
      this.mechMaterial=new T.MeshPhysicalMaterial({
        color:0x102837,metalness:.92,roughness:.17,
        emissive:0x0b2d3c,emissiveIntensity:.48,
        clearcoat:.74,clearcoatRoughness:.12,
        transparent:true,opacity:0
      });
      this.silverMaterial=new T.MeshPhysicalMaterial({
        color:0x8ea7b1,metalness:.94,roughness:.16,
        emissive:0x173541,emissiveIntensity:.24,
        clearcoat:.82,clearcoatRoughness:.10,
        transparent:true,opacity:0
      });
      this.mechEdgeMaterial=new T.LineBasicMaterial({
        color:0xaaf5ff,transparent:true,opacity:0,
        blending:T.AdditiveBlending,depthWrite:false
      });
      ...
      const topGeo=new T.ConeGeometry(.62,1.42,3,1,false);
      const topCenter=new T.Mesh(topGeo,this.silverMaterial);
      topCenter.scale.set(.72,1.00,.48);
      topCenter.position.set(0,.82,.08);
      topCenter.rotation.set(Math.PI/2,0,Math.PI);
      this.mechanicalGroup.add(topCenter);this.silverParts.push(topCenter);

      const topLeft=new T.Mesh(topGeo,this.silverMaterial);
      topLeft.scale.set(.50,.78,.40);
      topLeft.position.set(-.43,.62,.05);
      topLeft.rotation.set(Math.PI/2,.16,2.72);
      this.mechanicalGroup.add(topLeft);this.silverParts.push(topLeft);

      const topRight=new T.Mesh(topGeo,this.silverMaterial);
      topRight.scale.set(.50,.82,.40);
      topRight.position.set(.44,.61,.07);
      topRight.rotation.set(Math.PI/2,-.12,-2.72);
      this.mechanicalGroup.add(topRight);this.silverParts.push(topRight);

      const shoulderGeo=new T.ConeGeometry(.48,.96,3,1,false);
      const leftShoulder=new T.Mesh(shoulderGeo,this.mechMaterial);
      leftShoulder.scale.set(.58,1.10,.42);
      leftShoulder.position.set(-.78,.06,.02);
      leftShoulder.rotation.set(Math.PI/2,.18,-Math.PI/2);
      this.mechanicalGroup.add(leftShoulder);this.plates.push(leftShoulder);

      const rightShoulder=new T.Mesh(shoulderGeo,this.mechMaterial);
      rightShoulder.scale.set(.62,1.06,.43);
      rightShoulder.position.set(.80,.03,.03);
      rightShoulder.rotation.set(Math.PI/2,-.14,Math.PI/2);
      this.mechanicalGroup.add(rightShoulder);this.plates.push(rightShoulder);

      const jawGeo=new T.ConeGeometry(.42,1.04,3,1,false);
      const jawL=new T.Mesh(jawGeo,this.mechMaterial);
      jawL.scale.set(.48,.92,.40);
      jawL.position.set(-.25,-.75,.00);
      jawL.rotation.set(Math.PI/2,.12,.16);
      this.mechanicalGroup.add(jawL);this.plates.push(jawL);

      const jawR=new T.Mesh(jawGeo,this.mechMaterial);
      jawR.scale.set(.48,.96,.40);
      jawR.position.set(.26,-.77,.02);
      jawR.rotation.set(Math.PI/2,-.10,-.16);
      this.mechanicalGroup.add(jawR);this.plates.push(jawR);

      const spineGeo=new T.BoxGeometry(.13,.55,.18);
      const spineL=new T.Mesh(spineGeo,this.mechMaterial);
      spineL.position.set(-.14,-1.02,.03);
      spineL.rotation.z=.08;
      const spineR=new T.Mesh(spineGeo,this.mechMaterial);
      spineR.position.set(.15,-1.00,.04);
      spineR.rotation.z=-.07;
      ...
      this.mechInnerRing=new T.Mesh(new T.TorusGeometry(.49,.014,8,72),this.mechInnerMaterial);
      this.mechInnerRing.position.z=.31;
      this.mechanicalGroup.add(this.mechInnerRing);

      this.seamMaterial=new T.MeshBasicMaterial({
        color:0x62e9ff,transparent:true,opacity:0,
        depthWrite:false,blending:T.AdditiveBlending
      });
      this.seams=[];
      const seamGeo=new T.BoxGeometry(.105,.022,.025);
      const seamDefs=[
        [-.60,.08,.33,0],[.61,.06,.34,0],
        [-.13,-.82,.28,Math.PI/2],[.14,-.82,.29,Math.PI/2],
        [-.20,.72,.30,.30],[.21,.71,.30,-.28]
      ];
      for(const [x,y,z,rz] of seamDefs){
        const seam=new T.Mesh(seamGeo,this.seamMaterial);
        seam.position.set(x,y,z);seam.rotation.z=rz;
        this.mechanicalGroup.add(seam);this.seams.push(seam);
      }
    ...
    updateCore(t,time){
      let sc=.001;
      if(t>=.75 && t<2.55)sc=.06+ease((t-.75)/1.80)*1.18;
      else if(t>=2.55 && t<3.35)sc=mix(1.24,.60,smooth((t-2.55)/.80));
      else if(t>=3.35 && t<5.65)sc=.60+Math.sin((t-3.35)*1.05)*.006;
      else if(t>=5.65 && t<8.95)sc=mix(.60,.76,smooth((t-5.65)/3.30));
      else if(t>=8.95)sc=.76+smooth((t-8.95)/.72)*.03;
      ...
      const surfaceZ=phase2*1.28+finalSurface*.78;
      ...
      this.coreGroup.rotation.y=Math.sin(time*.00024)*.020;
      this.coreGroup.rotation.x=Math.sin(time*.00029)*.016;

      const eye=smooth((t-1.80)/.72);
      const phase1=smooth((t-1.55)/.70)*(1-smooth((t-2.55)/.45));
      const phase2Boost=smooth((t-2.82)/.50)*(1-smooth((t-6.05)/1.15));
      const finalBoost=smooth((t-6.10)/1.25);
      const pulse=.94+.06*Math.sin(time*.0054);
      this.irisGroup.scale.setScalar(
        (phase1*.20 + phase2Boost*.96 + finalBoost*1.12 + .07)*pulse
      );
      this.irisRays.rotation.z=time*.00010;
      this.glowSprite.material.opacity=(.022+.020*phase1+.48*phase2Boost+.50*finalBoost)*pulse;
      this.glowSprite.scale.setScalar(1.42+phase1*.22+phase2Boost*.42+finalBoost*.36);
      this.coreInner.material.opacity=.06+.04*phase1+.66*phase2Boost+.68*finalBoost;
      this.coreEdges.material.opacity=.22+.18*eye;
      this.coreLight.intensity=eye*(1.1+phase1*.40+phase2Boost*8.0+finalBoost*9.8+Math.sin(time*.005)*1.0);
      if(this.coreLabel)this.coreLabel.material.opacity=.42*(1-smooth((t-2.62)/.48))+.045*finalBoost;
    }
    ...
    updateMechanical(t,time){
      ...
      this.mechanicalGroup.scale.setScalar(.001+grow*1.36);
      this.mechMaterial.opacity=.99*grow;
      this.silverMaterial.opacity=.96*grow;
      this.mechEdgeMaterial.opacity=.38*grow;
      this.mechInnerMaterial.opacity=.62*grow;
      if(this.seamMaterial)this.seamMaterial.opacity=.58*grow;
      this.mechInnerRing.rotation.z=time*.00016;
      if(this.mechLight)this.mechLight.intensity=9.2*grow;
      this.plates.forEach((p,i)=>{
        p.rotation.y+=Math.sin(time*.00022+i)*.00008*grow;
      });
      this.silverParts.forEach((p,i)=>{
        p.rotation.y+=Math.sin(time*.00018+i*.8)*.00006*grow;
      });
    }
    ...
    render(r,time){
      ...
      this.renderer.toneMappingExposure=1.18+flash*.68+after*.05;
      this.coreLight.intensity+=flash*15+after*4;
      if(this.glowSprite){
        const g=1+flash*.52;
  ...
  async function attach(canvas,getTarget){
    ...
    try{
      ...
      if(!visualProof && isSoftwareWebGL()){
        document.documentElement.dataset.fxMagBirthR667='software-webgl-r649-fallback';
        return null;
      }
      loader ||= loadThree();
      ...
      return {
        ...
        engine,
        revision:'r667-armored-organic-three-genesis'
      };
    }catch(error){
      console.error('FormatX R667 genesis renderer failed:',error);
      document.documentElement.dataset.fxMagBirthR667='fallback-r649';
      return null;
    }
  }
  ...
  window.FormatXMagGenesisThreeR667={
    attach,
    revision:'r667-armored-organic-three-genesis-dna-cellular-living-architecture'
  };
})();