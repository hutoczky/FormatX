(() => {
  ...
  class FormatXGenesisThree {
    ...
    makeDebris(){
      ...
      const tubeA=new T.Mesh(new T.TubeGeometry(curveA,seg,.036,8,false),ma);
      const tubeB=new T.Mesh(new T.TubeGeometry(curveB,seg,.036,8,false),mb);
      ...
      const rungGeo=new T.CylinderGeometry(.020,.020,1,8,1,false);
    ...
    makeOrganicLayer(){
      ...
      this.organicLobes=[];
      const lobeGeo=new T.IcosahedronGeometry(.225,3);
      const count=60;
      for(let i=0;i<count;i++){
    ...
    updateCore(t,time){
      ...
      if(this.corePetalMaterial)this.corePetalMaterial.opacity=.42*stage1+.94*reveal*lateDim;

      this.irisGroup.scale.setScalar((.030+eyeHold*1.38)*pulse);
      this.irisRays.rotation.z=time*.000050;
      if(this.irisCorona)this.irisCorona.material.opacity=.94*eyeHold;
      ...
      this.coreLight.intensity=eyeHold*(3.8+Math.sin(time*.0046)*.16);
    ...
    updateOrganic(t,time){
      ...
      this.organicGroup.scale.set(bodyScale*1.24,bodyScale*1.20,bodyScale*1.04);

      this.organicShellMaterial.opacity=.64*visible;
      ...
      if(this.organicFoldGlowMaterial)this.organicFoldGlowMaterial.opacity=.038*visible;
    ...
    updateCamera(t,time){
      ...
      if(t<2.70){
        ...
        z=4.35+Math.sin(time*.00020)*.006;
        ...
        z=5.55+Math.sin(time*.00018)*.007;
        y=.003;
      }else{
        z=mix(5.55,5.48,smooth((t-9.10)/.60));
        y=.003;
      }
  ...
  async function attach(canvas,getTarget){
    ...
    try{
      ...
      if(!visualProof && isSoftwareWebGL()){
        document.documentElement.dataset.fxMagBirthR1340='software-webgl-r649-fallback';
        return null;
      }
      loader ||= loadThree();
      ...
      return {
        ...
        engine,
        revision:'r1340-reference-surface-radial-iris-lock'
      };
    }catch(error){
      console.error('FormatX R1340 genesis renderer failed:',error);
      document.documentElement.dataset.fxMagBirthR1340='fallback-r649';
      return null;
    }
  }
  ...
  window.FormatXMagGenesisThreeR1340={
    attach,
    revision:'r1340-reference-surface-radial-iris-lock-dna-cellular-living-architecture'
  };
})();