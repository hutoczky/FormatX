(() => {
  ...
  class FormatXGenesisThree {
    ...
    makeCore(){
      ...
      const shellGlass=new T.MeshPhysicalMaterial({
        ...
        emissive:0x0a2f39,emissiveIntensity:.10,
    ...
    makeOrganicLayer(){
      ...
      for(let i=0;i<58;i++){
        ...
        const fold=new T.Mesh(new T.TubeGeometry(curve,40,.030+r()*.010,7,false),this.organicFoldMaterial);
        const glow=new T.Mesh(new T.TubeGeometry(curve,40,.0055,5,false),this.organicFoldGlowMaterial);
    ...
    makeMechanicalLayer(){
      ...
      this.mechMidMaterial=new T.MeshPhysicalMaterial({
        color:0x0b202b,metalness:.86,roughness:.19,
        emissive:0x082b35,emissiveIntensity:.22,
        clearcoat:.76,clearcoatRoughness:.12,
        transparent:true,opacity:0
      });
      this.silverMaterial=new T.MeshPhysicalMaterial({
        color:0xb2c4ca,metalness:.95,roughness:.14,
        emissive:0x173641,emissiveIntensity:.14,
      ...
      baseShape.moveTo(0,.88);
      baseShape.bezierCurveTo(.28,.70,.66,.36,.74,.04);
      baseShape.bezierCurveTo(.62,-.34,.30,-.68,0,-.88);
      baseShape.bezierCurveTo(-.30,-.68,-.62,-.34,-.74,.04);
      baseShape.bezierCurveTo(-.66,.36,-.28,.70,0,.88);
      ...
      this.mechBody.scale.set(1.12,1.03,.92);
      ...
      const railGeo=new T.BoxGeometry(.34,.055,.10);
      this.seams=[];
      for(const x of [-.68,.68]){
        const rail=new T.Mesh(railGeo,this.seamMaterial);
        rail.position.set(x,.00,.40);
        this.mechanicalGroup.add(rail);
        this.seams.push(rail);
      }
      const spineGeo=new T.BoxGeometry(.065,.32,.10);
      ...
      this.mechInnerRing=new T.Mesh(
        new T.TorusGeometry(.25,.010,8,72),
    ...
    updateCore(t,time){
      let sc;
      if(t<2.70) sc=.92+ease(t/2.70)*.08;
      else if(t<3.25) sc=mix(1.00,.66,smooth((t-2.70)/.55));
      else if(t<6.30) sc=.66;
      else sc=mix(.66,.52,smooth((t-6.30)/.75));
      ...
      const surfaceZ=cellular*.84+mechanical*.47;
      ...
      this.coreShell.material.opacity=.96*stage1 + .40*(1-stage1)*(1-mech) + .06*mech;
      this.coreGlass.material.opacity=.10*stage1 + .05*(1-stage1)*(1-mech) + .015*mech;
      this.coreEdges.material.opacity=.095*stage1 + .055*(1-stage1)*(1-mech) + .015*mech;
      if(this.corePetalMaterial)this.corePetalMaterial.opacity=.06*stage1+.18*(1-stage1)*(1-mech)+.02*mech;

      this.irisGroup.scale.setScalar((.010+eyeHold*.64)*(1-mech*.94)*pulse);
      this.irisRays.rotation.z=time*.000060;
      if(this.irisCorona)this.irisCorona.material.opacity=.80*eyeHold*(1-mech*.92);
      this.glowSprite.material.opacity=(.001+eyeHold*.035+mech*.018)*pulse;
      this.glowSprite.scale.setScalar(1.00+eyeHold*.14+mech*.06);
      this.coreInner.material.opacity=.002+eyeHold*.060+mech*.025;
      this.coreLight.intensity=eyeHold*(2.15+Math.sin(time*.0046)*.18)*(1-mech*.88);

      if(this.coreLabel)this.coreLabel.material.opacity=.34*stage1;
      if(this.coreContours){
        this.coreContours.forEach((m,i)=>{
          m.material.opacity=(.13-i*.035)*stage1;
    ...
    updateOrganic(t,time){
      ...
      this.organicGroup.scale.set(bodyScale*1.10,bodyScale*1.06,bodyScale*1.02);

      this.organicShellMaterial.opacity=.84*visible;
    ...
    updateCells(t,time){
      ...
      this.cellVeinMaterial.opacity=.18*visible;
    ...
    updateMechanical(t,time){
      ...
      this.mechanicalGroup.scale.setScalar(.001+grow*1.20);

      this.mechMaterial.opacity=.995*grow;
      this.mechMidMaterial.opacity=.985*grow;
      this.silverMaterial.opacity=.92*grow;
      this.mechEdgeMaterial.opacity=.12*grow;
      this.mechInnerMaterial.opacity=.48*grow;
      if(this.seamMaterial)this.seamMaterial.opacity=.82*grow;
      if(this.mechEyeCorona)this.mechEyeCorona.material.opacity=.90*grow;
      this.mechInnerRing.rotation.z=time*.00012;
      if(this.mechLight)this.mechLight.intensity=7.6*grow;

      if(this.mechBody){
        this.mechBody.rotation.y=Math.sin(time*.00018)*.010*grow;
        this.mechBody.rotation.x=Math.sin(time*.00015)*.006*grow;
      }
      if(this.mechCradle)this.mechCradle.rotation.z=Math.sin(time*.00016)*.007*grow;
      this.silverParts.forEach((p,i)=>{
        p.rotation.y=Math.sin(time*.00015+i)*.008*grow;
      });
    }
    ...
    updateCamera(t,time){
      ...
      if(t<2.70){
        ...
        z=4.72+Math.sin(time*.00018)*.010;
        y=.004;
      }
  ...
  async function attach(canvas,getTarget){
    ...
    try{
      ...
      if(!visualProof && isSoftwareWebGL()){
        document.documentElement.dataset.fxMagBirthR952='software-webgl-r649-fallback';
        return null;
      }
      loader ||= loadThree();
      ...
      return {
        ...
        engine,
        revision:'r952-reference-material-depth-three-genesis'
      };
    }catch(error){
      console.error('FormatX R952 genesis renderer failed:',error);
      document.documentElement.dataset.fxMagBirthR952='fallback-r649';
      return null;
    }
  }
  ...
  window.FormatXMagGenesisThreeR952={
    attach,
    revision:'r952-reference-material-depth-three-genesis-dna-cellular-living-architecture'
  };
})();