(() => {
  ...
  class FormatXGenesisThree {
    ...
    makeDebris(){
      ...
      const tubeA=new T.Mesh(new T.TubeGeometry(curveA,seg,.026,9,false),ma);
      const tubeB=new T.Mesh(new T.TubeGeometry(curveB,seg,.026,9,false),mb);
      const auraA=new T.Mesh(new T.TubeGeometry(curveA,seg,.043,8,false),ga);
      const auraB=new T.Mesh(new T.TubeGeometry(curveB,seg,.043,8,false),gb);

      const rungGeo=new T.CylinderGeometry(.013,.013,1,8,1,false);
    ...
    makeDNAField(){
      ...
      for(const [x,y,z,rz,sc] of placements){
        const h=this.createHelix(4.42,.288,4.36);
    ...
    makeOrganicLayer(){
      ...
      this.organicFoldMaterial=new T.MeshPhysicalMaterial({
        color:0x65546c,roughness:.64,metalness:.003,
        emissive:0x14242d,emissiveIntensity:.10,
    ...
    updateCore(t,time){
      ...
      if(this.corePetalMaterial)this.corePetalMaterial.opacity=.46*stage1+.76*reveal*lateDim;

      this.irisGroup.scale.setScalar((.026+eyeHold*1.08)*pulse);
      ...
      this.coreLight.intensity=eyeHold*(3.15+Math.sin(time*.0046)*.14);
    ...
    updateOrganic(t,time){
      ...
      this.organicShellMaterial.opacity=.38*visible;
      this.organicLobeMaterial.opacity=.92*visible;
      this.organicWireMaterial.opacity=.0003*visible;
      this.organicVeinMaterial.opacity=.065*visible;
      this.organicHoodMaterial.opacity=.24*visible;
      if(this.organicFoldMaterial)this.organicFoldMaterial.opacity=.105*visible;
      if(this.organicFoldGlowMaterial)this.organicFoldGlowMaterial.opacity=.008*visible;
  ...
  async function attach(canvas,getTarget){
    ...
    try{
      ...
      if(!visualProof && isSoftwareWebGL()){
        document.documentElement.dataset.fxMagBirthR1350='software-webgl-r649-fallback';
        return null;
      }
      loader ||= loadThree();
      ...
      return {
        ...
        engine,
        revision:'r1350-video-proportion-organic-final-lock'
      };
    }catch(error){
      console.error('FormatX R1350 genesis renderer failed:',error);
      document.documentElement.dataset.fxMagBirthR1350='fallback-r649';
      return null;
    }
  }
  ...
  window.FormatXMagGenesisThreeR1350={
    attach,
    revision:'r1350-video-proportion-organic-final-lock-dna-cellular-living-architecture'
  };
})();