/* FormatX R865 — cinematic scene director.
   Scene-state only. Native scrolling, anchors, controls and layout ownership stay untouched.
   The visual layer is allowed to preload, but scene observation starts only after the
   canonical Event Horizon preloader completes so P0 timing/LCP ownership is untouched. */
(() => {
  'use strict';
  ...
  const styleHref = new URL('../styles/formatx-cinematic-system-r860.css?v=20260919-r865-post-intro-scene-director', script?.src || document.baseURI).href;

  /* Network fetch may begin early, but the stylesheet remains inert until
     fx-cinematic-ready is added after canonical preloader completion. */
  ...
  function queueBoot(source) {
    root.dataset.fxCinematicBootSourceR865 = source;
    requestAnimationFrame(() => setTimeout(boot, 0));
  }

  if (root.dataset.fxPreloaderR531 === 'done') {
    queueBoot('preloader-already-done');
  } else {
    document.addEventListener('formatx:preloadercomplete', () => queueBoot('preloader-complete'), { once: true });
    /* Defensive fallback remains beyond the product intro maximum. It exists only
       for non-canonical pages where the event owner is absent. */
    fallbackTimer = setTimeout(() => queueBoot('post-intro-fallback'), 2300);
  }
})();
