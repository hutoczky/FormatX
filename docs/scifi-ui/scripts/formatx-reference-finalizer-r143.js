...
if(root.dataset.fxCoreCompositionR285==='pure-webgl3d-no-2d-overlays'){
  ...
  root.dataset.fxReferenceTailSchedulerR277='retired-no-idle-work-r292';
  return;
}
...
function ensure(){
  ...
  while(node&&node!==hero){
    ...
    if(node.matches('.fx-core-r112-stage,.fx-core-mobile-v55-stage')){imp(node,'box-shadow','none');imp(node,'border','0');imp(node,'outline','0');}
    node=node.parentElement;
  }
  imp(hero,'overflow','visible');
  if(!(tail instanceof HTMLCanvasElement)||!tail.isConnected){
    tail=document.createElement('canvas');tail.className='fx-core-reference-tail-r143';tail.setAttribute('aria-hidden','true');grid.appendChild(tail);
    tailCtx=tail.getContext('2d',{alpha:true,desynchronized:true});Object.assign(tail.style,{position:'absolute',pointerEvents:'none',zIndex:'7',display:'block'});layoutDirty=true;
  }
  observe(stage,grid);return{hero,grid,stage,detail};
}
function measure(host){
  const {grid,stage}=host,sr=stage.getBoundingClientRect(),gr=grid.getBoundingClientRect();
  const targetH=sr.width,extra=Math.max(0,targetH-sr.height);
  ...
  if(tail.style.left!==leftPx)tail.style.left=leftPx;if(tail.style.top!==topPx)tail.style.top=topPx;if(tail.style.width!==widthPx)tail.style.width=widthPx;if(tail.style.height!==heightPx)tail.style.height=heightPx;
  ...
  geom={stageHeight:sr.height,targetH,extra};layoutDirty=false;root.dataset.fxReferenceFinalizerR143='ready-r252';root.dataset.fxReferenceTailSchedulerR277='bounded-event-burst';root.dataset.fxReferenceTailBridgeR143=`${extra.toFixed(2)}px`;return geom;
}
function retryBoot(){if(bootTimer||document.hidden||bootTries>=16)return;bootTimer=setTimeout(()=>{bootTimer=0;requestBurst(1);},120);}
function draw(now){
  raf=0;if(document.hidden)return;const host=ensure();if(!host||!tailCtx){bootTries++;retryBoot();return;}bootTries=0;
  const g=layoutDirty||!geom?measure(host):geom;if(!g){burstFrames=0;return;}if(now-lastDraw<FRAME_INTERVAL){schedule();return;}
