(() => {
  ...
  class FormatXGenesisThree {
    ...
    makeCore(){
      ...
      const shellMat=new T.MeshPhysicalMaterial({
        color:0x213844,metalness:.82,roughness:.22,
    ...
    makeOrganicLayer(){
      ...
      for(let i=0;i<46;i++){
        ...
        const vein=new T.Mesh(
          new T.TubeGeometry(curve,28,.0075+r()*.0035,6,false),
        ...
        if(i%5===0){
          ...
          const branch=new T.Mesh(
            new T.TubeGeometry(new T.QuadraticBezierCurve3(end,bmid,branchEnd),18,.0055+r()*.002,5,false),
            this.organicVeinMaterial
          );
    ...
    updateCore(t,time){
      ...
      else if(t<3.30) sc=mix(.86,.68,smooth((t-2.70)/.60));
      else if(t<5.70) sc=.68;
      else if(t<7.60) sc=mix(.68,.57,smooth((t-5.70)/1.90));
      else if(t<9.55) sc=.57;
      else sc=.52;
      ...
      this.coreShell.material.opacity=.92*stage1 + .48*(1-stage1);
      this.coreGlass.material.opacity=.16*stage1 + .052*(1-stage1);
      this.coreEdges.material.opacity=.24*stage1 + .070*(1-stage1);
      if(this.corePetalMaterial)this.corePetalMaterial.opacity=.065*stage1+.040*(1-stage1);

      this.irisGroup.scale.setScalar((.010+eyeHold*.70)*pulse);
      this.irisRays.rotation.z=time*.000060;
      if(this.irisCorona)this.irisCorona.material.opacity=.72*eyeHold;
      this.glowSprite.material.opacity=(.001+eyeHold*.018)*pulse;
      this.glowSprite.scale.setScalar(1.00+eyeHold*.09);
      this.coreInner.material.opacity=.002+eyeHold*.032;
      this.coreLight.intensity=eyeHold*(2.30+Math.sin(time*.0046)*.16);
    ...
    updateOrganic(t,time){
      ...
      this.organicVeinMaterial.opacity=.58*visible;
      this.organicHoodMaterial.opacity=.28*visible;
      if(this.organicFoldMaterial)this.organicFoldMaterial.opacity=.72*visible;
      if(this.organicFoldGlowMaterial)this.organicFoldGlowMaterial.opacity=.24*visible;
  ...
  async function attach(canvas,getTarget){
    ...
    try{
      ...
      if(!visualProof && isSoftwareWebGL()){
        document.documentElement.dataset.fxMagBirthR1060='software-webgl-r649-fallback';
        return null;
      }
      loader ||= loadThree();
      ...
      return {
        ...
        engine,
        revision:'r1060-embedded-cellular-core-three-genesis'
      };
    }catch(error){
      console.error('FormatX R1060 genesis renderer failed:',error);
      document.documentElement.dataset.fxMagBirthR1060='fallback-r649';
      return null;
    }
  }
  ...
  window.FormatXMagGenesisThreeR1060={
    attach,
    revision:'r1060-embedded-cellular-core-three-genesis-dna-cellular-living-architecture'
  };
})();