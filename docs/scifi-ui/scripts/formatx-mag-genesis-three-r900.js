(() => {
  ...
  class FormatXGenesisThree {
    ...
    makeMechanicalLayer(){
      ...
      this.mechBody.scale.set(1.14,.93,.90);
      ...
      this.crown.scale.set(.92,.88,.82);
      this.crown.position.set(0,.34,.30);
      ...
      leftShoulder.scale.set(1.08,.84,.84);
      leftShoulder.position.set(-.58,.02,.25);
      ...
      rightShoulder.scale.set(-1.08,.84,.84);
      rightShoulder.position.set(.58,.02,.25);
      ...
      this.jaw.scale.set(.90,.74,.88);
      this.jaw.position.set(0,-.42,.22);
    ...
    updateCore(t,time){
      ...
      this.irisGroup.scale.setScalar((.010+eyeHold*.64+mech*.12)*pulse);
      this.irisRays.rotation.z=time*.000060;
      if(this.irisCorona)this.irisCorona.material.opacity=(.80*eyeHold+.18*mech);
      ...
      this.coreLight.intensity=eyeHold*(2.15+Math.sin(time*.0046)*.18)+mech*.95;

      if(this.coreLabel)this.coreLabel.material.opacity=.28*stage1;
    }
    ...
    updateMechanical(t,time){
      ...
      this.mechanicalGroup.scale.setScalar(.001+grow*1.15);
      ...
      if(this.mechLight)this.mechLight.intensity=6.8*grow;
  ...
  async function attach(canvas,getTarget){
    ...
    try{
      ...
      if(!visualProof && isSoftwareWebGL()){
        document.documentElement.dataset.fxMagBirthR900='software-webgl-r649-fallback';
        return null;
      }
      loader ||= loadThree();
      ...
      return {
        ...
        engine,
        revision:'r900-three-stage-reference-genesis'
      };
    }catch(error){
      console.error('FormatX R900 genesis renderer failed:',error);
      document.documentElement.dataset.fxMagBirthR900='fallback-r649';
      return null;
    }
  }
  ...
  window.FormatXMagGenesisThreeR900={
    attach,
    revision:'r900-three-stage-reference-genesis-dna-cellular-living-architecture'
  };
})();