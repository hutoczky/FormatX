(() => {
  ...
  class FormatXGenesisThree {
    ...
    makeDNAField(){
      const placements=[
        [-1.95,1.52,.92,-.52,1.34],
        [2.02,1.46,.78,.58,1.28],
        [-2.14,-1.34,.86,.39,1.40],
        [2.16,-1.26,.78,-.47,1.32],
        [-.36,2.38,.24,1.42,1.05],
        [.54,-2.30,.20,1.66,1.06],
        [-3.10,.18,-.70,1.28,.84],
        [3.08,-.14,-.80,1.74,.80],
        [-1.10,.20,-1.30,-.22,.78],
        [1.46,.32,-1.40,.18,.74]
      ...
      for(const [x,y,z,rz,sc] of placements){
        const h=this.createHelix(4.82,.325,3.82);
    ...
    makeTentacles(){
      ...
      this.tentacleMaterial=new T.MeshStandardMaterial({
        color:0x101b22,metalness:.52,roughness:.32,
        emissive:0x06171d,emissiveIntensity:.12,
        transparent:true,opacity:0
      });
    ...
    updateCore(t,time){
      ...
      this.coreShell.material.opacity=.96*stage1+.34*reveal*lateDim;
      this.coreGlass.material.opacity=.10*stage1+.035*reveal*lateDim;
      this.coreEdges.material.opacity=.18*stage1+.030*reveal*lateDim;
      if(this.corePetalMaterial)this.corePetalMaterial.opacity=.76*stage1+.62*reveal*lateDim;

      this.irisGroup.scale.setScalar((.030+eyeHold*.98)*pulse);
      this.irisRays.rotation.z=time*.000050;
      if(this.irisCorona)this.irisCorona.material.opacity=.98*eyeHold;
      this.glowSprite.material.opacity=(.02+eyeHold*.42)*pulse;
      this.glowSprite.scale.setScalar(.98+eyeHold*.34);
      this.coreInner.material.opacity=.015+eyeHold*.42;
      this.coreLight.intensity=eyeHold*(7.6+Math.sin(time*.0046)*.34);
    ...
    updateOrganic(t,time){
      ...
      this.organicLobeMaterial.opacity=.86*visible;
      ...
      if(this.organicFoldMaterial)this.organicFoldMaterial.opacity=.24*visible;
    ...
    updateTentacles(t,time){
      ...
      this.tentacleMaterial.opacity=(.92+.04*afterGlow)*grow;
      this.tentacleEdgeMaterial.opacity=(.004+.008*afterGlow)*grow;
      this.tentacleNodeMaterial.opacity=.66*grow;
      if(this.tentacleGlowMaterial)this.tentacleGlowMaterial.opacity=(.06+.18*afterGlow)*grow;
    ...
    updateCamera(t,time){
      let z=5.30,y=.012,x=0;
      if(t<2.70){
        const k=smooth(t/2.70);
        z=mix(5.36,5.18,k);
        x=Math.sin(time*.00027)*.036*(1-k*.35);
        ...
        z=4.76+Math.sin(time*.00020)*.006;
        ...
        z=6.28+Math.sin(time*.00018)*.008;
        y=.003;
      }else{
        z=mix(6.28,6.18,smooth((t-9.10)/.60));
        y=.003;
      }
  ...
  async function attach(canvas,getTarget){
    ...
    try{
      ...
      if(!visualProof && isSoftwareWebGL()){
        document.documentElement.dataset.fxMagBirthR1320='software-webgl-r649-fallback';
        return null;
      }
      loader ||= loadThree();
      ...
      return {
        ...
        engine,
        revision:'r1320-frame-framing-eye-dna-final-core-lock'
      };
    }catch(error){
      console.error('FormatX R1320 genesis renderer failed:',error);
      document.documentElement.dataset.fxMagBirthR1320='fallback-r649';
      return null;
    }
  }
  ...
  window.FormatXMagGenesisThreeR1320={
    attach,
    revision:'r1320-frame-framing-eye-dna-final-core-lock-dna-cellular-living-architecture'
  };
})();