...
function fallback(reason){
  ...
  root.dataset.fxMagOffscreenR564=`fallback-${reason}`;root.dataset.fxMagOffscreenR565=`fallback-${reason}`;
...
try{
  ...
  root.dataset.fxCrystalOrganismR326='booting';root.dataset.fxCoreReal3d='booting';root.dataset.fxMagStartupContractR530='living-core-autostart-navigation-owned';root.dataset.fxCurrentMagRequestR530='navigation-owned-r564-offscreen-started';root.dataset.fxMagOffscreenR564='worker-started-under-intro';root.dataset.fxMagOffscreenR565='worker-created-init-scheduled-post-critical-paint';root.dataset.fxMagWorkerInitAtR565=String(WORKER_INIT_AT_MS);
  ...
  initTimer=setTimeout(()=>{if(destroyed||fallbackStarted||!worker)return;const start=size()||initial;initPosted=true;root.dataset.fxMagOffscreenR565='worker-init-started-post-critical-paint';worker.postMessage({type:'init',canvas:offscreen,width:start.width,height:start.height},[offscreen]);readyTimer=setTimeout(()=>{if(!ready)fallback('ready-timeout');},5500);},initDelay);
