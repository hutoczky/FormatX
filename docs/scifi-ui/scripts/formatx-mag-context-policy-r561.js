/* FormatX R595 — all-device OffscreenCanvas MAG startup owner.
   ...
   Desktop keeps navigation-owned zero-input startup but expensive WebGL init can
   no longer race an unpainted page when paint PerformanceObserver is supported:
   it waits for an observed first-paint entry with the existing 620ms floor.
   The 1750ms fallback is retained only for browsers where paint observation is
   unavailable. No Lighthouse, CI, audit, intro-complete or user-input gate. */
(function(){
'use strict';
  ...
  if(mobile){
    ...
    if(alreadyPainted){armAfterPaint('buffered-first-paint');}
      ...
      if(typeof PerformanceObserver==='function'){
        ...
        if(!Array.isArray(supported)||supported.includes('paint')){
          try{
            paintObserver=new PerformanceObserver(list=>{if(list.getEntries().some(entry=>entry.name==='first-paint'))armAfterPaint('observed-first-paint');});
            paintObserver.observe({type:'paint',buffered:true});
            paintObservationSupported=true;
          }catch(_){paintObserver=null;}
        }
      }
      if(paintObservationSupported){
        root.dataset.fxMagDesktopDecontentionR595='paint-observer-supported-no-unpainted-init';
      }else{
        root.dataset.fxMagDesktopDecontentionR595='paint-observer-unavailable-fallback-1750';
        paintFallbackTimer=setTimeout(()=>armAfterPaint('paint-api-unavailable-fallback-1750'),DESKTOP_INIT_FALLBACK_MS);
      }
    }
  }
