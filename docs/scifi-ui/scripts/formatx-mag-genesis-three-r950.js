(() => {
  ...
  class FormatXGenesisThree {
    ...
    makeMechanicalLayer(){
      ...
      this.silverMaterial=new T.MeshPhysicalMaterial({
        color:0x9aabb2,metalness:.94,roughness:.16,
        emissive:0x152f38,emissiveIntensity:.12,
    ...
    updateCore(t,time){
      ...
      const surfaceZ=cellular*1.05+mechanical*.47;
      ...
      this.coreShell.material.opacity=.88*stage1 + .46*(1-stage1)*(1-mech) + .08*mech;
      this.coreGlass.material.opacity=.13*stage1 + .07*(1-stage1)*(1-mech) + .02*mech;
      this.coreEdges.material.opacity=.14*stage1 + .07*(1-stage1)*(1-mech) + .02*mech;
    ...
    updateOrganic(t,time){
      ...
      this.organicGroup.scale.set(bodyScale*1.06,bodyScale*1.02,bodyScale);
    ...
    updateCells(t,time){
      ...
      this.cellGroup.scale.set(cellScale*1.04,cellScale,cellScale);
  ...
  async function attach(canvas,getTarget){
    ...
    try{
      ...
      if(!visualProof && isSoftwareWebGL()){
        document.documentElement.dataset.fxMagBirthR950='software-webgl-r649-fallback';
        return null;
      }
      loader ||= loadThree();
      ...
      return {
        ...
        engine,
        revision:'r950-reference-geometry-three-genesis'
      };
    }catch(error){
      console.error('FormatX R950 genesis renderer failed:',error);
      document.documentElement.dataset.fxMagBirthR950='fallback-r649';
      return null;
    }
  }
  ...
  window.FormatXMagGenesisThreeR950={
    attach,
    revision:'r950-reference-geometry-three-genesis-dna-cellular-living-architecture'
  };
})();