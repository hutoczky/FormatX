(() => {
  ...
  function startR649Fallback(){
    ...
    if(window.FormatXMagReferenceFilmR649?.attach){
      ...
      ROOT.dataset.fxMagBirthRendererR952=filmRenderer?'native-canvas-reference-film':'fallback-particles';
  ...
  function sizeCanvas() {
    ...
    if(AUTOMATION && FORCE && !VISUAL_PROOF){
      ROOT.dataset.fxMagBirthRendererR952='automation-handoff-lightweight';
      canvas.hidden=true;
      return;
    }
    if(filmRenderer){
    ...
    if(window.FormatXMagGenesisThreeR952?.attach){
      if(!filmRendererPromise){
        ROOT.dataset.fxMagBirthRendererR952='loading-threejs';
        filmRendererPromise=Promise.resolve(
          window.FormatXMagGenesisThreeR952.attach(canvas,()=>({x:targetX,y:targetY}))
        ).then(renderer=>{
          ...
          if(renderer){
            ...
            ROOT.dataset.fxMagBirthRendererR952='threejs-active';
            renderer.resize?.();
            return renderer;
          }
          ROOT.dataset.fxMagBirthRendererR952='threejs-failed-r649-fallback';
          startR649Fallback();
          return null;
        }).catch(error=>{
          filmRendererPromise=null;
          ROOT.dataset.fxMagBirthRendererR952='threejs-error-r649-fallback';
          console.error('FormatX R952 intro attach failed:',error);
          startR649Fallback();
        });
      }
      return;
