(() => {
  ...
  function startR649Fallback(){
    ...
    if(window.FormatXMagReferenceFilmR649?.attach){
      ...
      ROOT.dataset.fxMagBirthRendererR1060=filmRenderer?'native-canvas-reference-film':'fallback-particles';
  ...
  function sizeCanvas() {
    ...
    if(AUTOMATION && FORCE && !VISUAL_PROOF){
      ROOT.dataset.fxMagBirthRendererR1060='automation-handoff-lightweight';
      canvas.hidden=true;
      return;
    }
    if(filmRenderer){
    ...
    if(window.FormatXMagGenesisThreeR1060?.attach){
      if(!filmRendererPromise){
        ROOT.dataset.fxMagBirthRendererR1060='loading-threejs';
        filmRendererPromise=Promise.resolve(
          window.FormatXMagGenesisThreeR1060.attach(canvas,()=>({x:targetX,y:targetY}))
        ).then(renderer=>{
          ...
          if(renderer){
            ...
            ROOT.dataset.fxMagBirthRendererR1060='threejs-active';
            renderer.resize?.();
            return renderer;
          }
          ROOT.dataset.fxMagBirthRendererR1060='threejs-failed-r649-fallback';
          startR649Fallback();
          return null;
        }).catch(error=>{
          filmRendererPromise=null;
          ROOT.dataset.fxMagBirthRendererR1060='threejs-error-r649-fallback';
          console.error('FormatX R1060 intro attach failed:',error);
          startR649Fallback();
        });
      }
      return;
