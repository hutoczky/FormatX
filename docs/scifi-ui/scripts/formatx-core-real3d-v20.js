(function () {
  'use strict';
  // r91 ray-glass shipment; preserve r87 size and replace the flat membrane renderer with refractive WebGL material.
  const root = document.documentElement;
  const BOOTSTRAP = 'responsive-cinematic-reference-v69-r91-rayglass';
  const MOBILE_SCRIPT = '/scifi-ui/scripts/formatx-core-mobile-v55.js?v=20260814-rayglass-r91';
  ...
  const RAYGLASS_STYLE = '/scifi-ui/styles/formatx-award-material-r91.css?v=20260814-rayglass-r91';
  ...
  if (root.dataset.fxCoreReal3dBootstrap === BOOTSTRAP && root.dataset.fxCoreMaterialR91 === 'ready') return;
  ...
  root.dataset.fxCoreRenderer = 'single-webgl-refractive-rayglass-r91';
  ...
  function addStyle(href, attr, ready) {
    ...
    if (ready) l.addEventListener('load', () => { root.dataset[ready] = 'ready'; }, { once:true });
  ...
  function addMobileStyle(){
    ...
    addStyle(RAYGLASS_STYLE,'data-fx-award-material-r91','fxCoreMaterialR91');
  ...
  function addFlowGuard(){if(document.querySelector('script[data-fx-flow-first-r75]'))return;const s=document.createElement('script');s.src=FLOW_SCRIPT;s.async=false;s.dataset.fxFlowFirstR75='true';s.addEventListener('load',()=>{root.dataset.fxFlowFirstGuard='ready-r91';},{once:true});s.addEventListener('error',()=>{root.dataset.fxFlowFirstGuard='failed-r91';},{once:true});document.head.appendChild(s);}
