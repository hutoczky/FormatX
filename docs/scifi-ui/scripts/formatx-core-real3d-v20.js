(function () {
  'use strict';
  // r121 bootstrap: native XYZ WebGL mesh + screenshot-locked prismatic caustic overlay.
  const root = document.documentElement;
  const BOOTSTRAP = 'responsive-cinematic-reference-v69-r99-luminous-interactive-r121';
  ...
  function addFlowGuard(){if(document.querySelector('script[data-fx-flow-first-r75]'))return;const s=document.createElement('script');s.src=FLOW_SCRIPT;s.async=false;s.dataset.fxFlowFirstR75='true';s.addEventListener('load',()=>{root.dataset.fxFlowFirstGuard='ready-r121';},{once:true});s.addEventListener('error',()=>{root.dataset.fxFlowFirstGuard='failed-r121';},{once:true});document.head.appendChild(s);}
