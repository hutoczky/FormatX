(function () {
  'use strict';
  // r94 native renderer with r95 optical match pass + r96 interactive motion; preserve r87 size and layout.
  const root = document.documentElement;
  const BOOTSTRAP = 'responsive-cinematic-reference-v69-r96-interactive';
  const MOBILE_SCRIPT = '/scifi-ui/scripts/formatx-core-mobile-v55.js?v=20260814-interactive-motion-r96';
  ...
  root.dataset.fxCoreReal3d='loading-v69';root.dataset.fxCoreRenderer='single-webgl-refractive-rayglass-r96';root.dataset.fxCoreReferenceLock='loading-v69';
  ...
  function addFlowGuard(){if(document.querySelector('script[data-fx-flow-first-r75]'))return;const s=document.createElement('script');s.src=FLOW_SCRIPT;s.async=false;s.dataset.fxFlowFirstR75='true';s.addEventListener('load',()=>{root.dataset.fxFlowFirstGuard='ready-r96';},{once:true});s.addEventListener('error',()=>{root.dataset.fxFlowFirstGuard='failed-r96';},{once:true});document.head.appendChild(s);}
