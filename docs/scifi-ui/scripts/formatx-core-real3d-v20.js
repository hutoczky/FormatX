(function () {
  'use strict';
  // r129 bootstrap: native XYZ WebGL depth + fractured crystal volume and reflective floor matched to the supplied mobile reference.
  const root = document.documentElement;
  const BOOTSTRAP = 'responsive-cinematic-reference-v69-r99-luminous-interactive-r129';
  ...
  const DETAIL_STYLE = '/scifi-ui/styles/formatx-core-detail-overlay-r122.css?v=20260815-fractured-volume-floor-r129';
  const DETAIL_SCRIPT = '/scifi-ui/scripts/formatx-core-detail-overlay-r122.js?v=20260815-fractured-volume-floor-r129';
  ...
  function addExactStyle(){addStyle(EXACT_STYLE,'data-fx-reference-exact-r112','fxCoreExactR112');addStyle(DETAIL_STYLE,'data-fx-core-detail-r122-style','fxCoreDetailStyleR122');}
  ...
  function addFlowGuard(){if(document.querySelector('script[data-fx-flow-first-r75]'))return;const s=document.createElement('script');s.src=FLOW_SCRIPT;s.async=false;s.dataset.fxFlowFirstR75='true';s.addEventListener('load',()=>{root.dataset.fxFlowFirstGuard='ready-r129';},{once:true});s.addEventListener('error',()=>{root.dataset.fxFlowFirstGuard='failed-r129';},{once:true});document.head.appendChild(s);}
