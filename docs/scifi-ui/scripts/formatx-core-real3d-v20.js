(function () {
  'use strict';
  // r120 bootstrap: native XYZ WebGL mesh, prismatic organic glass, screenshot-locked mobile framing.
  const root = document.documentElement;
  const BOOTSTRAP = 'responsive-cinematic-reference-v69-r99-luminous-interactive-r120';
  const MOBILE_SCRIPT = '/scifi-ui/scripts/formatx-core-mobile-v55.js?v=20260815-prismatic-organic-r120';
  ...
  const EXACT_STYLE = '/scifi-ui/styles/formatx-reference-exact-r112.css?v=20260815-prismatic-organic-r120';
  ...
  function addFlowGuard(){if(document.querySelector('script[data-fx-flow-first-r75]'))return;const s=document.createElement('script');s.src=FLOW_SCRIPT;s.async=false;s.dataset.fxFlowFirstR75='true';s.addEventListener('load',()=>{root.dataset.fxFlowFirstGuard='ready-r120';},{once:true});s.addEventListener('error',()=>{root.dataset.fxFlowFirstGuard='failed-r120';},{once:true});document.head.appendChild(s);}
