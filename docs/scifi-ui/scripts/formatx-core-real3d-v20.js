(function () {
  'use strict';
  // r116 bootstrap: actual 3D WebGL faceted mesh + organic refractive fracture volume.
  const root = document.documentElement;
  const BOOTSTRAP = 'responsive-cinematic-reference-v69-r99-luminous-interactive-r116';
  const MOBILE_SCRIPT = '/scifi-ui/scripts/formatx-core-mobile-v55.js?v=20260814-luminous-cinematic-r99&rev=20260815-refractive-fracture-r116';
  ...
  function addFlowGuard(){if(document.querySelector('script[data-fx-flow-first-r75]'))return;const s=document.createElement('script');s.src=FLOW_SCRIPT;s.async=false;s.dataset.fxFlowFirstR75='true';s.addEventListener('load',()=>{root.dataset.fxFlowFirstGuard='ready-r116';},{once:true});s.addEventListener('error',()=>{root.dataset.fxFlowFirstGuard='failed-r116';},{once:true});document.head.appendChild(s);}
