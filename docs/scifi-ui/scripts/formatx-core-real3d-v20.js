(function () {
  'use strict';
  // r137 bootstrap: full supplied reference pixels + exact mobile proof typography + native WebGL interaction.
  const root = document.documentElement;
  const BOOTSTRAP = 'responsive-cinematic-reference-v69-r99-luminous-interactive-r137';
  ...
  const DETAIL_STYLE = '/scifi-ui/styles/formatx-core-detail-overlay-r122.css?v=20260815-reference-material-r136';
  const FINAL_STYLE = '/scifi-ui/styles/formatx-reference-final-r132.css?v=20260815-pixel-lock-r137';
  const DETAIL_SCRIPT = '/scifi-ui/scripts/formatx-core-detail-overlay-r122.js?v=20260815-reference-material-r136';
  ...
  const FLOW_SCRIPT = '/scifi-ui/scripts/formatx-flow-first-r75.js?v=20260815-reference-geometry-r137';
  ...
  function addReferenceLayout(){if(document.querySelector('script[data-fx-mobile-reference-layout]')){addExactStyle();addReferenceCopy();return;}const s=document.createElement('script');s.src=LAYOUT_SCRIPT;s.async=false;s.dataset.fxMobileReferenceLayout='true';s.addEventListener('load',()=>{addExactStyle();addFlowGuard();addReferenceCopy();},{once:true});document.head.appendChild(s);}
  function addDetailScript(){if(document.querySelector('script[data-fx-core-detail-r122], script[src*="formatx-core-detail-overlay-r122.js"]'))return;const s=document.createElement('script');s.src=DETAIL_SCRIPT;s.async=false;s.dataset.fxCoreDetailR122='true';s.addEventListener('load',()=>{root.dataset.fxCoreDetailLoadR122='ready-r136';},{once:true});s.addEventListener('error',()=>{root.dataset.fxCoreDetailLoadR122='failed-r136';},{once:true});document.head.appendChild(s);}
  function addMobileScript(){if(document.querySelector('script[data-fx-core-mobile-v55-script], script[src*="formatx-core-mobile-v55.js"]')){addDetailScript();return;}const s=document.createElement('script');s.src=MOBILE_SCRIPT;s.async=false;s.dataset.fxCoreMobileV55Script='true';s.addEventListener('load',()=>{root.dataset.fxCoreReferenceLockLoad='ready-v69-r137';addDetailScript();},{once:true});s.addEventListener('error',()=>{root.dataset.fxCoreReal3d='context-unavailable';root.dataset.fxCoreReferenceLock='load-failed-v69';root.dataset.fxCoreReferenceLockLoad='failed-v69-r137';dispatchEvent(new CustomEvent('formatx:core3dfallback',{detail:{reason:'mobile-reference-material-load-failed',reference:'v69-r137'}}));},{once:true});document.head.appendChild(s);}
  ...
  function addFlowGuard(){if(document.querySelector('script[data-fx-flow-first-r75]'))return;const s=document.createElement('script');s.src=FLOW_SCRIPT;s.async=false;s.dataset.fxFlowFirstR75='true';s.addEventListener('load',()=>{root.dataset.fxFlowFirstGuard='ready-r137';},{once:true});s.addEventListener('error',()=>{root.dataset.fxFlowFirstGuard='failed-r137';},{once:true});document.head.appendChild(s);}
  addMobileStyle();addReferenceLayout();addMobileScript();addInteractionScript();setTimeout(()=>{addExactStyle();addDetailScript();addFlowGuard();addReferenceCopy();},0);
}());
