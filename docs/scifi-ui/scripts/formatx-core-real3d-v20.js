(function () {
  'use strict';
  // r139 bootstrap: clean reconstructed reference + final heading geometry + exact proof typography + native WebGL.
  const root = document.documentElement;
  const BOOTSTRAP = 'responsive-cinematic-reference-v69-r99-luminous-interactive-r139';
  ...
  function addFlowGuard(){if(document.querySelector('script[data-fx-flow-first-r75]'))return;const s=document.createElement('script');s.src=FLOW_SCRIPT;s.async=false;s.dataset.fxFlowFirstR75='true';s.addEventListener('load',()=>{root.dataset.fxFlowFirstGuard='ready-r139';},{once:true});s.addEventListener('error',()=>{root.dataset.fxFlowFirstGuard='failed-r139';},{once:true});document.head.appendChild(s);}
