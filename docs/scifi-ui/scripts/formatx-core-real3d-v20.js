(function () {
  ...
  const MOBILE_SCRIPT = '/scifi-ui/scripts/formatx-core-mobile-v55.js?v=20260813-cinematic-bloom-v64-r2';
  ...
  root.dataset.fxCoreRenderer = mobile ? 'single-webgl2-mobile-cinematic-bloom-reference-v64' : 'single-webgl2-reference-crystal-v53';
  root.dataset.fxCoreReferenceLock = mobile ? 'loading-v64' : 'loading-v53';
  ...
  function addMobileScript(){if(document.querySelector('script[data-fx-core-mobile-v55-script], script[src*="formatx-core-mobile-v55.js"]'))return;const s=document.createElement('script');s.src=MOBILE_SCRIPT;s.async=false;s.dataset.fxCoreMobileV55Script='true';s.addEventListener('load',()=>{root.dataset.fxCoreReferenceLockLoad='ready-v64';},{once:true});s.addEventListener('error',()=>{root.dataset.fxCoreReal3d='context-unavailable-v64';root.dataset.fxCoreReferenceLock='load-failed-v64';root.dataset.fxCoreReferenceLockLoad='failed-v64';dispatchEvent(new CustomEvent('formatx:core3dfallback',{detail:{reason:'mobile-cinematic-bloom-reference-load-failed',reference:'v64'}}));},{once:true});document.head.appendChild(s);}
