(function () {
  'use strict';
  // r88 visual-material shipment under the stable r83 runtime contract.
  ...
  if (root.dataset.fxCoreReal3dBootstrap === BOOTSTRAP && root.dataset.fxCoreMaterialR88 === 'ready') return;
  ...
  function addMobileStyle(){
    ...
    addStyle(MATERIAL_STYLE,'data-fx-award-material-r88','fxCoreMaterialR88');
  ...
  function addFlowGuard(){if(document.querySelector('script[data-fx-flow-first-r75]'))return;const s=document.createElement('script');s.src=FLOW_SCRIPT;s.async=false;s.dataset.fxFlowFirstR75='true';s.addEventListener('load',()=>{root.dataset.fxFlowFirstGuard='ready-r88';},{once:true});s.addEventListener('error',()=>{root.dataset.fxFlowFirstGuard='failed-r88';},{once:true});document.head.appendChild(s);}
