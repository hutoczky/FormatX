(function () {
  'use strict';
  // r90 reference-clarity shipment; preserve r87 geometry and refine material/flare only.
  ...
  if (root.dataset.fxCoreReal3dBootstrap === BOOTSTRAP && root.dataset.fxCoreMaterialR90 === 'ready') return;
  ...
  function addMobileStyle(){
    ...
    addStyle(CLARITY_STYLE,'data-fx-award-material-r90','fxCoreMaterialR90');
  ...
  function addFlowGuard(){if(document.querySelector('script[data-fx-flow-first-r75]'))return;const s=document.createElement('script');s.src=FLOW_SCRIPT;s.async=false;s.dataset.fxFlowFirstR75='true';s.addEventListener('load',()=>{root.dataset.fxFlowFirstGuard='ready-r90';},{once:true});s.addEventListener('error',()=>{root.dataset.fxFlowFirstGuard='failed-r90';},{once:true});document.head.appendChild(s);}
