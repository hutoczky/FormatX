(function () {
  'use strict';
  // r92 faceted ray-glass shipment; preserve r87 size and strengthen icy refractive detail.
  const root = document.documentElement;
  const BOOTSTRAP = 'responsive-cinematic-reference-v69-r92-rayglass';
  const MOBILE_SCRIPT = '/scifi-ui/scripts/formatx-core-mobile-v55.js?v=20260814-rayglass-r92';
  ...
  const RAYGLASS_STYLE = '/scifi-ui/styles/formatx-award-material-r91.css?v=20260814-rayglass-r92';
  ...
  if (root.dataset.fxCoreReal3dBootstrap === BOOTSTRAP && root.dataset.fxCoreMaterialR92 === 'ready') return;
  ...
  root.dataset.fxCoreRenderer = 'single-webgl-refractive-rayglass-r92';
  ...
  function addMobileStyle(){
    ...
    addStyle(RAYGLASS_STYLE,'data-fx-award-material-r91','fxCoreMaterialR92');
  ...
  function addFlowGuard(){if(document.querySelector('script[data-fx-flow-first-r75]'))return;const s=document.createElement('script');s.src=FLOW_SCRIPT;s.async=false;s.dataset.fxFlowFirstR75='true';s.addEventListener('load',()=>{root.dataset.fxFlowFirstGuard='ready-r92';},{once:true});s.addEventListener('error',()=>{root.dataset.fxFlowFirstGuard='failed-r92';},{once:true});document.head.appendChild(s);}
