(function () {
  'use strict';
  // r125 bootstrap: native XYZ WebGL mesh + filled faceted crystal overlay matched to the supplied mobile reference.
  const root = document.documentElement;
  const BOOTSTRAP = 'responsive-cinematic-reference-v69-r99-luminous-interactive-r125';
  ...
  const DETAIL_STYLE = '/scifi-ui/styles/formatx-core-detail-overlay-r122.css?v=20260815-filled-faceted-glass-r125';
  const DETAIL_SCRIPT = '/scifi-ui/scripts/formatx-core-detail-overlay-r122.js?v=20260815-filled-faceted-glass-r125';
  ...
  function addFlowGuard(){if(document.querySelector('script[data-fx-flow-first-r75]'))return;const s=document.createElement('script');s.src=FLOW_SCRIPT;s.async=false;s.dataset.fxFlowFirstR75='true';s.addEventListener('load',()=>{root.dataset.fxFlowFirstGuard='ready-r125';},{once:true});s.addEventListener('error',()=>{root.dataset.fxFlowFirstGuard='failed-r125';},{once:true});document.head.appendChild(s);}
