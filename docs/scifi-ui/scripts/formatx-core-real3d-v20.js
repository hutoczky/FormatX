(function () {
  'use strict';
  // r114 bootstrap: actual 3D WebGL mesh + faceted volume and circular reactor.
  const root = document.documentElement;
  const BOOTSTRAP = 'responsive-cinematic-reference-v69-r99-luminous-interactive-r114';
  ...
  function addFlowGuard(){if(document.querySelector('script[data-fx-flow-first-r75]'))return;const s=document.createElement('script');s.src=FLOW_SCRIPT;s.async=false;s.dataset.fxFlowFirstR75='true';s.addEventListener('load',()=>{root.dataset.fxFlowFirstGuard='ready-r114';},{once:true});s.addEventListener('error',()=>{root.dataset.fxFlowFirstGuard='failed-r114';},{once:true});document.head.appendChild(s);}
