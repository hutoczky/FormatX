(function () {
  'use strict';
  // r133 bootstrap: r132 visual lock + cache-busted flow geometry guard.
  const root = document.documentElement;
  const BOOTSTRAP = 'responsive-cinematic-reference-v69-r99-luminous-interactive-r133';
  ...
  const FINAL_STYLE = '/scifi-ui/styles/formatx-reference-final-r132.css?v=20260815-pixel-lock-r132';
  ...
  const FLOW_SCRIPT = '/scifi-ui/scripts/formatx-flow-first-r75.js?v=20260815-reference-geometry-r132';
  ...
  function addFlowGuard(){if(document.querySelector('script[data-fx-flow-first-r75]'))return;const s=document.createElement('script');s.src=FLOW_SCRIPT;s.async=false;s.dataset.fxFlowFirstR75='true';s.addEventListener('load',()=>{root.dataset.fxFlowFirstGuard='ready-r132';},{once:true});s.addEventListener('error',()=>{root.dataset.fxFlowFirstGuard='failed-r132';},{once:true});document.head.appendChild(s);}
