(function () {
  'use strict';
  // r89 faceted-glass shipment; r87 keeps the approved size, r89 changes material only.
  ...
  if (root.dataset.fxCoreReal3dBootstrap === BOOTSTRAP && root.dataset.fxCoreMaterialR89 === 'ready') return;
  ...
  function addMobileStyle(){
    ...
    addStyle(FACET_STYLE,'data-fx-award-material-r89','fxCoreMaterialR89');
  ...
  function addFlowGuard(){if(document.querySelector('script[data-fx-flow-first-r75]'))return;const s=document.createElement('script');s.src=FLOW_SCRIPT;s.async=false;s.dataset.fxFlowFirstR75='true';s.addEventListener('load',()=>{root.dataset.fxFlowFirstGuard='ready-r89';},{once:true});s.addEventListener('error',()=>{root.dataset.fxFlowFirstGuard='failed-r89';},{once:true});document.head.appendChild(s);}
