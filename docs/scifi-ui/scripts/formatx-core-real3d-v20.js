(function () {
  'use strict';
  // r113 bootstrap: actual 3D WebGL mesh + vivid reference-matched mobile optics.
  const root = document.documentElement;
  const BOOTSTRAP = 'responsive-cinematic-reference-v69-r99-luminous-interactive-r113';
  const MOBILE_SCRIPT = '/scifi-ui/scripts/formatx-core-mobile-v55.js?v=20260814-luminous-cinematic-r99&rev=20260815-vivid-faceted-r113';
  ...
  const EXACT_STYLE = '/scifi-ui/styles/formatx-reference-exact-r112.css?v=20260815-vivid-faceted-r113';
  ...
  function addFlowGuard(){if(document.querySelector('script[data-fx-flow-first-r75]'))return;const s=document.createElement('script');s.src=FLOW_SCRIPT;s.async=false;s.dataset.fxFlowFirstR75='true';s.addEventListener('load',()=>{root.dataset.fxFlowFirstGuard='ready-r113';},{once:true});s.addEventListener('error',()=>{root.dataset.fxFlowFirstGuard='failed-r113';},{once:true});document.head.appendChild(s);}
