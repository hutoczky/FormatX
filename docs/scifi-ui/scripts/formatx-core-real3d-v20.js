(function () {
  'use strict';
  // r86 proof-lane shipment under the stable r83 runtime contract.
  ...
  function addFlowGuard(){if(document.querySelector('script[data-fx-flow-first-r75]'))return;const s=document.createElement('script');s.src=FLOW_SCRIPT;s.async=false;s.dataset.fxFlowFirstR75='true';s.addEventListener('load',()=>{root.dataset.fxFlowFirstGuard='ready-r86';},{once:true});s.addEventListener('error',()=>{root.dataset.fxFlowFirstGuard='failed-r86';},{once:true});document.head.appendChild(s);}
