(() => {
  ...
  class FormatXGenesisThree {
    ...
    updateCore(t,time){
      ...
      else if(t<3.30) sc=mix(.86,.64,smooth((t-2.70)/.60));
      else if(t<5.70) sc=.64;
      else if(t<6.55) sc=mix(.64,.56,smooth((t-5.70)/.85));
      else sc=.56;
      ...
      this.coreShell.material.opacity=(.92*stage1 + .62*(1-stage1))*coreFade;
      this.coreGlass.material.opacity=(.16*stage1 + .060*(1-stage1))*coreFade;
      this.coreEdges.material.opacity=(.18*stage1 + .075*(1-stage1))*coreFade;
      if(this.corePetalMaterial)this.corePetalMaterial.opacity=(.12*stage1+.16*(1-stage1))*coreFade;

      this.irisGroup.scale.setScalar((.018+eyeHold*.60)*pulse);
    ...
    updateOrganic(t,time){
      ...
      this.organicVeinMaterial.opacity=.86*visible;
      this.organicHoodMaterial.opacity=.28*visible;
      if(this.organicFoldMaterial)this.organicFoldMaterial.opacity=.86*visible;
      if(this.organicFoldGlowMaterial)this.organicFoldGlowMaterial.opacity=.34*visible;
    ...
    updateCells(t,time){
      ...
      this.cellVeinMaterial.opacity=.32*visible;
  ...
  async function attach(canvas,getTarget){
    ...
    try{
      ...
      if(!visualProof && isSoftwareWebGL()){
        document.documentElement.dataset.fxMagBirthR1130='software-webgl-r649-fallback';
        return null;
      }
      loader ||= loadThree();
      ...
      return {
        ...
        engine,
        revision:'r1130-reference-proportion-lock'
      };
    }catch(error){
      console.error('FormatX R1130 genesis renderer failed:',error);
      document.documentElement.dataset.fxMagBirthR1130='fallback-r649';
      return null;
    }
  }
  ...
  window.FormatXMagGenesisThreeR1130={
    attach,
    revision:'r1130-reference-proportion-lock-dna-cellular-living-architecture'
  };
})();