(() => {
  ...
  class FormatXGenesisThree {
    ...
    makeTentacles(){
      ...
      for(let i=0;i<count;i++){
        ...
        const curve=new T.CatmullRomCurve3(pts,false,'centripetal');
        const geo=this.createTaperedTube(curve,60,8,.064+r()*.016,.008+r()*.003);
        const glowGeo=this.createTaperedTube(curve,60,6,.012+r()*.003,.003+r()*.001);
    ...
    updateCore(t,time){
      ...
      this.irisGroup.scale.setScalar((.05+eyeHold*.90)*pulse);
      ...
      this.coreInner.material.opacity=.006+eyeHold*.24;
      this.coreLight.intensity=eyeHold*(3.1+Math.sin(time*.005)*.32);
    ...
    updateOrganic(t,time){
      ...
      this.organicLobeMaterial.opacity=.46*visible;
      this.organicWireMaterial.opacity=.003*visible;
      this.organicVeinMaterial.opacity=.62*visible;
      this.organicHoodMaterial.opacity=.15*visible;
      if(this.organicFoldMaterial)this.organicFoldMaterial.opacity=.82*visible;
    ...
    updateTentacles(t,time){
      ...
      this.tentacleEdgeMaterial.opacity=.008*grow;
  ...
  async function attach(canvas,getTarget){
    ...
    try{
      ...
      if(!visualProof && isSoftwareWebGL()){
        document.documentElement.dataset.fxMagBirthR675='software-webgl-r649-fallback';
        return null;
      }
      loader ||= loadThree();
      ...
      return {
        ...
        engine,
        revision:'r675-neural-fold-three-genesis'
      };
    }catch(error){
      console.error('FormatX R675 genesis renderer failed:',error);
      document.documentElement.dataset.fxMagBirthR675='fallback-r649';
      return null;
    }
  }
  ...
  window.FormatXMagGenesisThreeR675={
    attach,
    revision:'r675-neural-fold-three-genesis-dna-cellular-living-architecture'
  };
})();