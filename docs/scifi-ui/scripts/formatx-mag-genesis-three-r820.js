(() => {
  ...
  class FormatXGenesisThree {
    ...
    updateCore(t,time){
      ...
      this.coreLight.intensity=eyeHold*(2.7+Math.sin(time*.0048)*.22);
    ...
    updateTentacles(t,time){
      ...
      this.tentacleNodeMaterial.opacity=.38*grow;
      if(this.tentacleGlowMaterial)this.tentacleGlowMaterial.opacity=.78*grow;
  ...
  async function attach(canvas,getTarget){
    ...
    try{
      ...
      if(!visualProof && isSoftwareWebGL()){
        document.documentElement.dataset.fxMagBirthR820='software-webgl-r649-fallback';
        return null;
      }
      loader ||= loadThree();
      ...
      return {
        ...
        engine,
        revision:'r820-material-locked-three-genesis'
      };
    }catch(error){
      console.error('FormatX R820 genesis renderer failed:',error);
      document.documentElement.dataset.fxMagBirthR820='fallback-r649';
      return null;
    }
  }
  ...
  window.FormatXMagGenesisThreeR820={
    attach,
    revision:'r820-material-locked-three-genesis-dna-cellular-living-architecture'
  };
})();