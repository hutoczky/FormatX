(() => {
  ...
  class FormatXGenesisThree {
    ...
    createHelix(length=4.7,radius=.28,turns=4.1){
      ...
      const rungMat=new T.MeshPhysicalMaterial({
        ...
        roughness:.42,transparent:true,opacity:.54,depthWrite:false
      });
      const mpa=new T.PointsMaterial({
        color:0xb7f3ff,size:.028,transparent:true,opacity:.55,
        depthWrite:false,blending:T.AdditiveBlending,sizeAttenuation:true
      });
      const mpb=new T.PointsMaterial({
        color:0xa994ff,size:.026,transparent:true,opacity:.50,
        depthWrite:false,blending:T.AdditiveBlending,sizeAttenuation:true
      });

      const tubeA=new T.Mesh(new T.TubeGeometry(curveA,seg,.021,7,false),ma);
      const tubeB=new T.Mesh(new T.TubeGeometry(curveB,seg,.021,7,false),mb);
      const auraA=new T.Mesh(new T.TubeGeometry(curveA,seg,.047,6,false),ga);
      const auraB=new T.Mesh(new T.TubeGeometry(curveB,seg,.047,6,false),gb);

      const rungGeo=new T.CylinderGeometry(.010,.010,1,6,1,false);
      ...
      group.userData.baseOpacity=[.72,.66,.050,.045,.54,.55,.50];
      return group;
    }
    ...
    makeDNAField(){
      ...
      for(const [x,y,z,rz,sc] of placements){
        const h=this.createHelix(3.75,.235,3.70);
    ...
    makeOrganicLayer(){
      ...
      this.organicLobeMaterial=new T.MeshPhysicalMaterial({
        color:0x170d23,roughness:.72,metalness:.02,
        clearcoat:.14,clearcoatRoughness:.50,
        transparent:true,opacity:0,
        emissive:0x1b0b2b,emissiveIntensity:.18,
        depthWrite:true
      });
      ...
      this.organicLobes=[];
      const lobeGeo=new T.IcosahedronGeometry(.142,3);
      const count=112;
      for(let i=0;i<count;i++){
        const phi=Math.acos(1-2*(i+.5)/count);
        const theta=Math.PI*(1+Math.sqrt(5))*i;
        const rr=1.31+(r()-.5)*.12;
    ...
    makeCellularLayer(){
      ...
      const blobGeo=new T.IcosahedronGeometry(.118,2);
      const edgeGeo=new T.IcosahedronGeometry(.121,1);
      this.cells=[];
      const count=128;
      for(let i=0;i<count;i++){
        const phi=Math.acos(1-2*(i+.5)/count);
    ...
    makeTentacles(){
      ...
      this.tentacleMaterial=new T.MeshStandardMaterial({
        color:0x061720,metalness:.44,roughness:.31,
        emissive:0x0b6b7d,emissiveIntensity:.52,
        transparent:true,opacity:0
      });
      ...
      for(let i=0;i<count;i++){
        ...
        const geo=this.createTaperedTube(curve,48,8,.058+r()*.016,.010+r()*.004);
    ...
    updateCore(t,time){
      let sc=.001;
      if(t<2.55)sc=.36+ease(t/2.55)*.05;
      else if(t<3.35)sc=mix(.41,.39,smooth((t-2.55)/.80));
      else if(t<5.65)sc=.39+Math.sin((t-3.35)*1.02)*.002;
      else if(t<8.95)sc=mix(.39,.31,smooth((t-5.65)/3.30));
      else sc=.30;
      ...
      const surfaceZ=cellSurface*1.22;
      ...
      this.coreGroup.rotation.y=Math.sin(time*.00020)*.012;
      this.coreGroup.rotation.x=Math.sin(time*.00024)*.010;

      const early=smooth((t-.20)/.68);
      const eye=smooth((t-2.66)/.46);
      const eyeHold=eye*(1-smooth((t-9.46)/.28));
      const pulse=.97+.03*Math.sin(time*.0050);

      this.coreShell.material.opacity=early*(.44+.24*eyeHold);
      this.coreGlass.material.opacity=early*(.045+.085*eyeHold);
      this.coreEdges.material.opacity=early*(.075+.15*eyeHold);

      this.irisGroup.scale.setScalar((.06+eyeHold*.78)*pulse);
      this.irisRays.rotation.z=time*.000080;
      this.glowSprite.material.opacity=(.002+eyeHold*.31)*pulse;
      this.glowSprite.scale.setScalar(1.12+eyeHold*.30);
      this.coreInner.material.opacity=.008+eyeHold*.50;
      this.coreLight.intensity=eyeHold*(4.5+Math.sin(time*.005)*.55);

      if(this.coreLabel)this.coreLabel.material.opacity=.24*early*(1-smooth((t-2.64)/.32));
    }
    ...
    updateOrganic(t,time){
      ...
      this.organicShellMaterial.opacity=.68*visible;
      this.organicLobeMaterial.opacity=.56*visible;
    ...
    updateCells(t,time){
      ...
      this.cellMaterial.opacity=.42*visible;
    ...
    render(r,time){
      ...
      if(this.glowSprite){
        const g=1+flash*4.20;
        this.glowSprite.scale.multiplyScalar(g);
        this.glowSprite.material.opacity=Math.min(1,this.glowSprite.material.opacity+flash*.72);
      }
      if(this.flashBeam){
        this.flashBeam.material.opacity=flash*.62;
        this.flashBeam.scale.x=1+flash*.34;
      }
  ...
  async function attach(canvas,getTarget){
    ...
    try{
      ...
      if(!visualProof && isSoftwareWebGL()){
        document.documentElement.dataset.fxMagBirthR673='software-webgl-r649-fallback';
        return null;
      }
      loader ||= loadThree();
      ...
      return {
        ...
        engine,
        revision:'r673-reference-material-three-genesis'
      };
    }catch(error){
      console.error('FormatX R673 genesis renderer failed:',error);
      document.documentElement.dataset.fxMagBirthR673='fallback-r649';
      return null;
    }
  }
  ...
  window.FormatXMagGenesisThreeR673={
    attach,
    revision:'r673-reference-material-three-genesis-dna-cellular-living-architecture'
  };
})();