(() => {
  ...
  class FormatXGenesisThree {
    ...
    makeOrganicLayer(){
      ...
      this.organicFoldMaterial=new T.MeshPhysicalMaterial({
        color:0x34263b,roughness:.66,metalness:.004,
        emissive:0x140b1b,emissiveIntensity:.045,
        transparent:true,opacity:0,depthWrite:true
      });
      this.organicFoldGlowMaterial=new T.MeshBasicMaterial({
        color:0x38646c,transparent:true,opacity:0,
        depthWrite:false,blending:T.AdditiveBlending
      });
      this.organicFolds=new T.Group();

      const frontRadius=1.36;
      for(let lane=-8;lane<=8;lane++){
        ...
        for(const offset of [-.055,.055]){
          ...
          for(let j=0;j<13;j++){
            ...
            const x=(u-.5)*2.18;
            const y=y0+offset
              +Math.sin(u*Math.PI*3.2+phase)*.055
              +Math.sin(u*Math.PI*6.1+phase*.7)*.018;
            const rr2=x*x+y*y;
            if(rr2>frontRadius*frontRadius*.95)continue;
            const z=Math.sqrt(Math.max(.02,frontRadius*frontRadius-rr2))*.76+.06;
            pts.push(new T.Vector3(x,y,z));
          }
          if(pts.length<4)continue;
          const curve=new T.CatmullRomCurve3(pts,false,'centripetal');
          const fold=new T.Mesh(
            new T.TubeGeometry(curve,42,.031+r()*.006,7,false),
            this.organicFoldMaterial
          );
          const glow=new T.Mesh(
            new T.TubeGeometry(curve,42,.0038,5,false),
      ...
      for(let lane=-5;lane<=5;lane++){
        ...
        for(let j=0;j<12;j++){
          ...
          const y=(u-.5)*2.08;
          const x=x0
            +Math.sin(u*Math.PI*2.7+phase)*.045
            +Math.sin(u*Math.PI*5.2+phase*.6)*.016;
          const rr2=x*x+y*y;
          if(rr2>frontRadius*frontRadius*.93)continue;
          const z=Math.sqrt(Math.max(.02,frontRadius*frontRadius-rr2))*.765+.072;
          pts.push(new T.Vector3(x,y,z));
        }
        if(pts.length<4)continue;
        const curve=new T.CatmullRomCurve3(pts,false,'centripetal');
        const fold=new T.Mesh(
          new T.TubeGeometry(curve,38,.026+r()*.005,7,false),
          this.organicFoldMaterial
        );
        fold.userData.phase=phase;
        this.organicFolds.add(fold);
      }
      ...
      const hoodDefs=[
        [-.28,.48,1.05,.82,1.22,.76,-.18,.16],
        [.28,.48,1.05,.82,1.22,.76,.18,-.16],
        [0,.72,1.00,.46,.78,.66,0,0]
      ];
    ...
    updateCore(t,time){
      let sc=.001;
      if(t>=2.52 && t<3.28) sc=.05+ease((t-2.52)/.76)*.31;
      else if(t>=3.28 && t<5.58) sc=.36+Math.sin((t-3.28)*1.02)*.002;
      else if(t>=5.58 && t<7.15) sc=mix(.36,.31,smooth((t-5.58)/1.57));
      else if(t>=7.15) sc=.31;
      ...
      this.coreGroup.rotation.y=Math.sin(time*.00018)*.005;
      ...
      this.coreShell.material.opacity=.10*reveal*preFlash;
      this.coreGlass.material.opacity=.012*reveal*preFlash;
      this.coreEdges.material.opacity=.010*reveal*preFlash;
      if(this.corePetalMaterial)this.corePetalMaterial.opacity=.22*reveal*preFlash;

      this.irisGroup.scale.setScalar((.020+eyeHold*1.62)*pulse);
      this.irisRays.rotation.z=time*.000050;
      if(this.irisCorona)this.irisCorona.material.opacity=.96*eyeHold;
      this.glowSprite.material.opacity=(.001+eyeHold*.012)*pulse;
      this.glowSprite.scale.setScalar(.88+eyeHold*.08);
      this.coreInner.material.opacity=.002+eyeHold*.022;
      this.coreLight.intensity=eyeHold*(2.5+Math.sin(time*.0046)*.12);

      if(this.coreLabel)this.coreLabel.material.opacity=.04*reveal*(1-smooth((t-3.08)/.32));
    }
    ...
    updateOrganic(t,time){
      ...
      this.organicGroup.scale.set(bodyScale*1.04,bodyScale*1.01,bodyScale*.98);

      this.organicShellMaterial.opacity=.74*visible;
      this.organicLobeMaterial.opacity=.16*visible;
      this.organicWireMaterial.opacity=.0003*visible;
      this.organicVeinMaterial.opacity=.16*visible;
      this.organicHoodMaterial.opacity=.96*visible;
      if(this.organicFoldMaterial)this.organicFoldMaterial.opacity=.96*visible;
      if(this.organicFoldGlowMaterial)this.organicFoldGlowMaterial.opacity=.035*visible;
      ...
      if(this.organicPrimaryVeinMaterial)this.organicPrimaryVeinMaterial.opacity=.045*visible;
    ...
    updateCells(t,time){
      ...
      this.cellGroup.scale.set(cellScale*1.02,cellScale,cellScale*.98);
    ...
    updateCamera(t,time){
      ...
      if(t<2.70){
        ...
        z=mix(4.96,5.20,k);
        x=Math.sin(time*.00027)*.048*(1-k*.35);
        y=.010+Math.sin(time*.00024)*.017;
      }else if(t<3.30){
        const k=smooth((t-2.70)/.60);
        z=mix(5.20,4.12,k);
        x=mix(.020,0,k); y=mix(.013,0,k);
      }else if(t<5.55){
        z=4.12+Math.sin(time*.00020)*.008;
        x=Math.sin(time*.00015)*.004;
        ...
        z=mix(6.15,6.28,smooth((t-9.10)/.60));
        y=.004;
      }
  ...
  async function attach(canvas,getTarget){
    ...
    try{
      ...
      if(!visualProof && isSoftwareWebGL()){
        document.documentElement.dataset.fxMagBirthR1230='software-webgl-r649-fallback';
        return null;
      }
      loader ||= loadThree();
      ...
      return {
        ...
        engine,
        revision:'r1230-frame-ratio-brain-lock'
      };
    }catch(error){
      console.error('FormatX R1230 genesis renderer failed:',error);
      document.documentElement.dataset.fxMagBirthR1230='fallback-r649';
      return null;
    }
  }
  ...
  window.FormatXMagGenesisThreeR1230={
    attach,
    revision:'r1230-frame-ratio-brain-lock-dna-cellular-living-architecture'
  };
})();