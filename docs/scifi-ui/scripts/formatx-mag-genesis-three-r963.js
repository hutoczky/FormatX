(() => {
  ...
  class FormatXGenesisThree {
    ...
    makeCore(){
      ...
      const shellMat=new T.MeshPhysicalMaterial({
        color:0x14242d,metalness:.54,roughness:.28,
        emissive:0x07151c,emissiveIntensity:.11,
        clearcoat:.72,clearcoatRoughness:.18,
        transparent:true,opacity:.97
      });
      const shellGlass=new T.MeshPhysicalMaterial({
        color:0x23424b,metalness:.12,roughness:.18,
        transparent:true,opacity:.11,depthWrite:false,
        emissive:0x0a2f39,emissiveIntensity:.16,
        clearcoat:.74,clearcoatRoughness:.16
      });
      ...
      this.coreShell.scale.set(.82,1.08,1.12);
      ...
      this.coreContourMaterial=new T.MeshBasicMaterial({
        color:0x80c8d8,transparent:true,opacity:.060,
        depthWrite:false,blending:T.AdditiveBlending
      });
      this.coreContours=[];
      for(const cfg of [
      ...
      this.coreEdges=new T.LineSegments(
        ...
        new T.LineBasicMaterial({
          color:0x8fd8e4,transparent:true,opacity:.20,
          depthWrite:false,blending:T.AdditiveBlending
        })
      );
    ...
    makeOrganicLayer(){
      ...
      for(let i=0;i<58;i++){
        ...
        const fold=new T.Mesh(new T.TubeGeometry(curve,42,.041+r()*.012,7,false),this.organicFoldMaterial);
        const glow=new T.Mesh(new T.TubeGeometry(curve,42,.0075,5,false),this.organicFoldGlowMaterial);
    ...
    makeMechanicalLayer(){
      ...
      this.silverMaterial=new T.MeshPhysicalMaterial({
        color:0xaab4b8,metalness:.95,roughness:.16,
        emissive:0x10191d,emissiveIntensity:.06,
    ...
    makeTentacles(){
      ...
      for(let i=0;i<count;i++){
        ...
        const curve=new T.CatmullRomCurve3(pts,false,'centripetal');
        const geo=this.createTaperedTube(curve,62,8,.070+r()*.013,.008+r()*.0025);
        const glowGeo=this.createTaperedTube(curve,62,6,.009+r()*.002,.0025+r()*.0008);
    ...
    updateCore(t,time){
      let sc;
      if(t<2.70) sc=.52+ease(t/2.70)*.10;
      else if(t<3.25) sc=mix(.62,.70,smooth((t-2.70)/.55));
      else if(t<6.10) sc=.70;
      else sc=mix(.70,.60,smooth((t-6.10)/.80));

      const endMove=smooth((t-9.76)/.22);
      const target=this.targetWorld();
      const tx=target.x*endMove,ty=target.y*endMove;
      const cellular=smooth((t-2.72)/.48);
      const surfaceZ=cellular*.72;
      ...
      this.tentacleGroup.position.set(tx,ty,-.08);
      ...
      const mech=smooth((t-6.10)/.72);
      const pulse=.982+.018*Math.sin(time*.0046);

      // The dark diamond remains visible through all three stages.
      this.coreShell.material.opacity=.97*stage1 + .58*(1-stage1);
      this.coreGlass.material.opacity=.10*stage1 + .055*(1-stage1);
      this.coreEdges.material.opacity=.095*stage1 + .060*(1-stage1);
      if(this.corePetalMaterial)this.corePetalMaterial.opacity=.045*stage1+.16*(1-stage1);

      // Stage 2 reference eye occupies roughly one quarter of the cellular orb.
      // It stays alive during stage 3 under the silver crown.
      const irisScale=(.02+eyeHold*1.02+mech*.20)*pulse;
      this.irisGroup.scale.setScalar(irisScale);
      this.irisRays.rotation.z=time*.000060;
      if(this.irisCorona)this.irisCorona.material.opacity=.94*eyeHold+.26*mech;
      this.glowSprite.material.opacity=(.001+eyeHold*.055+mech*.024)*pulse;
      this.glowSprite.scale.setScalar(1.00+eyeHold*.22+mech*.08);
      this.coreInner.material.opacity=.002+eyeHold*.085+mech*.035;
      this.coreLight.intensity=eyeHold*(4.6+Math.sin(time*.0046)*.28)+mech*1.4;

      if(this.coreLabel)this.coreLabel.material.opacity=.18*stage1;
      if(this.coreContours){
        this.coreContours.forEach((m,i)=>{
          m.material.opacity=(.060-i*.018)*stage1 + .020*(1-stage1);
    ...
    updateOrganic(t,time){
      ...
      const flashFade=1-smooth((t-9.52)/.30)*.18;
      const visible=grow*flashFade;
      this.organicGroup.visible=visible>.002;
      const bodyScale=.001+visible*.999;
      const pullback=smooth((t-6.10)/1.10);
      this.organicGroup.scale.set(
        bodyScale*mix(1.12,.92,pullback),
        bodyScale*mix(1.08,.94,pullback),
        bodyScale*1.02
      );

      this.organicShellMaterial.opacity=.60*visible;
      this.organicLobeMaterial.opacity=.82*visible;
      this.organicWireMaterial.opacity=.001*visible;
      this.organicVeinMaterial.opacity=.96*visible;
      // Dark membrane only: no large cyan blade silhouette.
    ...
    updateCells(t,time){
      ...
      const fade=1-smooth((t-6.45)/1.55)*.72;
      const flashFade=1-smooth((t-9.48)/.34)*.22;
      const visible=grow*fade*flashFade;
      this.cellGroup.visible=visible>.002;
      const cellScale=.001+visible*.99;
      this.cellGroup.scale.set(cellScale*1.07,cellScale*1.04,cellScale*1.01);
      this.cellMaterial.opacity=.11*visible;
    ...
    updateMechanical(t,time){
      const grow=smooth((t-6.05)/.86);
      this.mechanicalGroup.visible=grow>.002;
      this.mechanicalGroup.scale.setScalar(.001+grow*.82);

      // Keep the black structure recessed inside the living shell.
      this.mechMaterial.opacity=.34*grow;
      this.mechMidMaterial.opacity=.42*grow;
      this.silverMaterial.opacity=.96*grow;
      this.mechEdgeMaterial.opacity=.050*grow;
      this.mechInnerMaterial.opacity=.32*grow;
      if(this.seamMaterial)this.seamMaterial.opacity=.50*grow;
      if(this.mechEyeCorona)this.mechEyeCorona.material.opacity=.78*grow;
      this.mechInnerRing.rotation.z=time*.00012;
      if(this.mechLight)this.mechLight.intensity=5.8*grow;
    ...
    updateTentacles(t,time){
      const grow=smooth((t-6.05)/1.05);
      const flashFade=1-smooth((t-9.58)/.28)*.08;
      this.tentacleGroup.visible=grow>.002;
      this.tentacleMaterial.opacity=.88*grow*flashFade;
      this.tentacleEdgeMaterial.opacity=.002*grow;
      this.tentacleNodeMaterial.opacity=.52*grow;
      if(this.tentacleGlowMaterial)this.tentacleGlowMaterial.opacity=.24*grow;
      this.tentacles.forEach((g,i)=>{
        const wave=1+Math.sin(time*.00070+g.userData.phase)*.006*grow;
    ...
    updateCamera(t,time){
      let z=5.05,y=.015,x=0;
      if(t<2.70){
        const k=smooth(t/2.70);
        z=mix(5.35,5.05,k);
        x=Math.sin(time*.00027)*.050*(1-k*.35);
        y=.012+Math.sin(time*.00024)*.018;
      }else if(t<3.28){
        const k=smooth((t-2.70)/.58);
        z=mix(5.05,4.10,k);
        x=mix(.022,0,k); y=mix(.016,0,k);
      }else if(t<6.10){
        z=4.10+Math.sin(time*.00020)*.009;
        x=Math.sin(time*.00015)*.006;
        y=Math.cos(time*.00018)*.005;
      }else if(t<7.35){
        const k=smooth((t-6.10)/1.25);
        z=mix(4.10,5.58,k);
      }else{
        z=5.58+Math.sin(time*.00018)*.008;
        y=.004;
      }
    ...
    render(r,time){
      ...
      const flash=smooth((t-9.10)/.10)*(1-smooth((t-9.58)/.24));
      const after=smooth((t-9.50)/.30);
      this.renderer.toneMappingExposure=1.13+flash*.22+after*.020;
      this.coreLight.intensity+=flash*14+after*2;
      if(this.glowSprite){
      ...
      if(this.flashBeam){
        this.flashBeam.material.opacity=flash*.68;
  ...
  async function attach(canvas,getTarget){
    ...
    try{
      ...
      if(!visualProof && isSoftwareWebGL()){
        document.documentElement.dataset.fxMagBirthR963='software-webgl-r649-fallback';
        return null;
      }
      loader ||= loadThree();
      ...
      return {
        ...
        engine,
        revision:'r963-organic-final-reference-three-genesis'
      };
    }catch(error){
      console.error('FormatX R963 genesis renderer failed:',error);
      document.documentElement.dataset.fxMagBirthR963='fallback-r649';
      return null;
    }
  }
  ...
  window.FormatXMagGenesisThreeR963={
    attach,
    revision:'r963-organic-final-reference-three-genesis-dna-cellular-living-architecture'
  };
})();