(() => {
  ...
  class FormatXGenesisThree {
    ...
    makeCore(){
      ...
      const shellMat=new T.MeshPhysicalMaterial({
        color:0x536d79,metalness:.80,roughness:.19,
        emissive:0x0d2935,emissiveIntensity:.16,
        clearcoat:.88,clearcoatRoughness:.10,
        transparent:true,opacity:.92
      });
      const shellGlass=new T.MeshPhysicalMaterial({
        color:0x5d7b88,metalness:.22,roughness:.15,
      ...
      shape.moveTo(.02,1.12);
      shape.bezierCurveTo(.16,.88,.58,.50,.88,.12);
      shape.bezierCurveTo(.98,-.02,.72,-.38,.42,-.66);
      shape.bezierCurveTo(.20,-.88,.08,-1.02,-.02,-1.08);
      shape.bezierCurveTo(-.16,-1.00,-.36,-.82,-.52,-.62);
      shape.bezierCurveTo(-.80,-.30,-.98,.02,-.88,.16);
      shape.bezierCurveTo(-.58,.56,-.18,.92,.02,1.12);
      ...
      const diamondMat=new T.MeshBasicMaterial({
        color:0x07131c,transparent:true,opacity:.76,
        side:T.DoubleSide,depthWrite:false
      });
      const diamond=new T.Mesh(new T.PlaneGeometry(1.02,1.02),diamondMat);
      diamond.rotation.z=Math.PI/4;
      diamond.position.z=-.02;
      this.irisGroup.add(diamond);

      const diamondEdge=new T.LineSegments(
        new T.EdgesGeometry(new T.PlaneGeometry(1.06,1.06)),
        new T.LineBasicMaterial({
          color:0x57c9dc,transparent:true,opacity:.20,
          depthWrite:false,blending:T.AdditiveBlending
        })
      );
      ...
      this.irisCorona.scale.set(1.08,1.08,1);
    ...
    makeOrganicLayer(){
      ...
      this.organicVeins=[];
      for(let i=0;i<52;i++){
        const a=i/52*Math.PI*2+(r()-.5)*.11;
        const e=.52+r()*.70;
        const ex=Math.cos(a)*e;
        const ey=Math.sin(a)*e;
        const ez=Math.sqrt(Math.max(.04,1.36*1.36-e*e))*.56+.20;
        const start=new T.Vector3(0,0,1.01);
        const side=(r()-.5)*.30;
        const bendA=a+side;
        const bendE=e*(.48+r()*.14);
        const bend=new T.Vector3(
          Math.cos(bendA)*bendE,
          Math.sin(bendA)*bendE,
          .88+(r()-.5)*.10
        );
        const end=new T.Vector3(ex,ey,ez);
        const curve=new T.QuadraticBezierCurve3(start,bend,end);
        const vein=new T.Mesh(
          new T.TubeGeometry(curve,30,.012+r()*.004,6,false),
    ...
    makeMechanicalLayer(){
      ...
      crownL.scale.set(.54,.88,.84);
      crownL.position.set(-.22,.34,.34);
      ...
      crownR.scale.set(-.54,.88,.84);
      crownR.position.set(.22,.34,.34);
      ...
      this.mechInnerRing=new T.Mesh(
        new T.TorusGeometry(.315,.012,8,72),
    ...
    updateCore(t,time){
      ...
      else if(t<3.30) sc=mix(.86,.54,smooth((t-2.70)/.60));
      else if(t<5.70) sc=.54;
      else if(t<9.55) sc=mix(.54,.43,smooth((t-5.70)/3.85));
      else sc=.43;
      ...
      const cellular=smooth((t-2.58)/.50)*(1-smooth((t-9.62)/.24));
      const surfaceZ=stage1*.80+cellular*1.02;
      ...
      const eye=smooth((t-2.78)/.34);
      const eyeHold=eye*(1-smooth((t-9.56)/.24));
      const pulse=.982+.018*Math.sin(time*.0046);

      this.coreShell.material.opacity=.92*stage1 + .54*(1-stage1);
      this.coreGlass.material.opacity=.16*stage1 + .060*(1-stage1);
      this.coreEdges.material.opacity=.24*stage1 + .075*(1-stage1);
      if(this.corePetalMaterial)this.corePetalMaterial.opacity=.065*stage1+.035*(1-stage1);

      this.irisGroup.scale.setScalar((.010+eyeHold*.86)*pulse);
      this.irisRays.rotation.z=time*.000060;
      if(this.irisCorona)this.irisCorona.material.opacity=.88*eyeHold;
      this.glowSprite.material.opacity=(.001+eyeHold*.020)*pulse;
      this.glowSprite.scale.setScalar(1.00+eyeHold*.10);
      this.coreInner.material.opacity=.002+eyeHold*.036;
      this.coreLight.intensity=eyeHold*(3.0+Math.sin(time*.0046)*.20);

      if(this.coreLabel)this.coreLabel.material.opacity=.46*stage1;
      if(this.coreContours){
        this.coreContours.forEach((m,i)=>{
          m.material.opacity=(.18-i*.045)*stage1;
          m.rotation.z=(i?-.026:.018)+Math.sin(time*.00017+i)*.010;
        });
      }
    }
    ...
    updateOrganic(t,time){
      ...
      const fade=1-smooth((t-9.56)/.30)*.94;
      ...
      this.organicLobeMaterial.opacity=.74*visible;
      ...
      if(this.organicFoldMaterial)this.organicFoldMaterial.opacity=.58*visible;
      if(this.organicFoldGlowMaterial)this.organicFoldGlowMaterial.opacity=.28*visible;
    ...
    updateCells(t,time){
      ...
      const fade=1-smooth((t-9.52)/.34)*.94;
      const visible=grow*fade;
    ...
    updateMechanical(t,time){
      ...
      this.silverMaterial.opacity=.94*grow;
      ...
      if(this.mechLight)this.mechLight.intensity=9.0*grow;
    ...
    updateTentacles(t,time){
      ...
      if(this.tentacleDashMaterial)this.tentacleDashMaterial.opacity=.86*grow;
    ...
    updateCamera(t,time){
      ...
      if(t<2.70){
        ...
        const k=smooth((t-5.55)/2.00);
        z=mix(3.94,4.82,k);
      }else{
        z=4.82+Math.sin(time*.00017)*.009;
        y=.004;
      }
    ...
    render(r,time){
      ...
      this.renderer.toneMappingExposure=1.13+flash*.12+after*.015;
      this.coreLight.intensity+=flash*7+after*2;
      if(this.glowSprite){
  ...
  async function attach(canvas,getTarget){
    ...
    try{
      ...
      if(!visualProof && isSoftwareWebGL()){
        document.documentElement.dataset.fxMagBirthR1000='software-webgl-r649-fallback';
        return null;
      }
      loader ||= loadThree();
      ...
      return {
        ...
        engine,
        revision:'r1000-reference-composition-three-genesis'
      };
    }catch(error){
      console.error('FormatX R1000 genesis renderer failed:',error);
      document.documentElement.dataset.fxMagBirthR1000='fallback-r649';
      return null;
    }
  }
  ...
  window.FormatXMagGenesisThreeR1000={
    attach,
    revision:'r1000-reference-composition-three-genesis-dna-cellular-living-architecture'
  };
})();