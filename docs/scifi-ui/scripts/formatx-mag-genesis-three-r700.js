(() => {
  ...
  class FormatXGenesisThree {
    ...
    makeTentacles(){
      ...
      for(let i=0;i<count;i++){
        ...
        const curve=new T.CatmullRomCurve3(pts,false,'centripetal');
        const geo=this.createTaperedTube(curve,62,8,.066+r()*.014,.008+r()*.003);
        const glowGeo=this.createTaperedTube(curve,62,6,.011+r()*.0025,.003+r()*.001);
    ...
    updateCore(t,time){
      ...
      this.glowSprite.material.opacity=(.002+eyeHold*.16)*pulse;
      this.glowSprite.scale.setScalar(1.10+eyeHold*.30);
      this.coreInner.material.opacity=.006+eyeHold*.24;
      this.coreLight.intensity=eyeHold*(4.0+Math.sin(time*.005)*.36);
    ...
    updateOrganic(t,time){
      ...
      this.organicHoodMaterial.opacity=.78*visible;
  ...
  async function attach(canvas,getTarget){
    ...
    try{
      ...
      if(!visualProof && isSoftwareWebGL()){
        document.documentElement.dataset.fxMagBirthR700='software-webgl-r649-fallback';
        return null;
      }
      loader ||= loadThree();
      ...
      return {
        ...
        engine,
        revision:'r700-reference-fused-three-genesis'
      };
    }catch(error){
      console.error('FormatX R700 genesis renderer failed:',error);
      document.documentElement.dataset.fxMagBirthR700='fallback-r649';
      return null;
    }
  }
  ...
  window.FormatXMagGenesisThreeR700={
    attach,
    revision:'r700-reference-fused-three-genesis-dna-cellular-living-architecture'
  };
})();