(() => {
  ...
  class FormatXGenesisThree {
    ...
    makeOrganicLayer(){
      ...
      this.organicFoldMaterial=new T.MeshPhysicalMaterial({
        color:0x332047,roughness:.70,metalness:.01,
        emissive:0x2a123d,emissiveIntensity:.30,
      ...
      for(let i=0;i<34;i++){
        ...
        const fold=new T.Mesh(new T.TubeGeometry(curve,34,.028,6,false),this.organicFoldMaterial);
        const glow=new T.Mesh(new T.TubeGeometry(curve,34,.0055,5,false),this.organicFoldGlowMaterial);
      ...
      this.organicHoodMaterial=new T.MeshPhysicalMaterial({
        color:0x242332,roughness:.58,metalness:.12,
        transparent:true,opacity:0,
        emissive:0x102733,emissiveIntensity:.10
      });
      this.organicHoodGroup=new T.Group();
      const hoodGeo=new T.IcosahedronGeometry(.19,2);
      const hoodDefs=[
        [-.28,.48,.88,.42,.42,.30,-.16],
        [-.10,.54,.90,.44,.46,.31,-.06],
        [.10,.54,.90,.44,.46,.31,.06],
        [.28,.48,.88,.42,.42,.30,.16]
      ];
    ...
    makeTentacles(){
      ...
      for(let i=0;i<count;i++){
        ...
        const curve=new T.CatmullRomCurve3(pts,false,'centripetal');
        const geo=this.createTaperedTube(curve,60,8,.062+r()*.014,.008+r()*.003);
        const glowGeo=this.createTaperedTube(curve,60,6,.011+r()*.0025,.003+r()*.001);
    ...
    updateCore(t,time){
      let sc=.001;
      if(t<2.55)sc=.39+ease(t/2.55)*.08;
      else if(t<3.35)sc=mix(.47,.58,smooth((t-2.55)/.80));
      else if(t<5.65)sc=.58+Math.sin((t-3.35)*1.02)*.002;
      else if(t<8.95)sc=mix(.58,.35,smooth((t-5.65)/3.30));
      else sc=.34;
      ...
      const surfaceZ=cellSurface*1.18;
      ...
      const pulse=.976+.024*Math.sin(time*.0050);

      this.coreShell.material.opacity=early*(.62+.10*eyeHold);
      this.coreGlass.material.opacity=early*(.050+.070*eyeHold);
      this.coreEdges.material.opacity=early*(.065+.095*eyeHold);

      this.irisGroup.scale.setScalar((.055+eyeHold*.90)*pulse);
      this.irisRays.rotation.z=time*.000075;
      this.glowSprite.material.opacity=(.003+eyeHold*.18)*pulse;
      this.glowSprite.scale.setScalar(1.06+eyeHold*.24);
      this.coreInner.material.opacity=.007+eyeHold*.24;
      this.coreLight.intensity=eyeHold*(3.2+Math.sin(time*.005)*.32);
    ...
    updateOrganic(t,time){
      ...
      this.organicShellMaterial.opacity=.52*visible;
      this.organicLobeMaterial.opacity=.64*visible;
      this.organicWireMaterial.opacity=.001*visible;
      this.organicVeinMaterial.opacity=.68*visible;
      this.organicHoodMaterial.opacity=.16*visible;
      if(this.organicFoldMaterial)this.organicFoldMaterial.opacity=.88*visible;
      if(this.organicFoldGlowMaterial)this.organicFoldGlowMaterial.opacity=.25*visible;
    ...
    updateTentacles(t,time){
      ...
      this.tentacleMaterial.opacity=.74*grow*flashFade;
      this.tentacleEdgeMaterial.opacity=.006*grow;
      this.tentacleNodeMaterial.opacity=.050*grow;
      if(this.tentacleGlowMaterial)this.tentacleGlowMaterial.opacity=.46*grow;
  ...
  async function attach(canvas,getTarget){
    ...
    try{
      ...
      if(!visualProof && isSoftwareWebGL()){
        document.documentElement.dataset.fxMagBirthR681='software-webgl-r649-fallback';
        return null;
      }
      loader ||= loadThree();
      ...
      return {
        ...
        engine,
        revision:'r681-reference-locked-three-genesis'
      };
    }catch(error){
      console.error('FormatX R681 genesis renderer failed:',error);
      document.documentElement.dataset.fxMagBirthR681='fallback-r649';
      return null;
    }
  }
  ...
  window.FormatXMagGenesisThreeR681={
    attach,
    revision:'r681-reference-locked-three-genesis-dna-cellular-living-architecture'
  };
})();