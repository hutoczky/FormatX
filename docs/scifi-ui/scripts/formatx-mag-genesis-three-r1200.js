(() => {
  ...
  class FormatXGenesisThree {
    ...
    updateCore(t,time){
      let sc=.001;
      if(t>=2.52 && t<3.28) sc=.08+ease((t-2.52)/.76)*.58;
      else if(t>=3.28 && t<5.58) sc=.66+Math.sin((t-3.28)*1.02)*.004;
      else if(t>=5.58 && t<7.15) sc=mix(.66,.50,smooth((t-5.58)/1.57));
      else if(t>=7.15) sc=.50;
      ...
      const reveal=smooth((t-2.58)/.48);
      const eyeHold=reveal*(1-smooth((t-9.52)/.30));
      const preFlash=1-smooth((t-9.48)/.26);
      const pulse=.982+.018*Math.sin(time*.0046);

      // Reference: a dark integrated diamond/hood around a much smaller blue iris.
      this.coreShell.material.opacity=.28*reveal*preFlash;
      this.coreGlass.material.opacity=.035*reveal*preFlash;
      this.coreEdges.material.opacity=.040*reveal*preFlash;
      if(this.corePetalMaterial)this.corePetalMaterial.opacity=.54*reveal*preFlash;

      this.irisGroup.scale.setScalar((.010+eyeHold*.56)*pulse);
      this.irisRays.rotation.z=time*.000060;
      if(this.irisCorona)this.irisCorona.material.opacity=.72*eyeHold;
      this.glowSprite.material.opacity=(.001+eyeHold*.025)*pulse;
      this.glowSprite.scale.setScalar(1.00+eyeHold*.12);
      this.coreInner.material.opacity=.002+eyeHold*.050;
      this.coreLight.intensity=eyeHold*(2.4+Math.sin(time*.0046)*.16);

      if(this.coreLabel)this.coreLabel.material.opacity=.18*reveal*(1-smooth((t-3.25)/.45));
    }

    updateOrganic(t,time){
      const grow=smooth((t-2.48)/.72);
      const fade=1-smooth((t-9.52)/.34)*.96;
      ...
      if(this.organicFoldGlowMaterial)this.organicFoldGlowMaterial.opacity=.44*visible;
    ...
    updateCells(t,time){
      ...
      const fade=1-smooth((t-9.50)/.36)*.96;
      const visible=grow*fade;
    ...
    updateMechanical(t,time){
      ...
      this.silverMaterial.opacity=.90*grow;
      ...
      if(this.mechLight)this.mechLight.intensity=8.6*grow;
    ...
    updateCamera(t,time){
      ...
      if(t<2.70){
        ...
        z=3.72+Math.sin(time*.00020)*.009;
        ...
        z=mix(4.62,4.72,smooth((t-9.10)/.60));
        y=.004;
      }
  ...
  async function attach(canvas,getTarget){
    ...
    try{
      ...
      if(!visualProof && isSoftwareWebGL()){
        document.documentElement.dataset.fxMagBirthR1200='software-webgl-r649-fallback';
        return null;
      }
      loader ||= loadThree();
      ...
      return {
        ...
        engine,
        revision:'r1200-video-timeline-lock'
      };
    }catch(error){
      console.error('FormatX R1200 genesis renderer failed:',error);
      document.documentElement.dataset.fxMagBirthR1200='fallback-r649';
      return null;
    }
  }
  ...
  window.FormatXMagGenesisThreeR1200={
    attach,
    revision:'r1200-video-timeline-lock-dna-cellular-living-architecture'
  };
})();