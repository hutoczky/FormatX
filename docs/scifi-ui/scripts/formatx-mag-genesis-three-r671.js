(() => {
  ...
  class FormatXGenesisThree {
    ...
    createHelix(length=4.7,radius=.28,turns=4.1){
      ...
      const mpa=new T.PointsMaterial({color:0xcafaff,size:.068,transparent:true,opacity:.72,depthWrite:false,blending:T.AdditiveBlending,sizeAttenuation:true});
      const mpb=new T.PointsMaterial({color:0xb9a0ff,size:.064,transparent:true,opacity:.64,depthWrite:false,blending:T.AdditiveBlending,sizeAttenuation:true});

      const tubeA=new T.Mesh(new T.TubeGeometry(curveA,seg,.032,7,false),ma);
      const tubeB=new T.Mesh(new T.TubeGeometry(curveB,seg,.032,7,false),mb);
      const auraA=new T.Mesh(new T.TubeGeometry(curveA,seg,.064,6,false),ga);
      const auraB=new T.Mesh(new T.TubeGeometry(curveB,seg,.064,6,false),gb);
    ...
    makeDNAField(){
      const placements=[
        [-2.55,1.38,.70,-.46,1.04],
        [2.44,1.44,.78,.42,1.00],
        [-2.52,-1.34,.62,.36,1.02],
        [2.48,-1.32,.66,-.38,1.00],
        [0,2.16,.15,1.56,.88],
        [0,-2.10,.20,1.56,.88],
        [-3.20,.04,-.30,1.50,.76],
        [3.18,.02,-.22,1.62,.76],
      ...
      for(const [x,y,z,rz,sc] of placements){
        const h=this.createHelix(4.35,.255,4.05);
        h.position.set(x,y,z);
        h.rotation.z=rz;
        h.rotation.y=(this.rand()-.5)*.34;
    ...
    makeOrganicLayer(){
      ...
      this.organicLobeMaterial=new T.MeshPhysicalMaterial({
        color:0x1a1028,roughness:.66,metalness:.03,
        clearcoat:.14,clearcoatRoughness:.50,
        transparent:true,opacity:0,
        emissive:0x241033,emissiveIntensity:.33,
        depthWrite:true
      });
      ...
      this.organicLobes=[];
      const lobeGeo=new T.IcosahedronGeometry(.185,3);
      const count=78;
      for(let i=0;i<count;i++){
        const phi=Math.acos(1-2*(i+.5)/count);
        const theta=Math.PI*(1+Math.sqrt(5))*i;
        const rr=1.30+(r()-.5)*.17;
    ...
    makeCellularLayer(){
      ...
      const blobGeo=new T.IcosahedronGeometry(.145,2);
      const edgeGeo=new T.IcosahedronGeometry(.148,1);
      this.cells=[];
      const count=92;
      for(let i=0;i<count;i++){
        const phi=Math.acos(1-2*(i+.5)/count);
        const theta=Math.PI*(1+Math.sqrt(5))*i;
        const rr=1.36+(r()-.5)*.16;
        const g=new T.Group();
    ...
    updateCore(t,time){
      let sc=.001;
      if(t<2.55)sc=.58+ease(t/2.55)*.12;
      else if(t<3.35)sc=mix(.70,.84,smooth((t-2.55)/.80));
      else if(t<5.65)sc=.84+Math.sin((t-3.35)*1.02)*.004;
      else if(t<8.95)sc=mix(.84,.58,smooth((t-5.65)/3.30));
      else sc=.58;

      const endMove=smooth((t-9.76)/.22);
      const target=this.targetWorld();
      const tx=target.x*endMove,ty=target.y*endMove;
      const cellSurface=smooth((t-2.65)/.55)*(1-smooth((t-9.48)/.36));
      ...
      this.coreGroup.rotation.y=Math.sin(time*.00020)*.014;
      this.coreGroup.rotation.x=Math.sin(time*.00024)*.011;

      const early=smooth((t-.12)/.58);
      const eye=smooth((t-2.72)/.50);
      const eyeHold=eye*(1-smooth((t-9.46)/.28));
      const pulse=.96+.04*Math.sin(time*.0050);

      // Early pod is dark and partially concealed by foreground DNA.
      this.coreShell.material.opacity=early*(.54+.32*eyeHold);
      this.coreGlass.material.opacity=early*(.08+.14*eyeHold);
      this.coreEdges.material.opacity=early*(.10+.22*eyeHold);

      this.irisGroup.scale.setScalar((.08+eyeHold*.92)*pulse);
      this.irisRays.rotation.z=time*.000090;
      this.glowSprite.material.opacity=(.004+eyeHold*.50)*pulse;
      this.glowSprite.scale.setScalar(1.30+eyeHold*.46);
      this.coreInner.material.opacity=.015+eyeHold*.70;
      this.coreLight.intensity=eyeHold*(8.4+Math.sin(time*.005)*1.1);

      if(this.coreLabel)this.coreLabel.material.opacity=.34*early*(1-smooth((t-2.72)/.30));
    }

    updateOrganic(t,time){
      const grow=smooth((t-2.62)/.72);
      ...
      this.organicShellMaterial.opacity=.44*visible;
      this.organicLobeMaterial.opacity=.92*visible;
      this.organicWireMaterial.opacity=.003*visible;
      this.organicVeinMaterial.opacity=.42*visible;
    ...
    updateTentacles(t,time){
      ...
      this.tentacleEdgeMaterial.opacity=.042*grow;
      this.tentacleNodeMaterial.opacity=.42*grow;
    ...
    updateCamera(t,time){
      ...
      if(t<2.55){
        ...
        z=mix(6.22,6.40,smooth((t-9.10)/.60));
        y=.015;
      }
      if(this.width<this.height)z+=1.10;
    ...
    render(r,time){
      ...
      this.renderer.toneMappingExposure=1.16+flash*2.20+after*.08;
      this.coreLight.intensity+=flash*42+after*5;
      if(this.glowSprite){
        const g=1+flash*.52;
  ...
  async function attach(canvas,getTarget){
    ...
    try{
      ...
      if(!visualProof && isSoftwareWebGL()){
        document.documentElement.dataset.fxMagBirthR671='software-webgl-r649-fallback';
        return null;
      }
      loader ||= loadThree();
      ...
      return {
        ...
        engine,
        revision:'r671-reference-cross-dna-early-pod-three-genesis'
      };
    }catch(error){
      console.error('FormatX R671 genesis renderer failed:',error);
      document.documentElement.dataset.fxMagBirthR671='fallback-r649';
      return null;
    }
  }
  ...
  window.FormatXMagGenesisThreeR671={
    attach,
    revision:'r671-reference-cross-dna-early-pod-three-genesis-dna-cellular-living-architecture'
  };
})();