(function () {
  ...
  if (root.dataset.fxApexSceneStability === 'ready-v15') return;
  ...
  function ensureTrueMeshAssets() {
    if (!document.querySelector('link[data-fx-core-mesh3d-style]')) {
      const link = document.createElement('link');
      link.rel = 'stylesheet';
      link.href = './styles/formatx-core-mesh3d.css?v=20260808-true-mesh3d-v7';
    ...
    script.src = './scripts/formatx-core-mesh3d-v7.js?v=20260808-true-mesh3d-v7';
    script.async = false;
    script.dataset.fxCoreMesh3dRuntime = 'true';
    script.addEventListener('load', () => { root.dataset.fxCoreMesh3dLoad = 'ready-v7'; }, { once: true });
  ...
  Context.prototype.shaderSource = function patchedShaderSource(shader, source) {
    if (!shaderPatched && typeof source === 'string' && source.includes('vec2 core(vec3 p)') && source.includes('uniform float uScene;')) {
      ...
      if (patched !== source && complete) {
        shaderPatched = true;
        root.dataset.fxApexReferenceShader = 'mesh-background-no-2d-reactor-v16';
  ...
  addEventListener('formatx:coremesh3dready', event => {
    ...
    root.dataset.fxNativeApexVisual = version === 'v7'
      ? 'reference-additive-layered-true-mesh3d-v16'
      : 'reference-layered-glass-true-mesh3d-v15';
  ...
  root.dataset.fxApexSceneStability = 'ready-v15';
  root.dataset.fxCoreHold = 'stable-before-morph';
  root.dataset.fxCoreMobileComposition = 'true-mesh3d-v7-pending';
}());