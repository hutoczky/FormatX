(function () {
  ...
  if (root.dataset.fxApexSceneStability === 'ready-v10') return;
  ...
  function ensureTrueMeshAssets() {
    if (!document.querySelector('link[data-fx-core-mesh3d-style]')) {
      const link = document.createElement('link');
      link.rel = 'stylesheet';
      link.href = './styles/formatx-core-mesh3d.css?v=20260808-true-mesh3d-v2';
    ...
    script.src = './scripts/formatx-core-mesh3d-v2.js?v=20260808-true-mesh3d-v2';
    script.async = false;
    script.dataset.fxCoreMesh3dRuntime = 'true';
    script.addEventListener('load', () => { root.dataset.fxCoreMesh3dLoad = 'ready-v2'; }, { once: true });
  ...
  Context.prototype.shaderSource = function patchedShaderSource(shader, source) {
    if (!shaderPatched && typeof source === 'string' && source.includes('vec2 core(vec3 p)') && source.includes('uniform float uScene;')) {
      ...
      if (patched !== source && complete) {
        shaderPatched = true;
        root.dataset.fxApexReferenceShader = 'reference-mesh3d-background-v11';
  ...
  addEventListener('formatx:coremesh3dready', event => {
    const version = event.detail?.version === 'v2' ? 'v2' : 'v1';
    root.dataset.fxNativeApexVisual = version === 'v2' ? 'reference-calibrated-true-mesh3d-v11' : 'reference-locked-true-mesh3d-v10';
  ...
  root.dataset.fxApexSceneStability = 'ready-v10';
  root.dataset.fxCoreHold = 'stable-before-morph';
  root.dataset.fxCoreMobileComposition = 'true-mesh3d-v2-pending';
}());