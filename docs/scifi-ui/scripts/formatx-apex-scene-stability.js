(function () {
  ...
  if (root.dataset.fxApexSceneStability === 'ready-v14') return;
  ...
  function ensureTrueMeshAssets() {
    if (!document.querySelector('link[data-fx-core-mesh3d-style]')) {
      const link = document.createElement('link');
      link.rel = 'stylesheet';
      link.href = './styles/formatx-core-mesh3d.css?v=20260808-true-mesh3d-v6';
    ...
    script.src = './scripts/formatx-core-mesh3d-v6.js?v=20260808-true-mesh3d-v6';
    script.async = false;
    script.dataset.fxCoreMesh3dRuntime = 'true';
    script.addEventListener('load', () => { root.dataset.fxCoreMesh3dLoad = 'ready-v6'; }, { once: true });
  ...
  function referenceMeshBackgroundShader(source) {
    let next = source;
    next = next.replace("vec2 core(vec3 p){\n        vec3 q=p;", "vec2 core(vec3 p){\n        if(uScene<.92)return vec2(10.,1.);\n        vec3 q=p;");
    next = next.replace("float angle=mix(-.035+sin(uTime*.11)*.022,travelAngle,1.-coreWeight);", "float angle=mix(.014+sin(uTime*.11)*.014,travelAngle,1.-coreWeight);");
    next = next.replace("float radius=mix(6.02,travelRadius,1.-coreWeight);", "float radius=mix(5.88,travelRadius,1.-coreWeight);");
    next = next.replace("float focal=mix(1.92,1.72,1.-coreWeight);", "float focal=mix(1.99,1.72,1.-coreWeight);");
    next = next.replace(
  ...
  Context.prototype.shaderSource = function patchedShaderSource(shader, source) {
    if (!shaderPatched && typeof source === 'string' && source.includes('vec2 core(vec3 p)') && source.includes('uniform float uScene;')) {
      ...
      const complete = ['if(uScene<.92)return vec2(10.,1.)','float hotCore=exp(-coreDistance*39.)','float reactor1=exp(-abs(coreDistance-.050)*132.)','float pulseWave=exp(-abs(coreDistance-waveRadius)*70.)','float angle=mix(.014+sin(uTime*.11)*.014'].every(marker => patched.includes(marker));
      if (patched !== source && complete) {
        shaderPatched = true;
        root.dataset.fxApexReferenceShader = 'reference-mesh3d-background-v15';
  ...
  addEventListener('formatx:coremesh3dready', event => {
    ...
    root.dataset.fxNativeApexVisual = version === 'v6' ? 'reference-layered-glass-true-mesh3d-v15' : version === 'v5' ? 'reference-bright-faceted-true-mesh3d-v14' : version === 'v4' ? 'reference-faceted-true-mesh3d-v13' : version === 'v3' ? 'reference-luminous-true-mesh3d-v12' : version === 'v2' ? 'reference-calibrated-true-mesh3d-v11' : 'reference-locked-true-mesh3d-v10';
  ...
  root.dataset.fxApexSceneStability = 'ready-v14';
  root.dataset.fxCoreHold = 'stable-before-morph';
  root.dataset.fxCoreMobileComposition = 'true-mesh3d-v6-pending';
}());