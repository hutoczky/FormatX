(function () {
  ...
  const RENDERER_URL = RENDERER_PATH + '?v=20260817-r196b-early-painted-frame';
  ...
  root.dataset.fxCoreRecoveryBootstrap = 'early-r196b';
  ...
  function finishReady(source) {
    ...
    removeEmergency();
    root.dataset.fxCoreNeverStuck = 'ready-r196b-painted-frame';
  ...
  function ensureEmergencyNode() {
    ...
    if (!host || host.querySelector('.fx-core-emergency-r195')) return false;
  ...
  function showFallback(reason) {
    ...
    const host = document.querySelector('#hero .hero-space');
    if (!host) {
      retryTimer = setTimeout(() => showFallback(reason || 'fallback-host-wait'), 120);
      return;
    }
    ensureEmergencyNode();
    root.classList.add('fx-core-fallback-r195');
    root.dataset.fxCoreNeverStuck = 'fallback-r196b';
    root.dataset.fxCoreRecoveryReason = reason || 'deadline';
    root.dataset.fxCoreReal3d = 'fallback-ready-r196b';
    root.dataset.fxCoreReferenceLock = 'fallback-ready-r196b';
    root.dataset.fxCoreMobileV55 = 'fallback-ready-r196b';
    window.dispatchEvent(new CustomEvent('formatx:corevisualready', {
      detail: { renderer: 'css-emergency-r196b', reason: reason || 'deadline' }
    }));
  }
  ...
  function resetRendererState() {
    ...
    root.dataset.fxCoreReal3d = 'loading-r196b-retry';
    root.dataset.fxCoreReferenceLock = 'loading-r196b-retry';
  }
  ...
  function forceRenderer(reason) {
    ...
    retryCount += 1;
    resetRendererState();
    root.dataset.fxCoreNeverStuck = 'retry-' + retryCount + '-r196b';
  ...
  function start() {
    ...
    if (root.dataset.fxCoreRecoveryStarted === 'r196b') return;
    root.dataset.fxCoreRecoveryStarted = 'r196b';
    root.dataset.fxCoreNeverStuck = 'watching-first-painted-frame-r196b';

    // Start independently of DOMContentLoaded. If an earlier deferred resource
    // prevents DOMContentLoaded, the recovery loop must still be alive.
    setTimeout(() => {
  ...
  window.addEventListener(READY_EVENT, () => {
    // Legacy ready fires before the first draw. Initialization is not enough.
    if (isReady()) finishReady('real3dready-with-painted-frame');
    else root.dataset.fxCoreNeverStuck = 'initialized-awaiting-painted-frame-r196b';
  });
