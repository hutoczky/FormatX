(() => {
  ...
  class FormatXGenesisThree {
    ...
    makeMechanicalLayer(){
      ...
      for(let i=0;i<4;i++){
        ...
        m.position.set(Math.cos(a)*.66,Math.sin(a)*.66,-.03);
        m.rotation.z=a;
    ...
    updateCore(t,time){
      ...
      else if(t>=2.55 && t<3.35)sc=mix(1.24,.64,smooth((t-2.55)/.80));
      else if(t>=3.35 && t<5.65)sc=.64+Math.sin((t-3.35)*1.05)*.006;
      else if(t>=5.65 && t<8.95)sc=mix(.64,.94,smooth((t-5.65)/3.30));
  ...
  async function attach(canvas,getTarget){
    ...
    try{
      ...
      if(!visualProof && isSoftwareWebGL()){
        document.documentElement.dataset.fxMagBirthR664='software-webgl-r649-fallback';
        return null;
      }
      loader ||= loadThree();
      ...
      return {
        ...
        engine,
        revision:'r664-soft-optic-reference-three-genesis'
      };
    }catch(error){
      console.error('FormatX R664 genesis renderer failed:',error);
      document.documentElement.dataset.fxMagBirthR664='fallback-r649';
      return null;
    }
  }
  ...
  window.FormatXMagGenesisThreeR664={
    attach,
    revision:'r664-soft-optic-reference-three-genesis-dna-cellular-living-architecture'
  };
})();