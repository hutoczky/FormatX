(() => {
  ...
  class FormatXGenesisThree {
    ...
    updateCore(t,time){
      let sc;
      if(t<2.52) sc=.94+ease(t/2.52)*.10;
      else if(t<3.28) sc=mix(1.04,.36,smooth((t-2.52)/.76));
    ...
    updateOrganic(t,time){
      ...
      this.organicLobeMaterial.opacity=.52*visible;
      this.organicWireMaterial.opacity=.0003*visible;
      this.organicVeinMaterial.opacity=.50*visible;
      this.organicHoodMaterial.opacity=.42*visible;
      if(this.organicFoldMaterial)this.organicFoldMaterial.opacity=.68*visible;
      if(this.organicFoldGlowMaterial)this.organicFoldGlowMaterial.opacity=.12*visible;
    ...
    updateCells(t,time){
      ...
      this.cellVeinMaterial.opacity=.22*visible;
    ...
    updateMechanical(t,time){
      ...
      this.mechanicalGroup.scale.setScalar(.001+grow*1.12);
      ...
      if(this.mechLight)this.mechLight.intensity=13.0*grow;
    ...
    updateCamera(t,time){
      ...
      if(t<2.70){
        ...
        z=4.70+Math.sin(time*.00018)*.006;
        y=.003;
      }else{
        z=mix(4.70,4.84,smooth((t-9.10)/.60));
        y=.003;
      }
  ...
  async function attach(canvas,getTarget){
    ...
    try{
      ...
      if(!visualProof && isSoftwareWebGL()){
        document.documentElement.dataset.fxMagBirthR1270='software-webgl-r649-fallback';
        return null;
      }
      loader ||= loadThree();
      ...
      console.error('FormatX R1270 genesis renderer failed:',error);
      document.documentElement.dataset.fxMagBirthR1270='fallback-r649';
      return null;
    }
  }
  ...
  window.FormatXMagGenesisThreeR1270={
    attach,
