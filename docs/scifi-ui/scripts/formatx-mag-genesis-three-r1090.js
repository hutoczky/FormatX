(() => {
  ...
  class FormatXGenesisThree {
    ...
    updateCore(t,time){
      ...
      else if(t<7.60) sc=mix(.78,.62,smooth((t-5.70)/1.90));
      else if(t<9.55) sc=.62;
      else sc=.56;
      ...
      const surfaceZ=stage1*.80+cellular*1.06;
      ...
      this.coreShell.material.opacity=.92*stage1 + .70*(1-stage1);
      this.coreGlass.material.opacity=.16*stage1 + .080*(1-stage1);
      this.coreEdges.material.opacity=.18*stage1 + .10*(1-stage1);
      if(this.corePetalMaterial)this.corePetalMaterial.opacity=.12*stage1+.20*(1-stage1);

      this.irisGroup.scale.setScalar((.018+eyeHold*.72)*pulse);
      this.irisRays.rotation.z=time*.000060;
      if(this.irisCorona)this.irisCorona.material.opacity=.78*eyeHold;
      this.glowSprite.material.opacity=(.002+eyeHold*.035)*pulse;
      this.glowSprite.scale.setScalar(1.00+eyeHold*.14);
      this.coreInner.material.opacity=.004+eyeHold*.065;
      this.coreLight.intensity=eyeHold*(2.6+Math.sin(time*.0046)*.18);
  ...
  async function attach(canvas,getTarget){
    ...
    try{
      ...
      if(!visualProof && isSoftwareWebGL()){
        document.documentElement.dataset.fxMagBirthR1090='software-webgl-r649-fallback';
        return null;
      }
      loader ||= loadThree();
      ...
      return {
        ...
        engine,
        revision:'r1090-reference-cellular-core-three-genesis'
      };
    }catch(error){
      console.error('FormatX R1090 genesis renderer failed:',error);
      document.documentElement.dataset.fxMagBirthR1090='fallback-r649';
      return null;
    }
  }
  ...
  window.FormatXMagGenesisThreeR1090={
    attach,
    revision:'r1090-reference-cellular-core-three-genesis-dna-cellular-living-architecture'
  };
})();