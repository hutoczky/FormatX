(() => {
  ...
  class FormatXGenesisThree {
    ...
    createHelix(length=4.7,radius=.28,turns=4.1){
      ...
      const rungMat=new T.MeshPhysicalMaterial({
        color:0x72d7e9,emissive:0x2c5c7c,emissiveIntensity:.92,
      ...
      const tubeA=new T.Mesh(new T.TubeGeometry(curveA,seg,.035,8,false),ma);
      const tubeB=new T.Mesh(new T.TubeGeometry(curveB,seg,.035,8,false),mb);
      ...
      const rungGeo=new T.CylinderGeometry(.025,.025,1,8,1,false);
      ...
      group.userData.baseOpacity=[.84,.78,.060,.055,.86,.70,.64];
      return group;
    }
    ...
    makeCore(){
      ...
      const shellMat=new T.MeshPhysicalMaterial({
        color:0x06131b,metalness:.58,roughness:.28,
        emissive:0x071825,emissiveIntensity:.28,
        clearcoat:.52,clearcoatRoughness:.22,
      ...
      const ring1=new T.Mesh(new T.TorusGeometry(.238,.018,10,72),cyan.clone());
      ring1.material.opacity=.42;
      const ring2=new T.Mesh(new T.TorusGeometry(.342,.008,8,72),cyan.clone());
      ring2.material.opacity=.18;
      const pupil=new T.Mesh(new T.CircleGeometry(.095,56),new T.MeshBasicMaterial({color:0x01070b,side:T.DoubleSide}));
      const pupilRing=new T.Mesh(new T.RingGeometry(.103,.148,64),cyan.clone());
      pupilRing.material.opacity=.42;
      this.irisGroup.add(ring2,ring1,pupil,pupilRing);

      const rayMat=new T.LineBasicMaterial({color:0x45d5ff,transparent:true,opacity:.38,depthWrite:false,blending:T.AdditiveBlending});
      const rays=[];
      for(let i=0;i<64;i++){
        const a=i/64*Math.PI*2;
        const inner=.155+(i%3)*.006;
        const outer=.385+(i%7)*.012;
        rays.push(
    ...
    makeOrganicLayer(){
      ...
      this.organicLobes=[];
      const lobeGeo=new T.IcosahedronGeometry(.152,3);
      const count=108;
      for(let i=0;i<count;i++){
        const phi=Math.acos(1-2*(i+.5)/count);
        const theta=Math.PI*(1+Math.sqrt(5))*i;
        const rr=1.31+(r()-.5)*.11;
      ...
      this.organicFoldMaterial=new T.MeshStandardMaterial({
        color:0x241431,roughness:.74,metalness:.015,
        emissive:0x15091f,emissiveIntensity:.16,
        transparent:true,opacity:0
      });
      this.organicFoldGlowMaterial=new T.MeshBasicMaterial({
        color:0x5adbed,transparent:true,opacity:0,
        depthWrite:false,blending:T.AdditiveBlending
      });
      this.organicFolds=new T.Group();
      for(let i=0;i<38;i++){
        const theta0=-1.18+r()*2.36;
        const phi0=.30+r()*2.48;
        const drift=(r()-.5)*.17;
        const phase=r()*Math.PI*2;
        const pts=[];
        for(let j=0;j<8;j++){
          const u=(j-3.5)/7;
          const theta=theta0+u*(.28+r()*.18)+Math.sin(j*.92+phase)*.055;
          const phi=Math.max(.16,Math.min(2.98,phi0+u*(.40+r()*.18)+drift+Math.sin(j*1.1+phase)*.045));
          const rr=1.345+(r()-.5)*.025;
          pts.push(new T.Vector3(
            Math.sin(phi)*Math.sin(theta)*rr,
            Math.cos(phi)*rr,
            Math.sin(phi)*Math.cos(theta)*rr*.73+.035
          ));
        }
        const curve=new T.CatmullRomCurve3(pts,false,'centripetal');
        const ridge=new T.Mesh(
          new T.TubeGeometry(curve,30,.030,6,false),
          this.organicFoldMaterial
        );
        const glow=new T.Mesh(
          new T.TubeGeometry(curve,30,.0065,5,false),
          this.organicFoldGlowMaterial
        );
        ridge.userData.phase=phase;
        glow.userData.phase=phase;
        this.organicFolds.add(ridge,glow);
      }
    ...
    makeTentacles(){
      ...
      this.tentacleMaterial=new T.MeshStandardMaterial({
        color:0x050e14,metalness:.32,roughness:.40,
        emissive:0x06323e,emissiveIntensity:.26,
        transparent:true,opacity:0
      });
      this.tentacleEdgeMaterial=new T.MeshBasicMaterial({
        color:0x71dcea,wireframe:true,transparent:true,opacity:0,
        depthWrite:false,blending:T.AdditiveBlending
      });
      this.tentacleNodeMaterial=new T.PointsMaterial({
        color:0x76e7f7,size:.026,transparent:true,opacity:0,
        depthWrite:false,blending:T.AdditiveBlending,sizeAttenuation:true
      });
      this.tentacleGlowMaterial=new T.MeshBasicMaterial({
        color:0x42d7ec,transparent:true,opacity:0,
        depthWrite:false,blending:T.AdditiveBlending
      });
      ...
      const count=9;
      for(let i=0;i<count;i++){
        const base=i/count*Math.PI*2+(r()-.5)*.13;
        const sign=i%2?1:-1;
        const len=1.72+r()*1.08;
        const phase=r()*Math.PI*2;
        const pts=[];
        for(let j=0;j<8;j++){
          const u=j/7;
          const radius=1.10+len*u;
          const curl=sign*Math.sin(u*Math.PI*2.05+phase*.22)*(.10+.24*u);
          const a=base+curl;
          const sideWave=Math.sin(u*Math.PI*3.0+phase)*(.05+.12*u);
          pts.push(new T.Vector3(
            Math.cos(a)*radius-Math.sin(a)*sideWave,
            Math.sin(a)*radius+Math.cos(a)*sideWave,
            Math.sin(u*Math.PI*2.2+phase)*(.08+.22*u)
          ));
        }
        const curve=new T.CatmullRomCurve3(pts,false,'centripetal');
        const geo=this.createTaperedTube(curve,60,8,.080+r()*.018,.008+r()*.003);
        const glowGeo=this.createTaperedTube(curve,60,6,.016+r()*.004,.003+r()*.001);
    ...
    updateCore(t,time){
      ...
      else if(t<3.35)sc=mix(.60,.68,smooth((t-2.55)/.80));
      else if(t<5.65)sc=.68+Math.sin((t-3.35)*1.02)*.003;
      else if(t<8.95)sc=mix(.68,.52,smooth((t-5.65)/3.30));
      else sc=.48;
      ...
      const early=smooth((t-.08)/.52);
      const eye=smooth((t-2.58)/.40);
      const eyeHold=eye*(1-smooth((t-9.46)/.28));
      const pulse=.975+.025*Math.sin(time*.0050);

      this.coreShell.material.opacity=early*(.66+.18*eyeHold);
      this.coreGlass.material.opacity=early*(.070+.095*eyeHold);
      this.coreEdges.material.opacity=early*(.10+.14*eyeHold);

      this.irisGroup.scale.setScalar((.055+eyeHold*.96)*pulse);
      this.irisRays.rotation.z=time*.000075;
      this.glowSprite.material.opacity=(.002+eyeHold*.34)*pulse;
      this.glowSprite.scale.setScalar(1.08+eyeHold*.34);
      this.coreInner.material.opacity=.008+eyeHold*.58;
      this.coreLight.intensity=eyeHold*(6.1+Math.sin(time*.005)*.70);

      if(this.coreLabel)this.coreLabel.material.opacity=.30*early*(1-smooth((t-2.62)/.32));
    }
    ...
    updateOrganic(t,time){
      ...
      this.organicShellMaterial.opacity=.36*visible;
      ...
      if(this.organicFoldMaterial)this.organicFoldMaterial.opacity=.92*visible;
      ...
      if(this.organicFolds){
        this.organicFolds.rotation.y=Math.sin(time*.00018)*.018;
        this.organicFolds.rotation.x=Math.sin(time*.00014)*.010;
      }
    }
    ...
    updateCells(t,time){
      ...
      this.cellMaterial.opacity=.14*visible;
    ...
    updateTentacles(t,time){
      ...
      this.tentacleEdgeMaterial.opacity=.025*grow;
      this.tentacleNodeMaterial.opacity=.055*grow;
      if(this.tentacleGlowMaterial)this.tentacleGlowMaterial.opacity=.28*grow;
    ...
    render(r,time){
      ...
      this.particles.material.opacity=.54+.14*Math.sin(time*.00045);

      const flash=smooth((t-9.15)/.10)*(1-smooth((t-9.55)/.24));
      const after=smooth((t-9.44)/.34);
      this.renderer.toneMappingExposure=1.13+flash*.12+after*.02;
      this.coreLight.intensity+=flash*9+after*2;
      if(this.glowSprite){
      ...
      if(this.flashBurst){
        ...
        const burstScale=4.15+flash*3.10;
  ...
  async function attach(canvas,getTarget){
    ...
    try{
      ...
      if(!visualProof && isSoftwareWebGL()){
        document.documentElement.dataset.fxMagBirthR676='software-webgl-r649-fallback';
        return null;
      }
      loader ||= loadThree();
      ...
      return {
        ...
        engine,
        revision:'r676-reference-scale-three-genesis'
      };
    }catch(error){
      console.error('FormatX R676 genesis renderer failed:',error);
      document.documentElement.dataset.fxMagBirthR676='fallback-r649';
      return null;
    }
  }
  ...
  window.FormatXMagGenesisThreeR676={
    attach,
    revision:'r676-reference-scale-three-genesis-dna-cellular-living-architecture'
  };
})();