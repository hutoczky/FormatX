(() => {
  ...
  class FormatXGenesisThree {
    ...
    makeCore(){
      ...
      const rayMat=new T.LineBasicMaterial({
        color:0x45d8ff,transparent:true,opacity:.045,
        depthWrite:false,blending:T.AdditiveBlending
      });
      const rays=[];
      for(let i=0;i<48;i++){
    ...
    updateCore(t,time){
      ...
      this.coreShell.material.opacity=.92*stage1 + .42*(1-stage1);
      this.coreGlass.material.opacity=.16*stage1 + .045*(1-stage1);
      ...
      this.irisGroup.scale.setScalar((.010+eyeHold*.78)*pulse);
      ...
      this.coreLight.intensity=eyeHold*(2.45+Math.sin(time*.0046)*.18);
  ...
  async function attach(canvas,getTarget){
    ...
    try{
      ...
      if(!visualProof && isSoftwareWebGL()){
        document.documentElement.dataset.fxMagBirthR1020='software-webgl-r649-fallback';
        return null;
      }
      loader ||= loadThree();
      ...
      return {
        ...
        engine,
        revision:'r1020-reference-optics-three-genesis'
      };
    }catch(error){
      console.error('FormatX R1020 genesis renderer failed:',error);
      document.documentElement.dataset.fxMagBirthR1020='fallback-r649';
      return null;
    }
  }
  ...
  window.FormatXMagGenesisThreeR1020={
    attach,
    revision:'r1020-reference-optics-three-genesis-dna-cellular-living-architecture'
  };
})();