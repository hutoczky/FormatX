(() => {
  ...
  class FormatXGenesisThree {
    ...
    makeCore(){
      ...
      const shellMat=new T.MeshPhysicalMaterial({
        color:0x172630,metalness:.70,roughness:.24,
        emissive:0x091c25,emissiveIntensity:.14,
        clearcoat:.78,clearcoatRoughness:.14,
        transparent:true,opacity:.92
      });
      const shellGlass=new T.MeshPhysicalMaterial({
        color:0x31515e,metalness:.16,roughness:.18,
    ...
    makeOrganicLayer(){
      ...
      this.organicLobeMaterial=new T.MeshPhysicalMaterial({
        color:0x342047,roughness:.76,metalness:.006,
        clearcoat:.08,clearcoatRoughness:.62,
        transparent:true,opacity:0,
        emissive:0x2f143f,emissiveIntensity:.24,
        depthWrite:true
      });
      ...
      this.organicLobes=[];
      const lobeGeo=new T.IcosahedronGeometry(.158,3);
      const count=92;
      for(let i=0;i<count;i++){
        const phi=Math.acos(1-2*(i+.5)/count);
        const theta=Math.PI*(1+Math.sqrt(5))*i;
        const rr=1.31+(r()-.5)*.085;
      ...
      this.organicFoldMaterial=new T.MeshPhysicalMaterial({
        color:0x321d43,roughness:.76,metalness:.005,
        emissive:0x24102f,emissiveIntensity:.18,
      ...
      for(let pair=0;pair<foldPairs;pair++){
        ...
        for(const side of [-1,1]){
          ...
          const fold=new T.Mesh(
            new T.TubeGeometry(curve,40,.018+r()*.007,7,false),
            this.organicFoldMaterial
          );
          const glow=new T.Mesh(
            new T.TubeGeometry(curve,40,.0028,5,false),
      ...
      for(let i=0;i<40;i++){
        ...
        const vein=new T.Mesh(
          new T.TubeGeometry(curve,28,.010+r()*.003,6,false),
    ...
    makeTentacles(){
      ...
      for(let i=0;i<count;i++){
        ...
        const curve=new T.CatmullRomCurve3(pts,false,'centripetal');
        const geo=this.createTaperedTube(curve,62,8,.090+r()*.016,.011+r()*.003);
        const glowGeo=this.createTaperedTube(curve,62,6,.019+r()*.003,.0045+r()*.001);
        ...
        const nodeGeo=new T.BufferGeometry().setFromPoints(curve.getPoints(36).filter((_,idx)=>idx%4===0));
    ...
    updateCore(t,time){
      let sc;
      if(t<2.70) sc=.66+ease(t/2.70)*.06;
      else if(t<3.30) sc=mix(.72,.52,smooth((t-2.70)/.60));
      else if(t<5.70) sc=.52;
      else if(t<9.55) sc=mix(.52,.43,smooth((t-5.70)/3.85));
      else sc=.43;
      ...
      this.coreShell.material.opacity=.58*stage1 + .30*(1-stage1);
      this.coreGlass.material.opacity=.10*stage1 + .030*(1-stage1);
      this.coreEdges.material.opacity=.14*stage1 + .030*(1-stage1);
      if(this.corePetalMaterial)this.corePetalMaterial.opacity=.040*stage1+.010*(1-stage1);

      this.irisGroup.scale.setScalar((.010+eyeHold*1.18)*pulse);
      this.irisRays.rotation.z=time*.000060;
      if(this.irisCorona)this.irisCorona.material.opacity=.92*eyeHold;
      this.glowSprite.material.opacity=(.001+eyeHold*.030)*pulse;
      this.glowSprite.scale.setScalar(1.00+eyeHold*.12);
      this.coreInner.material.opacity=.002+eyeHold*.050;
      this.coreLight.intensity=eyeHold*(3.5+Math.sin(time*.0046)*.24);

      if(this.coreLabel)this.coreLabel.material.opacity=.10*stage1;
      if(this.coreContours){
        this.coreContours.forEach((m,i)=>{
          m.material.opacity=(.10-i*.030)*stage1;
    ...
    updateOrganic(t,time){
      ...
      this.organicLobeMaterial.opacity=.76*visible;
      this.organicWireMaterial.opacity=.0006*visible;
      this.organicVeinMaterial.opacity=.78*visible;
      this.organicHoodMaterial.opacity=.16*visible;
      if(this.organicFoldMaterial)this.organicFoldMaterial.opacity=.78*visible;
      if(this.organicFoldGlowMaterial)this.organicFoldGlowMaterial.opacity=.08*visible;
    ...
    updateTentacles(t,time){
      ...
      this.tentacleNodeMaterial.opacity=.46*grow;
      if(this.tentacleGlowMaterial)this.tentacleGlowMaterial.opacity=.10*grow;
      if(this.tentacleDashMaterial)this.tentacleDashMaterial.opacity=.68*grow;
    ...
    updateCamera(t,time){
      ...
      if(t<2.70){
        ...
        z=4.05+Math.sin(time*.00020)*.010;
        ...
        z=5.02+Math.sin(time*.00017)*.009;
        y=.004;
      }
  ...
  async function attach(canvas,getTarget){
    ...
    try{
      ...
      if(!visualProof && isSoftwareWebGL()){
        document.documentElement.dataset.fxMagBirthR990='software-webgl-r649-fallback';
        return null;
      }
      loader ||= loadThree();
      ...
      return {
        ...
        engine,
        revision:'r990-biotic-tissue-three-genesis'
      };
    }catch(error){
      console.error('FormatX R990 genesis renderer failed:',error);
      document.documentElement.dataset.fxMagBirthR990='fallback-r649';
      return null;
    }
  }
  ...
  window.FormatXMagGenesisThreeR990={
    attach,
    revision:'r990-biotic-tissue-three-genesis-dna-cellular-living-architecture'
  };
})();