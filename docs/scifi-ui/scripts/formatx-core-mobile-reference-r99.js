(function(){
'use strict';
const root=document.documentElement,READY='ready-v69',VERSION='reference-luminous-crystal-webgl-r99-award-r109';
...
function boot(attempt=0){
  ...
  const stage=document.createElement('div');stage.className='fx-core-mobile-v55-stage fx-core-rayglass-r91-stage';stage.dataset.active='true';stage.dataset.renderer='reference-r109-volumetric';stage.setAttribute('aria-hidden','true');host.prepend(stage);
...
void main(){
 ...
 float rayFan=0.;rayFan+=band(p.x-p.y*.18+.018*sin(p.y*4.3+fbm(wp*2.1)*2.8+t*.025),.0036)+band(p.x+p.y*.20-.016*sin(p.y*4.7+fbm(wp*2.35+2.)*2.5-t*.021),.0036);rayFan+=.40*(band(p.y-p.x*.30+.015*sin(p.x*5.1+fbm(wp*2.2-2.)*2.4),.0038)+band(p.y+p.x*.28-.014*sin(p.x*5.4+fbm(wp*2.4+4.)*2.2),.0038));rayFan*=inside*smoothstep(.12,.94,rr)*(.20+.80*fbm(wp*2.8+4.));
 float curveV2=.075+.255*pow(sat(1.-vy),1.22),curveH2=.070+.235*pow(sat(1.-vx),1.24);float ribs2=(band(abs(p.x)-curveV2,.0070)*smoothstep(.22,.96,vy)+band(abs(p.y)-curveH2,.0070)*smoothstep(.22,.96,vx))*inside*(.24+.76*depth);
 ...
 float caustic=band(fbm(q1*3.35+vec2(.3,-.7))-.50,.0105)+.86*band(fbm(q2*3.75+vec2(-1.7,.8))-.53,.0100)+.72*band(fbm(q3*4.05+vec2(1.9,2.4))-.46,.0095)+.55*band(fbm(q4*4.35+vec2(-2.8,-1.3))-.57,.0090);caustic*=inside*(.13+.87*depth);
 float glassVeil=inside*depth*(.20+.80*pow(sat(.5+.5*sin(fbm(q1*1.7)*5.2+fbm(q2*1.9+2.)*4.1+a*1.2)),2.4));
 ...
 col+=cyan*(edge*1.18+edgeGlow*.18+cEdge*.34+shells*.42+ribs*.48+ribs2*.30+rayFan*.20+fracture*.44+caustic*.64+wisps*.40);
 col+=vio*(vEdge*.52+shells*.18+ribs*.10+ribs2*.16+rayFan*.12+fracture*.32+caustic*.26+wisps*.28);
 col+=ice*(fracture*.46+shard*.54+ribs*.42+ribs2*.24+rayFan*.20+caustic*.78+wisps*.34+shells*.22+rings*.84+spoke*.16+beamH*.42+beamV*.24);
 col+=cyan*(rings*1.24+shard*.20+ribs*.22+ribs2*.18+rayFan*.14+caustic*.44+spoke*.12+beamH*1.58+beamV*.78);
 // Compact white reactor with a cyan halo and restrained violet spectral edge.
 col+=ice*(hot*(6.2+.82*uEnergy)+core*1.65)+cyan*(core*1.42+halo*.66)+vio*(band(rr-.126,.0028)*.14+band(rr-.176,.0030)*.11);
 col=mix(col,vec3(1.10,1.24,1.30)*2.15,sat(hot*.96));
 col+=ice*(tipX+tipY)*.14+cyan*(tipX+tipY)*.34;
 vec3 outer=cyan*(o1*.62+o3*.36)+vio*(o2*.60+o3*.28);col+=outer+cyan*(ripple*.92+horizon*.62+reflection*.46)+vio*(ripple*.48+reflection*.18)+ice*(spark*.72+horizon*.22+reflection*.16);
 float alpha=inside*(.016+.028*depth+.018*pane+.030*folds+.018*glassVeil)+edge*.52+edgeGlow*.040+shells*.030+ribs*.026+ribs2*.020+rayFan*.010+fracture*.020+caustic*.032+shard*.022+wisps*.020+rings*.038+core*.20+hot*.55+beamH*.036+beamV*.026;
 ...
 col*=1.62+uEnergy*.14;col=pow(max(col,vec3(0.)),vec3(.88));${webgl2?'outColor':'gl_FragColor'}=vec4(col,clamp(alpha,0.,.88));
  ...
  let prog;try{prog=program(gl,vs,fs);}catch(e){console.error('[FormatX MAG r109 shader]',e);stage.remove();root.dataset.fxCoreReal3d='shader-failed';root.dataset.fxCoreMobileR99='shader-failed';return;}
