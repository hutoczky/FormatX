/* FormatX R770/R852 — contract-window single-owner premium intro release.
...
let audio=null,preloaderTimer=0,preloaderDeadlineTimer=0,preloaderRevealTimer=0,preloaderPhaseTimerA=0,preloaderPhaseTimerB=0,preloaderWatchdog=0,preloaderReleaseWorker=null,preloaderReleased=false,preloaderDeadlineAbort=null,preloaderReleaseAbort=null,introNotifyFrameA=0,introNotifyFrameB=0;
...
function cancelWorkerReleaseClock(){if(!preloaderReleaseWorker){return;}try{preloaderReleaseWorker.postMessage({type:'cancel'});}catch(_){}try{preloaderReleaseWorker.terminate();}catch(_){}preloaderReleaseWorker=null;}
function cancelPreloaderTimers(){if(preloaderTimer)clearTimeout(preloaderTimer);preloaderTimer=0;if(preloaderRevealTimer)clearTimeout(preloaderRevealTimer);preloaderRevealTimer=0;if(preloaderPhaseTimerA)clearTimeout(preloaderPhaseTimerA);preloaderPhaseTimerA=0;if(preloaderPhaseTimerB)clearTimeout(preloaderPhaseTimerB);preloaderPhaseTimerB=0;if(preloaderWatchdog)clearInterval(preloaderWatchdog);preloaderWatchdog=0;cancelWorkerReleaseClock();cancelReleaseWindow();}
...
function armWorkerReleaseClock(delay){if(MOBILE||REDUCED||typeof Worker!=='function')return;cancelWorkerReleaseClock();try{const boundedDelay=Math.max(0,delay);const targetEpoch=Date.now()+boundedDelay;const worker=new Worker('/scifi-ui/scripts/formatx-preloader-release-clock-r852.js?v=20260918-r852');preloaderReleaseWorker=worker;worker.onmessage=event=>{if(event.data?.type!=='release'||preloaderReleased)return;ROOT.dataset.fxPreloaderReleaseClockR852='signal-received';introMark('workerReleaseSignalAt');requestPreloaderRelease('worker-release-clock-r852');};worker.onerror=()=>{ROOT.dataset.fxPreloaderReleaseClockR852='worker-error-existing-fallbacks-active';cancelWorkerReleaseClock();};worker.postMessage({type:'arm',targetEpoch,token:'r852'});ROOT.dataset.fxPreloaderReleaseClockR852='armed-absolute-target-existing-fallbacks-preserved';}catch(_){preloaderReleaseWorker=null;ROOT.dataset.fxPreloaderReleaseClockR852='unavailable-existing-fallbacks-active';}}
