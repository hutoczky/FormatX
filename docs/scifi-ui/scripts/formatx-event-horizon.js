...
function armFrameDeadline(target){if(REDUCED||preloaderFrameDeadline||preloaderReleased)return;const frame=()=>{preloaderFrameDeadline=0;if(preloaderReleased)return;const elapsed=performance.now()-PRELOADER_BOOT_AT;if(elapsed>=target){INTRO_TIMING.frameDeadlineAt=performance.now();requestPreloaderRelease('frame-deadline-r813');return;}preloaderFrameDeadline=requestAnimationFrame(frame);};preloaderFrameDeadline=requestAnimationFrame(frame);}
