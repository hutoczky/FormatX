...
ROOT.dataset.fxIntroNarrativeR852='mag-birth-text-seed-to-hero-core';
...
function setIntroPhase(overlay,phase){if(!(overlay instanceof HTMLElement)||preloaderReleased)return;overlay.dataset.fxIntroPhaseR635=phase;const output=overlay.querySelector('[data-fx-intro-output]');const progress=overlay.querySelector('[data-fx-intro-progress]');const status=overlay.querySelector('[data-fx-intro-status]');const word=overlay.querySelector('.fx-intro-word'),wordSpan=word?.querySelector('span');const english=ROOT.lang==='en';const phases={wake:{out:'BIRTH',value:24,en:'LIVING CORE BIRTH',hu:'AZ ÉLŐ MAG SZÜLETIK'},sync:{out:'ALIVE',value:72,en:'LIVING CORE AWAKE',hu:'AZ ÉLŐ MAG FELÉBRED'},ready:{out:'MERGE',value:100,en:'CORE JOINING SYSTEM',hu:'A MAG A RENDSZERBE LÉP'}};const state=phases[phase]||phases.wake;if(output instanceof HTMLOutputElement)output.value=state.out;if(progress instanceof HTMLProgressElement){progress.max=100;progress.value=state.value;progress.setAttribute('aria-label',english?state.en:state.hu);}if(status instanceof HTMLElement)status.textContent=english?state.en:state.hu;if(wordSpan instanceof HTMLElement)wordSpan.textContent=english?'CORE':'MAG';if(phase==='ready'&&word instanceof HTMLElement&&!REDUCED){const target=MOBILE?'translate3d(0,-19svh,0) scale(.14)':'translate3d(clamp(210px,21vw,300px),clamp(-58px,-4vh,-28px),0) scale(.14)';animateEffect(word,[{transform:'translate3d(0,0,0) scale(1)',opacity:1},{transform:target,opacity:.14}],{duration:MOBILE?150:140,easing:'cubic-bezier(.22,.78,.18,1)',fill:'forwards'});}}
