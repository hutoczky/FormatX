...
ROOT.dataset.fxIntroNarrativeR850='mag-policy-birth-alive-merge';
...
function setIntroPhase(overlay,phase){if(!(overlay instanceof HTMLElement)||preloaderReleased)return;overlay.dataset.fxIntroPhaseR635=phase;const output=overlay.querySelector('[data-fx-intro-output]');const progress=overlay.querySelector('[data-fx-intro-progress]');const status=overlay.querySelector('[data-fx-intro-status]');const english=ROOT.lang==='en';const phases={wake:{out:'BIRTH',value:24,en:'CORE SIGNAL FORMING',hu:'MAG JEL SZÜLETIK'},sync:{out:'ALIVE',value:72,en:'LIVING CORE AWAKENING',hu:'AZ ÉLŐ MAG FELÉBRED'},ready:{out:'MERGE',value:100,en:'CORE JOINING SYSTEM',hu:'A MAG A RENDSZERBE LÉP'}};const state=phases[phase]||phases.wake;if(output instanceof HTMLOutputElement)output.value=state.out;if(progress instanceof HTMLProgressElement){progress.max=100;progress.value=state.value;progress.setAttribute('aria-label',english?state.en:state.hu);}if(status instanceof HTMLElement)status.textContent=english?state.en:state.hu;dispatchEvent(new CustomEvent('formatx:introphase',{detail:{phase,value:state.value}}));}
