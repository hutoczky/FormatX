...
function runPreloader(overlay){if(!(overlay instanceof HTMLElement)){ROOT.dataset.fxPreloaderR531='unavailable';return;}const elapsed=()=>performance.now()-PRELOADER_BOOT_AT;const scheduleAt=(target,callback)=>setTimeout(callback,Math.max(0,target-elapsed()));introMark('releaseScheduledAt');INTRO_TIMING.releaseDueAt=PRELOADER_BOOT_AT+(MOBILE?PRELOADER_MIN_MS:PRELOADER_RELEASE_AT_MS);const absoluteDeadline=()=>finalizePreloader('absolute-deadline');armDeadline(absoluteDeadline,Math.max(0,PRELOADER_MAX_MS-elapsed()));if(REDUCED){preloaderTimer=scheduleAt(PRELOADER_MIN_MS,()=>requestPreloaderRelease('reduced-semantic-ready'));return;}const watchdogTarget=MOBILE?PRELOADER_MIN_MS:PRELOADER_RELEASE_AT_MS;preloaderWatchdog=setInterval(()=>{if(preloaderReleased)return;if(elapsed()>=watchdogTarget)requestPreloaderRelease('monotonic-watchdog-r687');},PRELOADER_WATCHDOG_MS);armReleaseWindow(()=>requestPreloaderRelease('bounded-window-release-r828'),Math.max(0,PRELOADER_RELEASE_AT_MS-elapsed()));preloaderPhaseTimerA=scheduleAt(PRELOADER_VISUAL_MS*.30,()=>setIntroPhase(overlay,'sync'));preloaderPhaseTimerB=scheduleAt(PRELOADER_VISUAL_MS*.75,()=>setIntroPhase(overlay,'ready'));preloaderRevealTimer=scheduleAt(PRELOADER_REVEAL_AT_MS,()=>startReveal(overlay));const sentinel=overlay.querySelector('[data-fx-intro-deadline-sentinel]');if(sentinel instanceof HTMLElement){sentinel.addEventListener('animationend',event=>{if(event.target!==sentinel||event.animationName!=='fx-r856-release-sentinel')return;ROOT.dataset.fxPreloaderSentinelR856=String(Math.round(elapsed()));requestPreloaderRelease('compositor-sentinel-r856');},{once:true});} overlay.addEventListener('animationend',event=>{if(event.target!==overlay||event.animationName!=='fx-r533-preloader-visual-bound')return;ROOT.dataset.fxPreloaderVisualEndR635=String(Math.round(elapsed()));requestPreloaderRelease('visual-complete');},{once:true});}
