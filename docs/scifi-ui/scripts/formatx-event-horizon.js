...
let audio=null,preloaderTimer=0,preloaderDeadlineTimer=0,preloaderRevealTimer=0,preloaderPhaseTimerA=0,preloaderPhaseTimerB=0,preloaderWatchdog=0,preloaderReleased=false,preloaderDeadlineAbort=null,preloaderReleaseAbort=null,introNotifyFrameA=0,introNotifyFrameB=0,introMagStage=null;
...
function configureIntroMag(stage){
  if(MOBILE||REDUCED||!(stage instanceof HTMLElement)||preloaderReleased)return false;
  ...
  const core=window.FormatXLivingCore;
  try{core?.requestRender?.(24);core?.surfacePulse?.('intro-birth-r840');}catch(_){}
  return true;
}
function armIntroMagContinuity(){
  if(MOBILE||REDUCED||preloaderReleased)return;
  const adopt=()=>{const stage=document.querySelector('#hero .hero-space > .fx-crystal-organism-r326-stage');if(stage instanceof HTMLElement)configureIntroMag(stage);};
  adopt();
  if(!introMagStage)addEventListener('formatx:real3dready',adopt,{once:true,passive:true});
}
function dockIntroMag(){
...
function finalizePreloader(source){
  ...
  ROOT.dataset.fxPreloaderR531='done';ROOT.dataset.fxPreloaderReleaseR531=source;ROOT.dataset.fxPreloaderReleaseElapsedR635=String(Math.round(elapsed));ROOT.dataset.fxPreloaderFinalizerR635='single-dispatch-complete';releaseIntroMag();introMark('completedAt');
