...
function runPreloader(overlay){if(!(overlay instanceof HTMLElement)){ROOT.dataset.fxPreloaderR531='unavailable';return;}const elapsed=()=>performance.now()-PRELOADER_BOOT_AT;const scheduleAt=(target,callback)=>setTimeout(callback,Math.max(0,target-elapsed()));introMark('releaseScheduledAt');INTRO_TIMING.releaseDueAt=PRELOADER_BOOT_AT+(MOBILE?PRELOADER_MIN_MS:PRELOADER_RELEASE_AT_MS);const absoluteDeadline=()=>finalizePreloader('absolute-deadline');armDeadline(absoluteDeadline,Math.max(0,PRELOADER_MAX_MS-elapsed()));if(REDUCED){preloaderTimer=scheduleAt(PRELOADER_MIN_MS,()=>requestPreloaderRelease('reduced-semantic-ready'));return;}const watchdogTarget=MOBILE?PRELOADER_MIN_MS:PRELOADER_RELEASE_AT_MS;preloaderWatchdog=setInterval(()=>{if(preloaderReleased)return;if(elapsed()>=watchdogTarget)requestPreloaderRelease('monotonic-watchdog-r687');},PRELOADER_WATCHDOG_MS);armReleaseWindow(()=>requestPreloaderRelease('bounded-window-release-r828'),Math.max(0,PRELOADER_RELEASE_AT_MS-elapsed()));preloaderPhaseTimerA=scheduleAt(PRELOADER_VISUAL_MS*.30,()=>setIntroPhase(overlay,'sync'));preloaderPhaseTimerB=scheduleAt(PRELOADER_VISUAL_MS*.75,()=>setIntroPhase(overlay,'ready'));preloaderRevealTimer=scheduleAt(PRELOADER_REVEAL_AT_MS,()=>startReveal(overlay));const onIntroAnimationEnd=event=>{if(preloaderReleased){overlay.removeEventListener('animationend',onIntroAnimationEnd);return;}const desktopClock=!MOBILE&&event.animationName==='fx-r840j-desktop-release-clock'&&event.target instanceof HTMLElement&&event.target.classList.contains('fx-intro-progress-wrap');const visualClock=event.target===overlay&&event.animationName==='fx-r533-preloader-visual-bound';if(!desktopClock&&!visualClock)return;ROOT.dataset.fxPreloaderVisualEndR635=String(Math.round(elapsed()));const released=requestPreloaderRelease(desktopClock?'desktop-css-release-clock-r840j':'visual-complete');if(released)overlay.removeEventListener('animationend',onIntroAnimationEnd);};overlay.addEventListener('animationend',onIntroAnimationEnd);}
