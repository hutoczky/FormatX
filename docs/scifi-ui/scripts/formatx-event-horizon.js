...
ROOT.dataset.fxIntroNarrativeR847='mag-birth-handoff-to-live-core';
...
function setIntroPhase(overlay,phase){if(!(overlay instanceof HTMLElement)||preloaderReleased)return;overlay.dataset.fxIntroPhaseR635=phase;const output=overlay.querySelector('[data-fx-intro-output]');const progress=overlay.querySelector('[data-fx-intro-progress]');const status=overlay.querySelector('[data-fx-intro-status]');const english=ROOT.lang==='en';const phases={wake:{out:'BIRTH',value:24,en:'ENERGY COALESCING',hu:'ENERGIA ÖSSZEÁLL'},sync:{out:'CORE',value:72,en:'LIVING CORE BORN',hu:'AZ ÉLŐ MAG MEGSZÜLETETT'},ready:{out:'MERGE',value:100,en:'MERGING WITH LIVING CORE',hu:'EGYESÜLÉS AZ ÉLŐ MAGGAL'}};const state=phases[phase]||phases.wake;if(output instanceof HTMLOutputElement)output.value=state.out;if(progress instanceof HTMLProgressElement){progress.max=100;progress.value=state.value;progress.setAttribute('aria-label',english?state.en:state.hu);}if(status instanceof HTMLElement)status.textContent=english?state.en:state.hu;}
function beginMagHandoff(overlay){
  if(preloaderReleased||REDUCED||!(overlay instanceof HTMLElement))return;
  setIntroPhase(overlay,'ready');
  const birthCore=overlay.querySelector('.fx-intro-word');
  const heroSpace=document.querySelector('#hero .hero-space');
  const liveCore=document.querySelector('#hero .hero-space .fx-reference-mag-button,#hero .hero-space .fx-crystal-organism-r326-stage')||heroSpace;
  if(!(birthCore instanceof HTMLElement)||!(liveCore instanceof HTMLElement)){ROOT.dataset.fxMagBirthHandoffR847='target-unavailable';return;}
