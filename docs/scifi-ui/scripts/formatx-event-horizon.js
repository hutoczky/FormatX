...
const PRELOADER_DEADLINE_WORKER_URL='/scifi-ui/scripts/formatx-preloader-deadline-worker-r842.js?v=20260919-r842-desktop-release-clock';
...
let audio=null,preloaderTimer=0,preloaderDeadlineTimer=0,preloaderRevealTimer=0,preloaderPhaseTimerA=0,preloaderPhaseTimerB=0,preloaderWatchdog=0,preloaderReleased=false,preloaderDeadlineAbort=null,preloaderReleaseAbort=null,preloaderReleaseWorker=null,introNotifyFrameA=0,introNotifyFrameB=0;
...
function cancelReleaseWorker(){if(preloaderReleaseWorker){try{preloaderReleaseWorker.terminate();}catch(_){}preloaderReleaseWorker=null;}}
function cancelReleaseWindow(){try{preloaderReleaseAbort?.abort();}catch(_){}preloaderReleaseAbort=null;cancelReleaseWorker();}
...
function armReleaseWorker(callback,delay){if(MOBILE||REDUCED||typeof Worker!=='function')return false;try{const worker=new Worker(PRELOADER_DEADLINE_WORKER_URL);const token=`${Math.round(PRELOADER_BOOT_AT)}-${Math.round(delay)}`;preloaderReleaseWorker=worker;worker.addEventListener('message',event=>{if(worker!==preloaderReleaseWorker||String(event.data?.token||'')!==token)return;cancelReleaseWorker();callback();},{once:true});worker.addEventListener('error',()=>{if(worker===preloaderReleaseWorker){cancelReleaseWorker();ROOT.dataset.fxPreloaderDeadlineWorkerR842='error-fallback-active';}},{once:true});worker.postMessage({delay:Math.max(0,delay),token});ROOT.dataset.fxPreloaderDeadlineWorkerR842='armed-desktop-single-signal';return true;}catch(_){ROOT.dataset.fxPreloaderDeadlineWorkerR842='unavailable-fallback-active';return false;}}
function armReleaseWindow(callback,delay){if(preloaderTimer)clearTimeout(preloaderTimer);preloaderTimer=0;cancelReleaseWindow();const boundedDelay=Math.max(0,delay);armReleaseWorker(callback,boundedDelay);const browserScheduler=globalThis.scheduler;if(browserScheduler&&typeof browserScheduler.postTask==='function'&&typeof AbortController==='function'){preloaderReleaseAbort=new AbortController();browserScheduler.postTask(callback,{delay:boundedDelay,priority:'user-blocking',signal:preloaderReleaseAbort.signal}).catch(()=>{});preloaderTimer=setTimeout(callback,boundedDelay+16);ROOT.dataset.fxPreloaderReleaseQueueR828='scheduler-user-blocking-plus-timer';return;}preloaderTimer=setTimeout(callback,boundedDelay);ROOT.dataset.fxPreloaderReleaseQueueR828='timer-fallback';}
