...
const CONTENT_PAINTED_BEFORE_BOOT=performance.getEntriesByType('paint').some(entry=>entry.name==='first-contentful-paint'&&entry.startTime<=PRELOADER_BOOT_AT);
...
function skipPaintedPreloader(){ROOT.dataset.fxPreloaderLatePaintSkipR840='true';ROOT.dataset.fxPreloaderBootPaintR840='content-already-painted-no-visual-reversal';finalizePreloader('painted-before-boot-skip-r840');return null;}
...
ROOT.dataset.fxPreloaderBootPaintR840=CONTENT_PAINTED_BEFORE_BOOT?'content-already-painted-no-visual-reversal':'pre-content-paint-full-intro';
const preloader=CONTENT_PAINTED_BEFORE_BOOT?skipPaintedPreloader():(PRELOADER_BOOT_AT>=PRELOADER_MAX_MS?skipLatePreloader():showPreloader());
