(() => {
  ...
  class FormatXGenesisThree {
    ...
    updateCore(t,time){
      ...
      else if(t<3.30) sc=mix(.86,.78,smooth((t-2.70)/.60));
      else if(t<5.70) sc=.78;
      else if(t<6.55) sc=mix(.78,.58,smooth((t-5.70)/.85));
      else sc=.58;
      ...
      this.coreShell.material.opacity=(.92*stage1 + .70*(1-stage1))*coreFade;
      this.coreGlass.material.opacity=(.16*stage1 + .080*(1-stage1))*coreFade;
      this.coreEdges.material.opacity=(.18*stage1 + .10*(1-stage1))*coreFade;
      if(this.corePetalMaterial)this.corePetalMaterial.opacity=(.12*stage1+.20*(1-stage1))*coreFade;

      this.irisGroup.scale.setScalar((.018+eyeHold*.72)*pulse);
      this.irisRays.rotation.z=time*.000060;
      if(this.irisCorona)this.irisCorona.material.opacity=.78*eyeHold*coreFade;
  ...
  async function attach(canvas,getTarget){
    ...
    try{
      ...
      if(!visualProof && isSoftwareWebGL()){
        document.documentElement.dataset.fxMagBirthR1110='software-webgl-r649-fallback';
        return null;
      }
      loader ||= loadThree();
      ...
      return {
        ...
        engine,
        revision:'r1110-three-stage-reference-timeline'
      };
    }catch(error){
      console.error('FormatX R1110 genesis renderer failed:',error);
      document.documentElement.dataset.fxMagBirthR1110='fallback-r649';
      return null;
    }
  }
  ...
  window.FormatXMagGenesisThreeR1110={
    attach,
    revision:'r1110-three-stage-reference-timeline-dna-cellular-living-architecture'
  };
})();