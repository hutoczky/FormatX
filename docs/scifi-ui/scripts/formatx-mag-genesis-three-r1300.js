(() => {
  ...
  class FormatXGenesisThree {
    ...
    updateCore(t,time){
      ...
      this.coreShell.material.opacity=.96*stage1+.16*reveal*lateDim;
      this.coreGlass.material.opacity=.08*stage1+.015*reveal*lateDim;
      this.coreEdges.material.opacity=.16*stage1+.012*reveal*lateDim;
      if(this.corePetalMaterial)this.corePetalMaterial.opacity=.72*stage1+.065*reveal*lateDim;

      this.irisGroup.scale.setScalar((.018+eyeHold*.72)*pulse);
      this.irisRays.rotation.z=time*.000050;
      if(this.irisCorona)this.irisCorona.material.opacity=.68*eyeHold;
      this.glowSprite.material.opacity=(.001+eyeHold*.006)*pulse;
      this.glowSprite.scale.setScalar(.80+eyeHold*.05);
      this.coreInner.material.opacity=.0015+eyeHold*.018;
      this.coreLight.intensity=eyeHold*(1.9+Math.sin(time*.0046)*.08);
    ...
    updateOrganic(t,time){
      ...
      this.organicVeinMaterial.opacity=.12*visible;
      this.organicHoodMaterial.opacity=.24*visible;
      if(this.organicFoldMaterial)this.organicFoldMaterial.opacity=.24*visible;
      if(this.organicFoldGlowMaterial)this.organicFoldGlowMaterial.opacity=.025*visible;
  ...
  async function attach(canvas,getTarget){
    ...
    try{
      ...
      if(!visualProof && isSoftwareWebGL()){
        document.documentElement.dataset.fxMagBirthR1300='software-webgl-r649-fallback';
        return null;
      }
      loader ||= loadThree();
      ...
      console.error('FormatX R1300 genesis renderer failed:',error);
      document.documentElement.dataset.fxMagBirthR1300='fallback-r649';
      return null;
    }
  }
  ...
  window.FormatXMagGenesisThreeR1300={
    attach,
