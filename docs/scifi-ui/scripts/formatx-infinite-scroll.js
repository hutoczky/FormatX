(function () {
  ...
  let lastMobileFlow = MOBILE_FLOW_QUERY.matches;
  ...
  function syncModeMarkers() {
    const mobile = isMobileFlow();
    root.dataset.fxMobileScrollMode = mobile ? 'native-no-jump' : 'desktop-seamless-loop';
    root.dataset.fxInfiniteCloneMode = mobile ? 'none' : 'visual-bridge';
    root.dataset.fxAutomaticLoop = mobile ? 'disabled' : 'enabled';
    root.dataset.fxScrollJumpGuard = mobile ? 'native-position-v2' : 'visual-match-v3';
    if (mobile) root.classList.add('fx-mobile-native-scroll');
    else root.classList.remove('fx-mobile-native-scroll');
  }
  ...
  function removeBridge() {
    ...
    bridge = null;
    root.dataset.fxLoopBridge = isMobileFlow() ? 'disabled-mobile' : 'missing';
  }
  ...
  function transferIfNeeded() {
    ...
    /* Touch/coarse-pointer scrolling stays 100% native. Programmatic scrollTo
       cancels Android/iOS momentum and is the source of the visible stop. */
    if (isMobileFlow()) return;
  ...
  function applyFlowMode(rebuildDesktopBridge) {
    syncModeMarkers();
    if (isMobileFlow()) {
      cancelAnimationFrame(landingFrame);
      landingFrame = 0;
    ...
    if (rebuildDesktopBridge !== false) buildBridge();
  }
  ...
  function onResize() {
    ...
    const modeChanged = lastMobileFlow !== isMobileFlow();
    if (widthChanged) layoutWidth = nextWidth;
    if (modeChanged) lastMobileFlow = isMobileFlow();
    scheduleRepair(widthChanged || modeChanged);
  }
  ...
  function initialise() {
    ...
    root.__FORMATX_INFINITE_SCROLL__ = Object.freeze({
      version: VERSION,
      mobileFlow: 'native-no-jump',
      desktopFlow: 'visual-bridge',
      automaticLoopOnMobile: false,
      automaticLoopOnDesktop: true,
      clonedContent: false,
      clonedHeroOnlyOnDesktop: true,
  ...
  MOBILE_FLOW_QUERY.addEventListener?.('change', () => {
    lastMobileFlow = isMobileFlow();
    scheduleRepair(true);
  });
