(function () {
  ...
  const TRANSLATIONS = [
    ...
    // Exact source variants used by the current checkout, support and legal pages.
    ...
    ['Az átutalás bejelentése', 'Reporting the transfer'],
    ['Banki tranzakció azonosítója', 'Bank transaction ID'],
    ['Belső rendelési hivatkozás — opcionális', 'Internal order reference — optional'],
    ['Cég vagy vállalkozás neve', 'Company or business name'],
    ['HUF-fizetésnél az RFC 8905 szabvány szerinti', 'For HUF payments, the RFC 8905-compliant'],
    ['Hozzájárulok, hogy a FormatX üzemeltetője az adatokat az átutalás azonosításához és a licenc aktiválásához kezelje.', 'I consent to the FormatX operator processing these data to identify the transfer and activate the licence.'],
    ['Kezelhető gépek száma', 'Number of managed devices'],
    ['Közvetítő bank BIC-kódja (ha szükséges)', 'Intermediary bank BIC (if required)'],
    ['QR-kód, EUR-fizetésnél pedig EPC SEPA átutalási QR-kód készül. A kiválasztott csomaghoz rögzített összeg és egyedi rendelési azonosító tartozik.', 'QR code is generated; for EUR payments, an EPC SEPA transfer QR code is generated. Each selected plan has a fixed amount and a unique order reference.'],
    ['RÖGZÍTETT ÖSSZEG HUF-BAN VAGY EURÓBAN', 'FIXED AMOUNT IN HUF OR EUR'],
    ['Technikusok száma', 'Number of technicians'],
    ['Átutalás bejelentése', 'Report transfer'],
    ['Add meg a kívánt licenctípust, a kezelendő gépek számát és a tervezett felhasználási módot. Licenckulcsot vagy gépazonosítót nyilvános hibajegybe ne írj.', 'Specify the required licence type, number of managed devices and intended use. Never post a licence key or device identifier in a public issue.'],
    ['Beküldés előtt ellenőrizd, hogy a jelenséget már jelentette-e más. A meglévő hibajegyet új reprodukciós adatokkal is kiegészítheted.', 'Before submitting, check whether the issue has already been reported. You can add new reproduction details to an existing issue.'],
    ['Hasonlítsd össze a letöltött ZIP-fájl SHA-256-ellenőrzőösszegét a kiadási oldalon látható ellenőrzőösszeggel.', 'Compare the downloaded ZIP file SHA-256 checksum with the checksum shown on the release page.'],
    ['Licencigénylés indítása', 'Start licence request'],
    ['Licencigénylés', 'Licence request'],
    ['ÜZLETI / TULAJDONOSI', 'BUSINESS / OWNER'],
    ['A FormatX Suite Pro licencei közvetlen HUF vagy EUR banki átutalással vásárolhatók meg. A weboldal nem kezel bankkártyaadatot.', 'FormatX Suite Pro licences can be purchased by direct HUF or EUR bank transfer. The website does not process bank card data.'],
    ['A letöltés sértetlenségét a közzétett SHA-256-ellenőrzőösszeggel kell ellenőrizni.', 'Verify download integrity using the published SHA-256 checksum.'],
    ['A letöltött csomag használatára a csomagban található licencfeltételek érvényesek. A meghajtó- és rendszerkezelési műveletek adatvesztést okozhatnak. Minden művelet előtt ellenőrizd a célmeghajtót, készíts mentést, és csak olyan rendszeren dolgozz, amelyhez megfelelő jogosultságod van.', 'Use of the downloaded package is governed by the licence terms included with it. Drive and system operations can cause data loss. Before every operation, verify the target drive, create a backup and work only on systems for which you have proper authorization.'],
    ['A licenc nem aktiválódik pusztán a vásárlói visszajelzés alapján. Az üzemeltető kézzel ellenőrzi a bankszámlán jóváírt összeget, devizát és közleményt, majd jóváhagyja a rendelést és létrehozza a licenckulcsot.', 'The licence is not activated solely from the buyer’s report. The operator manually verifies the credited amount, currency and payment reference, then approves the order and creates the licence key.'],
    ['A vásárló forintban (HUF) vagy euróban (EUR) fizethet. Mindkét devizához előre rögzített csomagár tartozik; a rendszer nem végez fizetés közbeni élő devizaátváltást.', 'The buyer may pay in Hungarian forints (HUF) or euros (EUR). Each currency has predefined plan prices; the system does not perform live currency conversion during payment.'],
    ['EUR-fizetésnél a QR-kód az EPC SEPA Credit Transfer szabvány szerinti adatokat tartalmazza: kedvezményezett, IBAN, BIC, rögzített EUR-összeg és FormatX-rendelési azonosító. A banki alkalmazás támogatása ebben az esetben is eltérhet.', 'For EUR payments, the QR code contains EPC SEPA Credit Transfer data: beneficiary, IBAN, BIC, fixed EUR amount and FormatX order reference. Banking app support may also vary.'],
    ['A FormatX weboldal nem kér és nem tárol bankkártyaszámot, lejárati adatot vagy CVC-kódot. A fizetés forintban (HUF) vagy euróban (EUR), banki átutalással történik.', 'The FormatX website does not request or store card numbers, expiry data or CVC codes. Payment is made by bank transfer in Hungarian forints (HUF) or euros (EUR).'],
    ['A HUF vagy EUR QR-kód a kedvezményezett nyilvános fizetési adatait, a kiválasztott fix összeget, a devizát és a szerver által kiadott egyedi rendelési azonosítót tartalmazza. EUR-fizetés esetén az EPC SEPA Credit Transfer szabvány szerinti adat készül.', 'The HUF or EUR QR code contains the beneficiary’s public payment details, selected fixed amount, currency and unique server-issued order reference. For EUR payments, EPC SEPA Credit Transfer data are generated.'],
    ['A QR-kép elkészítéséhez a böngésző ezeket a nyilvános fizetési adatokat továbbíthatja a QuickChart QR-szolgáltatásnak. A QR-adat nem tartalmaz bankkártyaadatot, bejelentkezési adatot vagy FormatX adatbázis-hozzáférést. A külső szolgáltatás a kérés teljesítéséhez technikai kapcsolatadatokat, például IP-címet is kezelhet.', 'To generate the QR image, the browser may send these public payment details to the QuickChart QR service. The QR data contain no bank card data, login credentials or FormatX database access. The external service may process technical connection data, such as an IP address, to fulfil the request.'],
    ['A checkout kizárólag a Cloudflare Worker szerveroldali API-ján keresztül indít rendelést. Ha az API nem érhető el vagy az éles értékesítés feltételei nincsenek megfelelően konfigurálva, a folyamat leáll: a böngésző nem készít önálló, követetlen átutalási megbízást.', 'Checkout starts an order only through the Cloudflare Worker server-side API. If the API is unavailable or live-sales requirements are not configured correctly, the process stops: the browser does not create an independent, untracked transfer order.'],
    ['A fizetés visszajelzésekor a rendelési azonosító, a deviza, az összeg, az utaló neve, a vásárló e-mail-címe, a banki tranzakció hivatkozása és az opcionális megjegyzés kerülhet a konfigurált rendelési adatbázisba.', 'When reporting payment, the order reference, currency, amount, payer name, buyer email, bank transaction reference and optional message may be stored in the configured order database.'],
    ['A megadott adatokat a rendelés nyilvántartásához, az átutalás beazonosításához, a licenc aktiválásához, a számlázási egyeztetéshez és az ügyfélszolgálati kapcsolattartáshoz használhatjuk. Rendelési adat csak a konfigurált szerveroldali adatbázisban tárolható.', 'The supplied data may be used to record the order, identify the transfer, activate the licence, reconcile billing and provide customer support. Order data may be stored only in the configured server-side database.'],
    ['A megjelenési mód és a rendeléshez tartozó korlátozott munkamenetadat a böngésző helyi vagy munkamenet-tárhelyén őrződhet meg. A legújabb kiadás megjelenítéséhez az oldal lekérheti a nyilvános GitHub Releases metaadatait. Szerveres rendeléskövetés használatakor a rendelési adatok a konfigurált Supabase adatbázisban tárolhatók.', 'The appearance mode and limited order-session data may be retained in the browser’s local or session storage. To display the latest release, the site may retrieve public GitHub Releases metadata. When server-side order tracking is used, order data may be stored in the configured Supabase database.'],
    ['A rendelés előkészítéséhez a weboldal cég- vagy vállalkozásnevet, kapcsolattartói nevet, e-mail-címet, számlázási címet, opcionális adószámot és belső hivatkozást kérhet.', 'To prepare an order, the website may request a company or business name, contact name, email address, billing address, optional tax number and internal reference.']
  ];
  ...
  function dynamic(value, targetLanguage) {
    const rules = targetLanguage === 'en' ? [
      ...
      [/^Közvetlen GitHub Pages mód aktív\.\s*(.*)$/u, function (match) { return 'Direct GitHub Pages mode is active. ' + exact(match[1].trim(), 'en'); }],
      ...
      [/^Direct GitHub Pages mode is active\.\s*(.*)$/u, function (match) { return 'Közvetlen GitHub Pages mód aktív. ' + exact(match[1].trim(), 'hu'); }],
