(() => {
  ...
  class FormatXGenesisThree {
    ...
    updateCore(t,time){
      ...
      else if(t>=5.65 && t<8.95)sc=mix(.78,.82,smooth((t-5.65)/3.30));
      else if(t>=8.95)sc=.82+smooth((t-8.95)/.72)*.04;
      ...
      const surfaceZ=phase2*.72+finalSurface*.30;
      ...
      this.irisGroup.scale.setScalar(
        (phase1*.72 + phase2Boost*.96 + finalBoost*1.08 + .12)*pulse
      );
      this.irisRays.rotation.z=time*.00010;
      this.glowSprite.material.opacity=(.06+.18*phase1+.56*phase2Boost+.48*finalBoost)*pulse;
      this.glowSprite.scale.setScalar(1.42+phase1*.22+phase2Boost*.42+finalBoost*.36);
      this.coreInner.material.opacity=.18+.20*phase1+.60*phase2Boost+.58*finalBoost;
      this.coreEdges.material.opacity=.22+.18*eye;
      this.coreLight.intensity=eye*(3.5+phase1*2.5+phase2Boost*9+finalBoost*8+Math.sin(time*.005)*1.4);
    ...
    updateMechanical(t,time){
      ...
      this.mechanicalGroup.scale.setScalar(.001+grow*1.16);
      this.mechMaterial.opacity=.97*grow;
      this.mechEdgeMaterial.opacity=.50*grow;
      this.mechInnerMaterial.opacity=.54*grow;
    ...
    render(r,time){
      ...
      this.renderer.toneMappingExposure=1.13+flash*1.18+after*.08;
      this.coreLight.intensity+=flash*19+after*4;
      if(this.glowSprite){
        const g=1+flash*.52;
  ...
  async function attach(canvas,getTarget){
    ...
    try{
      ...
      if(!visualProof && isSoftwareWebGL()){
        document.documentElement.dataset.fxMagBirthR662='software-webgl-r649-fallback';
        return null;
      }
      loader ||= loadThree();
      ...
      return {
        ...
        engine,
        revision:'r662-compact-pod-three-genesis'
      };
    }catch(error){
      console.error('FormatX R662 genesis renderer failed:',error);
      document.documentElement.dataset.fxMagBirthR662='fallback-r649';
      return null;
    }
  }
  ...
  window.FormatXMagGenesisThreeR662={
    attach,
    revision:'r662-compact-pod-three-genesis-dna-cellular-living-architecture'
  };
})();