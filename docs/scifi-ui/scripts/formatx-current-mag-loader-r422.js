/* FormatX R558 — navigation-owned current native MAG runtime.
   R326 remains the only full-size hero WebGL organism. Navigation arms the
   semantic heart and renderer immediately. Only the layout-critical MAG style is
   awaited before the native renderer request; FCP, intro release and user input
   are explicitly not startup owners. Noncritical styles load in parallel. */
(function(){
'use strict';
const root=document.documentElement;
const VERSION='direct-r326-r558-layout-ready-navigation-start-live-energy-zero-idle';
...
const RENDERER='/scifi-ui/scripts/formatx-crystal-organism-r326.js?v=20260906-r558-bounded-native-shader';
...
async function start(){
  ...
  root.dataset.fxMobileHeaderFinalR418=mobile?'loaded-last-mobile':'loaded-cross-device-desktop';root.dataset.fxCurrentMagStylesR423='ready';root.dataset.fxCurrentMagStartupR442='layout-style-before-renderer-noncritical-parallel';root.dataset.fxCurrentMagStartupR549='superseded-r558-no-first-paint-gate';
  root.dataset.fxCurrentMagOpticsR465=mobile?'soft-perimeter-low-bloom-low-cost-shader':'desktop-optics-unchanged';root.dataset.fxCurrentMagOpticsR468=mobile?'soft-bloom-soft-edge-compositor-breathe':'desktop-optics-unchanged';root.dataset.fxCurrentMagOpticsR474=mobile?'softer-glow-feathered-facets-zero-idle':'desktop-optics-unchanged';root.dataset.fxCurrentMagSchedulerR465=mobile?'automatic-lifecycle-zero-idle':'desktop-native-scheduler';root.dataset.fxCoreMobileIdlePolicyR426=mobile?'periodic-surface-bursts-between-zero-idle':'desktop-native-scheduler';root.dataset.fxMiniMagBootstrapR459='requested-alongside-primary-mag';void addScript(MINI_ASSISTANT,'data-fx-mini-mag-assistant-script-r459');
  const rendererReady=await rendererStart;
  if(rendererReady){if(mobile)await addScript(GOVERNOR,'data-fx-mobile-render-governor-r426');await addScript(LIFE,'data-fx-core-life-r455');root.dataset.fxCoreRendererSelection=mobile?'r326-direct-r558-bounded-shader-live-energy-zero-idle':'r326-direct-r558-desktop-live-energy';root.dataset.fxCoreReferenceLockLoad='ready-v69-r558';root.dataset.fxCurrentMagRuntimeR422='ready';}else root.dataset.fxCurrentMagRuntimeR422='renderer-timeout';
  root.dataset.fxCoreCriticalPathR422=mobile?'direct-r326-r558-bounded-native-shader-zero-idle-native-touch':'direct-r326-r558-primary-controller-desktop';root.dataset.fxCurrentMagLifecycleR536='navigation-owned-automatic-lifecycle';
  dispatchEvent(new CustomEvent('formatx:currentmagready',{detail:{version:VERSION,mobile,rendererReady,miniMag:true,legacyCleanup:true,energySweep:true,optics:'r558',heart:'r549-pointer-transparent-physical-router',startup:'r558-layout-ready-no-fcp-gate'}}));
}
