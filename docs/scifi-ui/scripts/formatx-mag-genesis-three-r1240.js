(() => {
  ...
  class FormatXGenesisThree {
    ...
    makeOrganicLayer(){
      ...
      this.organicFoldGlowMaterial=new T.MeshBasicMaterial({
        color:0x355c63,transparent:true,opacity:0,
        depthWrite:false,blending:T.AdditiveBlending
      });
      ...
      const hoodDefs=[
        [-.24,.58,1.07,.58,.82,.68,-.16,.14],
        [.24,.58,1.07,.58,.82,.68,.16,-.14],
        [0,.82,1.01,.34,.50,.58,0,0]
      ];
    ...
    updateCore(t,time){
      let sc=.001;
      if(t>=2.52 && t<3.28) sc=.06+ease((t-2.52)/.76)*.37;
      else if(t>=3.28 && t<5.58) sc=.43+Math.sin((t-3.28)*1.02)*.002;
      else if(t>=5.58 && t<7.15) sc=mix(.43,.36,smooth((t-5.58)/1.57));
      else if(t>=7.15) sc=.36;
      ...
      this.coreShell.material.opacity=.14*reveal*preFlash;
      this.coreGlass.material.opacity=.014*reveal*preFlash;
      this.coreEdges.material.opacity=.012*reveal*preFlash;
      if(this.corePetalMaterial)this.corePetalMaterial.opacity=.28*reveal*preFlash;

      this.irisGroup.scale.setScalar((.024+eyeHold*2.18)*pulse);
      this.irisRays.rotation.z=time*.000050;
      if(this.irisCorona)this.irisCorona.material.opacity=.98*eyeHold;
      this.glowSprite.material.opacity=(.001+eyeHold*.010)*pulse;
      this.glowSprite.scale.setScalar(.86+eyeHold*.06);
      this.coreInner.material.opacity=.0015+eyeHold*.018;
      this.coreLight.intensity=eyeHold*(2.8+Math.sin(time*.0046)*.12);

      if(this.coreLabel)this.coreLabel.material.opacity=.035*reveal*(1-smooth((t-3.08)/.32));
    }
    ...
    updateOrganic(t,time){
      ...
      this.organicShellMaterial.opacity=.78*visible;
      this.organicLobeMaterial.opacity=.10*visible;
      this.organicWireMaterial.opacity=.0003*visible;
      this.organicVeinMaterial.opacity=.26*visible;
      this.organicHoodMaterial.opacity=.90*visible;
      if(this.organicFoldMaterial)this.organicFoldMaterial.opacity=.98*visible;
      if(this.organicFoldGlowMaterial)this.organicFoldGlowMaterial.opacity=.055*visible;
    ...
    updateCamera(t,time){
      ...
      if(t<2.70){
        ...
        z=mix(5.28,5.38,smooth((t-9.10)/.60));
        y=.004;
      }
  ...
  async function attach(canvas,getTarget){
    ...
    try{
      ...
      if(!visualProof && isSoftwareWebGL()){
        document.documentElement.dataset.fxMagBirthR1240='software-webgl-r649-fallback';
        return null;
      }
      loader ||= loadThree();
      ...
      console.error('FormatX R1240 genesis renderer failed:',error);
      document.documentElement.dataset.fxMagBirthR1240='fallback-r649';
      return null;
    }
  }
  ...
  window.FormatXMagGenesisThreeR1240={
    attach,
