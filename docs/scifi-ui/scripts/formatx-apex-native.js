(function () {
  ...
  const state = {
    ...
    targetPointerY: 0,
    lastScrollY: scrollY,
    quality: coarse.matches ? 0.56 : 0.84,
    targetQuality: coarse.matches ? 0.56 : 0.84,
    fps: 0,
    sound: false
  };
  ...
  function installSurface() {
    ...
    hud.innerHTML = [
      ...
      '<div><small>FORMATX / NATIVE APEX</small><strong data-fx-apex-title>MAG</strong><p data-fx-apex-copy>PROCEDURAL GPU SYSTEM</p></div>',
      '</div>',
      '<div class="fx-transcend-progress" aria-hidden="true"><i></i><b></b></div>',
      '<div class="fx-transcend-telemetry" aria-hidden="true"><span>GPU</span><b data-fx-apex-mode>WEBGL2 / ADAPTIVE</b><span>FPS</span><b data-fx-apex-fps>—</b></div>'
    ].join('');
  ...
  function createRenderer(canvas, mode, fpsOutput) {
    ...
    const fragmentSource = `#version 300 es
      ...
      vec2 core(vec3 p){
        p.xz*=rot(uTime*.105+uScroll*2.2);p.xy*=rot(-uTime*.055+uPointer.y*.1);
        float pulse=1.+sin(uTime*1.7)*.022;
        float shell=octa(p,1.42*pulse)+(fbm(p*3.1)-.5)*.07;
        shell=smin(shell,roundBox(p,vec3(.66,.82,.66),.1),.18);
        vec2 result=vec2(shell,1.);
        float nucleus=sphere(p,.34+sin(uTime*2.15)*.025);
        if(nucleus<result.x)result=vec2(nucleus,7.);
        return result;
      }
      vec2 nerves(vec3 p){
        vec2 result=core(p);
      ...
      vec2 mapScene(vec3 p){
        float chapter=clamp(uScene,0.,5.);float base=floor(chapter+.0001),local=fract(chapter);float t=smoothstep(.12,.88,local);
        vec3 q=p;q.xz*=rot((t-.5)*.2*q.y+uVelocity*.012);
        vec2 a=shapeAt(q,base),b=shapeAt(q,min(base+1.,5.));vec2 result=mix(a,b,t);
        result.x+=(fbm(p*2.5+vec3(0,uTime*.06,0))-.5)*.11*sin(t*3.14159);
        float floorD=p.y+2.05+(noise3(vec3(p.xz*.7,uTime*.02))-.5)*.09;
        if(floorD<result.x)result=vec2(floorD,10.);
        return result;
      }
      vec3 normalAt(vec3 p){vec2 e=vec2(.002,0);return normalize(vec3(mapScene(p+e.xyy).x-mapScene(p-e.xyy).x,mapScene(p+e.yxy).x-mapScene(p-e.yxy).x,mapScene(p+e.yyx).x-mapScene(p-e.yyx).x));}
      ...
      vec3 background(vec3 rd){
        float horizon=pow(sat(1.-abs(rd.y)),4.);float aurora=pow(sat(sin(rd.x*5.2+uTime*.09+uScroll*4.)*.5+.5),9.)*pow(sat(rd.y+.35),2.);
        float stars=step(.9975,hash21(floor((rd.xy+1.)*vec2(480.,290.))));
        vec3 c=mix(vec3(.002,.006,.012),vec3(.014,.045,.074),horizon);c+=aurora*mix(vec3(.01,.25,.28),vec3(.28,.07,.38),sat(uScroll))*.45;c+=stars*vec3(.5,.72,1.);return c;
      }
      vec3 material(float id,float fresnel,float diffuse,float pulse){
        vec3 cyan=vec3(.2,.72,.92),violet=vec3(.53,.32,.96),white=vec3(.86,.97,1.);vec3 c=mix(cyan,violet,sat(uScroll*.9));c=mix(c,white,diffuse*.52+fresnel*.44);
        if(id>6.5&&id<7.5)c=mix(c,white,.78);if(id>7.5&&id<8.5)c=mix(vec3(1.,.22,.05),vec3(1.,.72,.18),pulse);if(id>8.5&&id<9.5)c=vec3(.34,.87,1.)*(1.4+pulse);if(id>9.5)c=mix(vec3(.012,.022,.034),vec3(.075,.15,.19),diffuse);return c;
      }
      void main(){
        vec2 uv=(gl_FragCoord.xy*2.-uResolution)/uResolution.y;
        float angle=-.62+uScroll*5.7+sin(uTime*.08)*.035;float radius=5.0-.38*sin(uScroll*3.14159)-.34*smoothstep(4.2,5.,uScene);
        vec3 ro=vec3(sin(angle)*radius,.15+sin(uScroll*TAU)*.38+uPointer.y*.2,cos(angle)*radius);vec3 target=vec3(uPointer.x*.13,mix(.02,.28,smoothstep(4.,5.,uScene)),0.);vec3 rd=camera(ro,target)*normalize(vec3(uv,1.72));
        float distance=0.,id=0.,glow=0.;vec3 p=ro;
        for(int i=0;i<MAX_STEPS;i++){if(float(i)>mix(42.,76.,uQuality))break;p=ro+rd*distance;vec2 hit=mapScene(p);id=hit.y;glow+=exp(-15.*abs(hit.x))*.0028;if(abs(hit.x)<mix(.0035,.0015,uQuality)||distance>FAR)break;distance+=hit.x*.72;}
        vec3 c=background(rd);
        if(distance<FAR){vec3 n=normalAt(p),key=normalize(vec3(-.55,.78,.42));float d=max(dot(n,key),0.),f=pow(1.-max(dot(n,-rd),0.),3.);float pulse=.5+.5*sin(uTime*2.2);vec3 base=material(id,f,d,pulse);vec3 refl=background(reflect(rd,n));float crystal=sat((fbm(p*5.2)-.38)*1.7);c=mix(base,refl,.22+f*.35);c+=crystal*vec3(.15,.34,.48);c*=.28+d*.82;c+=f*vec3(.24,.62,.82)*.48;}
        c+=glow*mix(vec3(.18,.65,1.),vec3(.7,.28,1.),uScroll);float aberration=(uv.x*uPointer.x+uv.y*uPointer.y)*.012+uVelocity*.002;c.r*=1.+aberration;c.b*=1.-aberration*.7;float vignette=1.-smoothstep(.32,1.15,length(uv));c*=.72+.28*vignette;c=c/(1.+c);c=pow(c,vec3(.86,.9,.96));c+=(hash21(gl_FragCoord.xy+fract(uTime)*91.)-.5)/255.;outColor=vec4(c,1.);
      }
    `;
    ...
    function resize() {
      const dpr = Math.min(devicePixelRatio || 1, coarse.matches ? 1.05 : 1.45);
      const scale = clamp(state.quality, .48, 1);
    ...
    function adapt(now) {
      ...
      frames = 0;
      sampleStarted = now;
      const min = coarse.matches ? .48 : .56;
      const max = coarse.matches ? .72 : 1;
      if (fps < 45) state.targetQuality = clamp(state.targetQuality - .07, min, max);
      else if (fps > 57) state.targetQuality = clamp(state.targetQuality + .035, min, max);
      if (Math.abs(state.targetQuality - state.quality) > .045) {
    ...
    function render(now) {
      ...
      const targetFps = coarse.matches ? 30 : 60;
    ...
    mode.textContent = 'WEBGL2 / NATIVE SDF';
  ...
  function createFallback(canvas, mode) {
    ...
    let height = 1;
    let last = 0;
    const particles = Array.from({ length: coarse.matches ? 90 : 180 }, () => ({ a: Math.random()*Math.PI*2, r: .12+Math.random()*.43, z: Math.random(), s: .3+Math.random()*1.1 }));
    function resize(){width=canvas.width=Math.max(1,Math.floor(innerWidth*Math.min(devicePixelRatio||1,1.1)));height=canvas.height=Math.max(1,Math.floor(innerHeight*Math.min(devicePixelRatio||1,1.1)));canvas.style.width=innerWidth+'px';canvas.style.height=innerHeight+'px';}
    function draw(now){raf=requestAnimationFrame(draw);if(!state.visible||now-last<33)return;last=now;context.fillStyle='#02060b';context.fillRect(0,0,width,height);const cx=width*(.62+state.pointerX*.02),cy=height*(.48-state.pointerY*.02),scale=Math.min(width,height)*(.2+state.smoothScroll*.03);particles.forEach((p,i)=>{p.a+=(state.reduced?0:.0014+p.z*.0018);const r=scale*(p.r+Math.sin(now*.0005+i)*.02);const x=cx+Math.cos(p.a+state.smoothScene*.65)*r,y=cy+Math.sin(p.a*1.3+state.smoothScene*.4)*r*.7;context.fillStyle='rgba(150,225,255,'+(.11+p.z*.43)+')';context.beginPath();context.arc(x,y,p.s+p.z,0,Math.PI*2);context.fill();});}
  ...
  root.dataset.fxNativeApexRenderer=surface.canvas.getContext('webgl2')?'webgl2-native-sdf':'canvas2d-safe';
  ...
  updateScroll();
  animateState();
  dispatchEvent(new CustomEvent('formatx:nativeapexready',{detail:{renderer:root.dataset.fxNativeApexRenderer,scrollOwnership:'seamless-v7'}}));
