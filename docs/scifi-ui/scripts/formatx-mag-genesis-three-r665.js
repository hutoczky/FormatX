(() => {
  ...
  class FormatXGenesisThree {
    ...
    makeOrganicLayer(){
      ...
      this.organicHoodMaterial=new T.MeshPhysicalMaterial({
        color:0x334150,roughness:.62,metalness:.08,
        clearcoat:.22,clearcoatRoughness:.42,
        transparent:true,opacity:0,
        emissive:0x112835,emissiveIntensity:.18
      });
      this.organicHoodGroup=new T.Group();
      const hoodGeo=new T.IcosahedronGeometry(.36,2);
      const hoodDefs=[
        [-.48,.63,1.00,.80,1.08,.46,-.22],
        [-.24,.78,1.06,.86,1.18,.48,-.10],
        [0,.84,1.10,.92,1.22,.50,0],
        [.24,.78,1.06,.86,1.18,.48,.10],
        [.48,.63,1.00,.80,1.08,.46,.22]
      ];
    ...
    makeMechanicalLayer(){
      ...
      this.mechMaterial=new T.MeshPhysicalMaterial({
        color:0x102837,metalness:.91,roughness:.18,
        emissive:0x0b2f40,emissiveIntensity:.58,
        clearcoat:.75,clearcoatRoughness:.12,
        transparent:true,opacity:0
      });
      this.mechEdgeMaterial=new T.LineBasicMaterial({
        color:0xa9f8ff,transparent:true,opacity:0,
        blending:T.AdditiveBlending,depthWrite:false
      });
      ...
      const defs=[
        [0,.48,.16,0,-.42,0,1.00],
        [.48,0,.12,-Math.PI/2,0,.42,.86],
        [0,-.48,.16,Math.PI,.42,0,1.00],
        [-.48,0,.12,Math.PI/2,0,-.42,.86]
      ];
      for(const [x,y,z,rz,rx,ry,sc] of defs){
        const g=new T.Group();
      ...
      for(let i=0;i<4;i++){
        ...
        const jitter=i%2?.05:-.04;
        m.position.set(Math.cos(a)*(.66+jitter),Math.sin(a)*(.66-jitter),-.03+(i%2?.025:-.015));
        m.rotation.z=a+(i%2?.06:-.04);
      ...
      this.seams=[];
      const seamGeo=new T.BoxGeometry(.28,.032,.030);
      const seamDefs=[
        [-.62,.06,.38,0],
        [.62,.06,.38,0],
        [0,.66,.36,Math.PI/2],
    ...
    updateCore(t,time){
      ...
      else if(t>=5.65 && t<8.95)sc=mix(.60,.94,smooth((t-5.65)/3.30));
      else if(t>=8.95)sc=.94+smooth((t-8.95)/.72)*.04;
      ...
      this.glowSprite.material.opacity=(.025+.025*phase1+.58*phase2Boost+.54*finalBoost)*pulse;
      ...
      this.coreLight.intensity=eye*(1.2+phase1*.45+phase2Boost*9.5+finalBoost*10.5+Math.sin(time*.005)*1.1);
    ...
    updateCells(t,time){
      ...
      this.cellVeinMaterial.opacity=.38*visible;
    ...
    updateMechanical(t,time){
      ...
      this.mechanicalGroup.scale.setScalar(.001+grow*1.40);
      this.mechMaterial.opacity=.99*grow;
      this.mechEdgeMaterial.opacity=.68*grow;
      this.mechInnerMaterial.opacity=.66*grow;
      if(this.seamMaterial)this.seamMaterial.opacity=.86*grow;
      this.mechInnerRing.rotation.z=time*.00018;
      if(this.mechLight)this.mechLight.intensity=8.5*grow;
      this.plates.forEach((p,i)=>{
        const breathe=1+Math.sin(time*.00088+i*.9)*.007*grow;
        const base=p.userData.baseScale||1;
        p.scale.setScalar(base*breathe);
      });
      this.sidePlates.forEach((p,i)=>{
        p.rotation.y=Math.sin(time*.00030+i)*.030*grow;
      });
    }
  ...
  async function attach(canvas,getTarget){
    ...
    try{
      ...
      if(!visualProof && isSoftwareWebGL()){
        document.documentElement.dataset.fxMagBirthR665='software-webgl-r649-fallback';
        return null;
      }
      loader ||= loadThree();
      ...
      return {
        ...
        engine,
        revision:'r665-organic-hood-three-genesis'
      };
    }catch(error){
      console.error('FormatX R665 genesis renderer failed:',error);
      document.documentElement.dataset.fxMagBirthR665='fallback-r649';
      return null;
    }
  }
  ...
  window.FormatXMagGenesisThreeR665={
    attach,
    revision:'r665-organic-hood-three-genesis-dna-cellular-living-architecture'
  };
})();