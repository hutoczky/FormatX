(() => {
  ...
  class FormatXGenesisThree {
    ...
    makeDebris(){
      ...
      const mat=new T.MeshStandardMaterial({
        color:0x6f7190,roughness:.72,metalness:.03,
        emissive:0x142d3d,emissiveIntensity:.26,
      ...
      for(let i=0;i<count;i++){
        ...
        const rr=1.55+r()*3.8;
        const sc=.42+r()*1.85;
        dummy.position.set(Math.cos(a)*rr,(r()-.5)*4.2,-1.1+(r()-.5)*3.0);
    ...
    makeCore(){
      ...
      this.corePetalMaterial=new T.MeshPhysicalMaterial({
        color:0x0a1720,metalness:.52,roughness:.24,
        emissive:0x082732,emissiveIntensity:.22,
        clearcoat:.72,clearcoatRoughness:.16,
      ...
      petalShape.bezierCurveTo(.16,.08,.40,.50,0,1.10);
      petalShape.bezierCurveTo(-.40,.50,-.16,.08,0,-.10);
      const petalGeo=new T.ExtrudeGeometry(petalShape,{
        depth:.13,bevelEnabled:true,bevelSegments:3,steps:1,
        bevelSize:.030,bevelThickness:.038,curveSegments:18
      });
      petalGeo.center();
      const petalDefs=[
        [0,.20,.16,0,1.00,.92],
        [.20,0,.15,-Math.PI/2,.92,1.00],
        [0,-.20,.15,Math.PI,.98,.92],
        [-.20,0,.15,Math.PI/2,.92,1.00]
      ];
      for(const [x,y,z,rz,sx,sy] of petalDefs){
        ...
        p.rotation.z=rz;
        p.scale.set(sx,sy,.84);
        this.coreGroup.add(p);
        this.corePetals.push(p);
      }
      ...
      const diamondMat=new T.MeshBasicMaterial({
        color:0x4edcf5,transparent:true,opacity:.025,
      ...
      const diamondEdge=new T.LineSegments(
        ...
        new T.LineBasicMaterial({
          color:0x67dff2,transparent:true,opacity:.10,
          depthWrite:false,blending:T.AdditiveBlending
        })
      );
      ...
      const ring1=new T.Mesh(new T.TorusGeometry(.232,.017,10,72),cyan.clone());
      ring1.material.opacity=.42;
      const ring2=new T.Mesh(new T.TorusGeometry(.340,.008,8,72),cyan.clone());
      ...
      this.irisCenterDot=new T.Mesh(
        new T.CircleGeometry(.042,48),
        new T.MeshBasicMaterial({
          color:0x74ecff,transparent:true,opacity:.72,
          blending:T.AdditiveBlending,depthWrite:false,side:T.DoubleSide
        })
      );
      this.irisCenterDot.position.z=.018;
      this.irisGroup.add(this.irisCenterDot);
      ...
      for(let i=0;i<64;i++){
        ...
        const inner=.152+(i%3)*.006;
        const outer=.382+(i%7)*.012;
        rays.push(
      ...
      this.irisCorona.scale.set(1.00,1.00,1);
    ...
    makeOrganicLayer(){
      ...
      this.organicFoldMaterial=new T.MeshPhysicalMaterial({
        color:0x462b5b,roughness:.70,metalness:.01,
        emissive:0x34184a,emissiveIntensity:.36,
      ...
      for(let i=0;i<34;i++){
        ...
        const fold=new T.Mesh(new T.TubeGeometry(curve,34,.041,7,false),this.organicFoldMaterial);
      ...
      const hoodDefs=[
        [0,.68,.94,.96,1.04,.82,0,0],
        [-.34,.50,.91,.76,.88,.74,-.34,.16],
        [.34,.50,.91,.76,.88,.74,.34,-.16]
      ];
    ...
    makeTentacles(){
      ...
      for(let i=0;i<count;i++){
        ...
        const curve=new T.CatmullRomCurve3(pts,false,'centripetal');
        const geo=this.createTaperedTube(curve,62,8,.086+r()*.016,.010+r()*.003);
        const glowGeo=this.createTaperedTube(curve,62,6,.014+r()*.003,.0035+r()*.001);
    ...
    updateCore(t,time){
      let sc=.001;
      if(t<2.55)sc=.44+ease(t/2.55)*.14;
      else if(t<3.35)sc=mix(.58,.86,smooth((t-2.55)/.80));
      else if(t<5.65)sc=.86+Math.sin((t-3.35)*1.02)*.003;
      else if(t<8.95)sc=mix(.86,.49,smooth((t-5.65)/3.30));
      else sc=.47;
      ...
      const surfaceZ=cellSurface*1.12;
      ...
      this.coreShell.material.opacity=early*(.28+.08*eyeHold);
      this.coreGlass.material.opacity=early*(.028+.050*eyeHold);
      this.coreEdges.material.opacity=early*(.035+.055*eyeHold);
      if(this.corePetalMaterial)this.corePetalMaterial.opacity=early*(.82+.10*eyeHold);

      this.irisGroup.scale.setScalar((.052+eyeHold*.94)*pulse);
      this.irisRays.rotation.z=time*.000070;
      this.glowSprite.material.opacity=(.003+eyeHold*.15)*pulse;
      this.glowSprite.scale.setScalar(1.04+eyeHold*.22);
      this.coreInner.material.opacity=.005+eyeHold*.16;
      this.coreLight.intensity=eyeHold*(2.8+Math.sin(time*.005)*.26);
      if(this.irisCenterDot)this.irisCenterDot.material.opacity=.48+.22*eyeHold;

      if(this.coreLabel)this.coreLabel.material.opacity=.18*early*(1-smooth((t-2.62)/.32));
    }
    ...
    updateOrganic(t,time){
      ...
      this.organicLobeMaterial.opacity=.42*visible;
      this.organicWireMaterial.opacity=.001*visible;
      this.organicVeinMaterial.opacity=.74*visible;
      this.organicHoodMaterial.opacity=.48*visible;
      if(this.organicFoldMaterial)this.organicFoldMaterial.opacity=.98*visible;
      if(this.organicFoldGlowMaterial)this.organicFoldGlowMaterial.opacity=.30*visible;
    ...
    updateTentacles(t,time){
      ...
      if(this.tentacleGlowMaterial)this.tentacleGlowMaterial.opacity=.52*grow;
  ...
  async function attach(canvas,getTarget){
    ...
    try{
      ...
      if(!visualProof && isSoftwareWebGL()){
        document.documentElement.dataset.fxMagBirthR701='software-webgl-r649-fallback';
        return null;
      }
      loader ||= loadThree();
      ...
      return {
        ...
        engine,
        revision:'r701-organic-petal-core-three-genesis'
      };
    }catch(error){
      console.error('FormatX R701 genesis renderer failed:',error);
      document.documentElement.dataset.fxMagBirthR701='fallback-r649';
      return null;
    }
  }
  ...
  window.FormatXMagGenesisThreeR701={
    attach,
    revision:'r701-organic-petal-core-three-genesis-dna-cellular-living-architecture'
  };
})();