...
function apply(){
 ...
 if(bar instanceof HTMLElement){
  ...
  for(const el of [mag,menu]){if(!(el instanceof HTMLElement))continue;normal(el);imp(el,'margin','0');imp(el,'flex','0 0 auto');imp(el,'visibility',mobile?'visible':'hidden');imp(el,'opacity',mobile?'1':'0');imp(el,'display',mobile?'inline-flex':'none');imp(el,'pointer-events',mobile?'auto':'none');}
 }
 ...
 if(mobile){
  if(rail instanceof HTMLElement){imp(rail,'position','absolute');imp(rail,'inset','auto');imp(rail,'top','18px');imp(rail,'right','6.2%');imp(rail,'left','auto');imp(rail,'bottom','auto');imp(rail,'z-index','20');imp(rail,'display','flex');imp(rail,'flex-direction','column');imp(rail,'align-items','center');imp(rail,'justify-content','flex-start');imp(rail,'gap','13px');imp(rail,'width','auto');imp(rail,'height','auto');imp(rail,'margin','0');imp(rail,'transform','none');}
 ...
 root.dataset.fxReferenceComposition='desktop-native-content-r81';root.dataset.fxFlowFirstR75='ready-r81';return true;
}
