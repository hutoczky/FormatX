...
function apply(){
 ...
 normal(copy);imp(copy,'order','2');imp(copy,'align-self','center');imp(copy,'display','grid');imp(copy,'grid-template-columns','minmax(0,1fr)');imp(copy,'grid-template-rows','none');imp(copy,'align-content','start');imp(copy,'row-gap',mobile?'12px':'16px');imp(copy,'width',mobile?'calc(100% - 28px)':'min(960px,calc(100% - 40px))');imp(copy,'max-width',mobile?'none':'960px');imp(copy,'height','auto');imp(copy,'min-height',mobile?'0':'260px');imp(copy,'margin','0 auto 48px');imp(copy,'padding','0');imp(copy,'overflow','visible');imp(copy,'clip','auto');imp(copy,'clip-path','none');imp(copy,'white-space','normal');imp(copy,'opacity','1');imp(copy,'visibility','visible');
 ...
 if(!mobile){
  imp(kicker,'min-height','16px');imp(kicker,'font-size','12px');imp(kicker,'line-height','1.35');
  imp(category,'width','min(620px,100%)');imp(category,'min-height','48px');imp(category,'font-size','14px');imp(category,'line-height','1.55');
  imp(title,'min-height','94px');imp(title,'margin','8px 0 4px');
  const main=title?.querySelector('.hero-title-main'),sub=title?.querySelector('.hero-title-sub');normal(main);normal(sub);imp(main,'display','block');imp(main,'height','auto');imp(main,'min-height','58px');imp(main,'font-size','clamp(64px,6.4vw,104px)');imp(main,'line-height','.9');imp(sub,'display','block');imp(sub,'height','auto');imp(sub,'min-height','26px');imp(sub,'margin-top','8px');imp(sub,'font-size','clamp(25px,2.5vw,38px)');imp(sub,'line-height','1');
  imp(lead,'width','min(620px,100%)');imp(lead,'min-height','48px');imp(lead,'font-size','17px');imp(lead,'line-height','1.55');
 }
 if(method instanceof HTMLElement){imp(method,'display','flex');imp(method,'flex-wrap','wrap');imp(method,'gap','8px 18px');imp(method,'width','100%');imp(method,'height','auto');imp(method,'min-height',mobile?'0':'24px');}
 if(actions instanceof HTMLElement){imp(actions,'display',mobile?'grid':'flex');if(mobile)imp(actions,'grid-template-columns','1fr');else imp(actions,'flex-wrap','wrap');imp(actions,'gap','10px');imp(actions,'width','100%');imp(actions,'height','auto');imp(actions,'min-height','1px');}
 if(facts instanceof HTMLElement){imp(facts,'display',mobile?'grid':'flex');if(mobile){imp(facts,'grid-template-columns','repeat(3,minmax(0,1fr))');imp(facts,'gap','8px');}else{imp(facts,'flex-wrap','wrap');imp(facts,'gap','10px 20px');}imp(facts,'width','100%');imp(facts,'height','auto');imp(facts,'min-height','1px');}
