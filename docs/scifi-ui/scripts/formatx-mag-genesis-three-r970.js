(() => {
  ...
  class FormatXGenesisThree {
    ...
    makeCore(){
      ...
      const shellMat=new T.MeshPhysicalMaterial({
        color:0x304651,metalness:.72,roughness:.22,
        emissive:0x0c202a,emissiveIntensity:.18,
        clearcoat:.82,clearcoatRoughness:.12,
        transparent:true,opacity:.96
      });
      const shellGlass=new T.MeshPhysicalMaterial({
        color:0x5b7d89,metalness:.20,roughness:.12,
        transparent:true,opacity:.16,depthWrite:false,
        emissive:0x123f4e,emissiveIntensity:.14,
    ...
    updateCore(t,time){
      ...
      this.coreShell.material.opacity=.80*stage1 + .32*(1-stage1);
      ...
      this.coreLight.intensity=eyeHold*(2.5+Math.sin(time*.0046)*.20);

      if(this.coreLabel)this.coreLabel.material.opacity=.22*stage1;
    ...
    updateTentacles(t,time){
      ...
      this.tentacleNodeMaterial.opacity=.62*grow;
      if(this.tentacleGlowMaterial)this.tentacleGlowMaterial.opacity=.50*grow;
  ...
  async function attach(canvas,getTarget){
    ...
    try{
      ...
      if(!visualProof && isSoftwareWebGL()){
        document.documentElement.dataset.fxMagBirthR970='software-webgl-r649-fallback';
        return null;
      }
      loader ||= loadThree();
      ...
      return {
        ...
        engine,
        revision:'r970-video-locked-organic-three-genesis'
      };
    }catch(error){
      console.error('FormatX R970 genesis renderer failed:',error);
      document.documentElement.dataset.fxMagBirthR970='fallback-r649';
      return null;
    }
  }
  ...
  window.FormatXMagGenesisThreeR970={
    attach,
    revision:'r970-video-locked-organic-three-genesis-dna-cellular-living-architecture'
  };
})();