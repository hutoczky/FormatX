(() => {
  ...
  class FormatXGenesisThree {
    ...
    makeDebris(){
      const T=this.THREE,r=this.rand;
      const geo=new T.IcosahedronGeometry(.055,0);
      const mat=new T.MeshStandardMaterial({
        color:0x73769a,roughness:.68,metalness:.05,
        emissive:0x173348,emissiveIntensity:.30,
      ...
      for(let i=0;i<count;i++){
        ...
        const z=-1.2+(r()-.5)*3.2;
        const sc=.45+r()*2.0;
        dummy.position.set(Math.cos(a)*rr,y,z);
        dummy.rotation.set(r()*3,r()*3,r()*3);
        dummy.scale.set(sc*(.55+r()*.75),sc,sc*(.48+r()*.70));
        dummy.updateMatrix();
    ...
    makeCore(){
      ...
      const ring1=new T.Mesh(new T.TorusGeometry(.285,.018,10,80),cyan.clone());
      ring1.material.opacity=.42;
      const ring2=new T.Mesh(new T.TorusGeometry(.410,.008,8,80),cyan.clone());
      ring2.material.opacity=.17;
      const pupil=new T.Mesh(new T.CircleGeometry(.105,64),new T.MeshBasicMaterial({color:0x01070b,side:T.DoubleSide}));
      const pupilRing=new T.Mesh(new T.RingGeometry(.112,.165,72),cyan.clone());
      pupilRing.material.opacity=.44;
      ...
      for(let i=0;i<64;i++){
        ...
        const inner=.168+(i%3)*.006;
        const outer=.485+(i%7)*.013;
        rays.push(
    ...
    makeOrganicLayer(){
      ...
      this.organicFoldMaterial=new T.MeshPhysicalMaterial({
        color:0x412655,roughness:.69,metalness:.01,
        emissive:0x321748,emissiveIntensity:.38,
      ...
      for(let i=0;i<34;i++){
        ...
        const fold=new T.Mesh(new T.TubeGeometry(curve,34,.043,7,false),this.organicFoldMaterial);
        const glow=new T.Mesh(new T.TubeGeometry(curve,34,.008,5,false),this.organicFoldGlowMaterial);
      ...
      // Retain the historical hood fields expected by update paths, but make
      // the hood a subtle integrated crown rather than five protruding fingers.
      this.organicHoodMaterial=new T.MeshPhysicalMaterial({
        color:0x697987,roughness:.30,metalness:.58,
        clearcoat:.48,clearcoatRoughness:.24,
        transparent:true,opacity:0,
        emissive:0x102936,emissiveIntensity:.12
      });
      this.organicHoodGroup=new T.Group();
      const hoodShape=new T.Shape();
      hoodShape.moveTo(0,.82);
      hoodShape.bezierCurveTo(.12,.62,.34,.24,.44,-.10);
      hoodShape.bezierCurveTo(.22,-.05,.09,.03,0,.18);
      hoodShape.bezierCurveTo(-.09,.03,-.22,-.05,-.44,-.10);
      hoodShape.bezierCurveTo(-.34,.24,-.12,.62,0,.82);
      const hoodGeo=new T.ExtrudeGeometry(hoodShape,{
        depth:.14,bevelEnabled:true,bevelSegments:3,steps:1,
        bevelSize:.032,bevelThickness:.040,curveSegments:14
      });
      hoodGeo.center();
      const hoodDefs=[
        [0,.69,1.00,.78,.88,.74,0,0],
        [-.30,.52,.96,.62,.72,.66,-.30,.14],
        [.30,.52,.96,.62,.72,.66,.30,-.14]
      ];
    ...
    makeTentacles(){
      ...
      this.tentacleMaterial=new T.MeshStandardMaterial({
        color:0x050e14,metalness:.30,roughness:.42,
      ...
      for(let i=0;i<count;i++){
        ...
        const sign=i%2?1:-1;
        const len=1.72+r()*1.00;
        const phase=r()*Math.PI*2;
        const pts=[];
        for(let j=0;j<9;j++){
          const u=j/8;
          const radius=1.08+len*u;
          const curl=sign*Math.sin(u*Math.PI*2.05+phase*.22)*(.08+.22*u);
          const a=base+curl;
          const sideWave=Math.sin(u*Math.PI*3.0+phase)*(.045+.11*u);
          pts.push(new T.Vector3(
            Math.cos(a)*radius-Math.sin(a)*sideWave,
            Math.sin(a)*radius+Math.cos(a)*sideWave,
            Math.sin(u*Math.PI*2.15+phase)*(.07+.20*u)
          ));
        }
        const curve=new T.CatmullRomCurve3(pts,false,'centripetal');
        const geo=this.createTaperedTube(curve,60,8,.086+r()*.016,.010+r()*.003);
        const glowGeo=this.createTaperedTube(curve,60,6,.014+r()*.003,.0035+r()*.001);
    ...
    updateCore(t,time){
      let sc=.001;
      if(t<2.55)sc=.44+ease(t/2.55)*.12;
      else if(t<3.35)sc=mix(.56,.82,smooth((t-2.55)/.80));
      else if(t<5.65)sc=.82+Math.sin((t-3.35)*1.02)*.003;
      else if(t<8.95)sc=mix(.82,.48,smooth((t-5.65)/3.30));
      else sc=.46;
      ...
      this.coreGroup.rotation.y=Math.sin(time*.00020)*.009;
      this.coreGroup.rotation.x=Math.sin(time*.00024)*.007;

      const early=smooth((t-.10)/.55);
      const eye=smooth((t-2.60)/.40);
      const eyeHold=eye*(1-smooth((t-9.46)/.28));
      const pulse=.978+.022*Math.sin(time*.0050);

      this.coreShell.material.opacity=early*(.64+.10*eyeHold);
      this.coreGlass.material.opacity=early*(.048+.065*eyeHold);
      this.coreEdges.material.opacity=early*(.060+.090*eyeHold);

      this.irisGroup.scale.setScalar((.050+eyeHold*.92)*pulse);
      this.irisRays.rotation.z=time*.000070;
      this.glowSprite.material.opacity=(.003+eyeHold*.15)*pulse;
      this.glowSprite.scale.setScalar(1.05+eyeHold*.22);
      this.coreInner.material.opacity=.006+eyeHold*.18;
      this.coreLight.intensity=eyeHold*(2.7+Math.sin(time*.005)*.26);

      if(this.coreLabel)this.coreLabel.material.opacity=.23*early*(1-smooth((t-2.62)/.32));
    }
    ...
    updateOrganic(t,time){
      ...
      this.organicVeinMaterial.opacity=.73*visible;
      this.organicHoodMaterial.opacity=.54*visible;
      if(this.organicFoldMaterial)this.organicFoldMaterial.opacity=.98*visible;
      if(this.organicFoldGlowMaterial)this.organicFoldGlowMaterial.opacity=.31*visible;
    ...
    updateCells(t,time){
      ...
      this.cellMaterial.opacity=.20*visible;
    ...
    updateTentacles(t,time){
      ...
      this.tentacleMaterial.opacity=.84*grow*flashFade;
      this.tentacleEdgeMaterial.opacity=.006*grow;
      this.tentacleNodeMaterial.opacity=.050*grow;
      if(this.tentacleGlowMaterial)this.tentacleGlowMaterial.opacity=.55*grow;
    ...
    updateCamera(t,time){
      let z=6.30,y=.025,x=0;
      if(t<2.55){
        const k=smooth(t/2.55);
        z=mix(5.42,5.82,k);
        x=Math.sin(time*.00030)*.10*(1-k*.35);
        y=.020+Math.sin(time*.00026)*.034;
      }else if(t<3.40){
        ...
        z=4.30+Math.sin(time*.00023)*.014;
        x=Math.sin(time*.00018)*.010;
        y=Math.cos(time*.00020)*.008;
      }else if(t<9.10){
        const k=smooth((t-5.55)/3.55);
        z=mix(4.30,6.10,k);
        y=mix(0,.008,k);
      }else{
        z=mix(6.10,6.28,smooth((t-9.10)/.60));
        y=.008;
      }
    ...
    render(r,time){
      ...
      if(this.debris){
        this.debris.rotation.y=time*.000020;
        this.debris.rotation.z=Math.sin(time*.00012)*.025;
      }
      ...
      if(this.flashBurst){
        this.flashBurst.material.opacity=flash*.98;
        const burstScale=4.0+flash*4.2;
        this.flashBurst.scale.set(burstScale,burstScale,1);
      }
      if(this.flashBeam){
        this.flashBeam.material.opacity=flash*.82;
        this.flashBeam.scale.x=1+flash*1.10;
      }
  ...
  async function attach(canvas,getTarget){
    ...
    try{
      ...
      if(!visualProof && isSoftwareWebGL()){
        document.documentElement.dataset.fxMagBirthR682='software-webgl-r649-fallback';
        return null;
      }
      loader ||= loadThree();
      ...
      return {
        ...
        engine,
        revision:'r682-frame-match-three-genesis'
      };
    }catch(error){
      console.error('FormatX R682 genesis renderer failed:',error);
      document.documentElement.dataset.fxMagBirthR682='fallback-r649';
      return null;
    }
  }
  ...
  window.FormatXMagGenesisThreeR682={
    attach,
    revision:'r682-frame-match-three-genesis-dna-cellular-living-architecture'
  };
})();