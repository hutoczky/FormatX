/* FormatX R563 — bounded-cost single native WebGL living core.
   One canonical canvas, one renderer and one lifecycle owner. Mobile keeps the
   crystal/sphere morph, cyan-violet glass, luminous nucleus and surface sweep,
   but uses a materially smaller mesh and a dedicated low-op shader so software
   WebGL startup cannot monopolise the intro deadline.
   Compatibility: const REVISION = 'living-luminous-electric-crystal-r558-bounded-shader' */
(function () {
  'use strict';
  ...
  root.dataset.fxCoreShaderBudgetR563=mobile?'mobile-low-op-no-pow-exp-12x24':'desktop-r558-preserved';
  ...
  function shaderFailure(gl,shader,fallback){if(gl.getShaderParameter(shader,gl.COMPILE_STATUS))return'';return gl.getShaderInfoLog(shader)||fallback;}
  function waitForProgram(gl,program,extension){
    if(!extension){root.dataset.fxCoreShaderCompileR550='synchronous-fallback';root.dataset.fxCoreShaderCompileR558='bounded-synchronous-fallback';root.dataset.fxCoreShaderCompileR563='mobile-low-op-synchronous-fallback';return Promise.resolve('synchronous-fallback');}
    root.dataset.fxCoreShaderCompileR550='parallel-khr-pending';root.dataset.fxCoreShaderCompileR558='parallel-khr-pending';root.dataset.fxCoreShaderCompileR563='parallel-khr-pending';
    return new Promise(resolve=>{const poll=()=>{if(gl.isContextLost()){resolve('context-lost');return;}if(gl.getProgramParameter(program,extension.COMPLETION_STATUS_KHR)){root.dataset.fxCoreShaderCompileR550='parallel-khr-complete';root.dataset.fxCoreShaderCompileR558='parallel-khr-complete';root.dataset.fxCoreShaderCompileR563='parallel-khr-complete';resolve('parallel-khr');return;}setTimeout(poll,12);};poll();});
  }
  async function link(gl,vertexSource,fragmentSource){
    const program=gl.createProgram(),vertex=createShader(gl,gl.VERTEX_SHADER,vertexSource),fragment=createShader(gl,gl.FRAGMENT_SHADER,fragmentSource);
    gl.attachShader(program,vertex);gl.attachShader(program,fragment);
    ['aSphere','aCrystal','aSphereNormal','aCrystalNormal','aUv','aBary','aFacet'].forEach((name,index)=>gl.bindAttribLocation(program,index,name));
    gl.linkProgram(program);
    const parallel=gl.getExtension('KHR_parallel_shader_compile');
    const compileMode=await waitForProgram(gl,program,parallel);
    if(compileMode==='context-lost'){gl.deleteShader(vertex);gl.deleteShader(fragment);gl.deleteProgram(program);throw new Error('crystal organism context lost during shader compile');}
    const vertexError=shaderFailure(gl,vertex,'crystal organism vertex shader compile failed'),fragmentError=shaderFailure(gl,fragment,'crystal organism fragment shader compile failed');
    gl.deleteShader(vertex);gl.deleteShader(fragment);
    if(vertexError||fragmentError){gl.deleteProgram(program);throw new Error(vertexError||fragmentError);}
    if(!gl.getProgramParameter(program,gl.LINK_STATUS)){const message=gl.getProgramInfoLog(program)||'crystal organism program link failed';gl.deleteProgram(program);throw new Error(message);}
    return program;
  }
  ...
  function buildOrganismGeometry(){
    const latitudeSegments=mobile?12:30,longitudeSegments=mobile?24:56;
    ...
    function vertex(latIndex,lonIndex){
      const latitude=latIndex/latitudeSegments,longitude=lonIndex/longitudeSegments,phi=latitude*Math.PI,theta=longitude*Math.PI*2,sinPhi=Math.sin(phi),direction=[sinPhi*Math.cos(theta),Math.cos(phi),sinPhi*Math.sin(theta)],spherePosition=direction.map(value=>value*.91);
      const axisX=direction[0]>=0?.88:.86,axisY=direction[1]>=0?1.09:.97,axisZ=direction[2]>=0?.64:.43,exponent=.78,terms=Math.pow(Math.abs(direction[0])/axisX,exponent)+Math.pow(Math.abs(direction[1])/axisY,exponent)+Math.pow(Math.abs(direction[2])/axisZ,exponent),radial=1/Math.pow(Math.max(.0001,terms),1/exponent),organic=1+.022*Math.sin(theta*4+phi*1.7)*sinPhi*sinPhi+.010*Math.sin(theta*7-phi*3.1);
    ...
    function triangle(vertices,facet){
      let normal=normalize(cross(subtract(vertices[1].crystal,vertices[0].crystal),subtract(vertices[2].crystal,vertices[0].crystal)));
      const centre=[0,1,2].map(i=>(vertices[0].crystal[i]+vertices[1].crystal[i]+vertices[2].crystal[i])/3);if(dot(normal,centre)<0)normal=normal.map(value=>-value);const bary=[[1,0,0],[0,1,0],[0,0,1]];
      vertices.forEach((item,index)=>{sphere.push(...item.sphere);crystal.push(...item.crystal);sphereNormals.push(...item.sphereNormal);crystalNormals.push(...normal);uvs.push(...item.uv);barycentrics.push(...bary[index]);facets.push(facet);});
    }
    for(let lat=0;lat<latitudeSegments;lat+=1)for(let lon=0;lon<longitudeSegments;lon+=1){const a=vertex(lat,lon),b=vertex(lat,lon+1),c=vertex(lat+1,lon),d=vertex(lat+1,lon+1);if(lat>0)triangle([a,b,c],.08+.92*facetValue(lon,lat*2));if(lat<latitudeSegments-1)triangle([b,d,c],.08+.92*facetValue(lon+37,lat*2+1));}
  ...
  async function boot(attempt=0){
    ...
    const vi=webgl2?'in':'attribute',vo=webgl2?'out':'varying',fi=webgl2?'in':'varying',out=webgl2?'outColor':'gl_FragColor',version=webgl2?'#version 300 es\n':'';
    const vertexSource=mobile?`${version}precision mediump float;
${vi} vec3 aSphere;${vi} vec3 aCrystal;${vi} vec3 aSphereNormal;${vi} vec3 aCrystalNormal;${vi} vec2 aUv;${vi} vec3 aBary;${vi} float aFacet;
uniform float uTime,uEnergy,uBreath,uLayer,uMorph,uAspect,uSiteProgress;uniform vec2 uPointer;uniform vec3 uRotation;
${vo} vec3 vNormal;${vo} vec3 vLocal;${vo} vec2 vUv;${vo} float vFacet;${vo} float vMorph;
void main(){float m=uMorph*uMorph*(3.0-2.0*uMorph);vec3 n=normalize(mix(aCrystalNormal,aSphereNormal,m));vec3 p=mix(aCrystal,aSphere,m);float s=uLayer>.5?.50:1.0;float cy=cos(.08+uRotation.y+uPointer.x*.20),sy=sin(.08+uRotation.y+uPointer.x*.20);vec3 q=vec3(cy*p.x+sy*p.z,p.y,-sy*p.x+cy*p.z);q=(q+n*sin(uTime*.8+aUv.y*9.0)*.009)*s*(1.0+uBreath*.018);q.xy+=uPointer*.030*uLayer;vNormal=n;vLocal=q;vUv=aUv;vFacet=aFacet;vMorph=m;float camera=3.12-q.z*.70;vec2 projected=vec2(q.x/max(.56,uAspect),q.y)*vec2(mix(1.46,1.0,m),mix(1.28,1.0,m))*(2.76/max(1.72,camera));projected.y+=.010;gl_Position=vec4(projected,q.z*.13,1.0);}`:`${version}precision highp float;
${vi} vec3 aSphere;${vi} vec3 aCrystal;${vi} vec3 aSphereNormal;${vi} vec3 aCrystalNormal;${vi} vec2 aUv;${vi} vec3 aBary;${vi} float aFacet;
uniform float uTime,uEnergy,uBreath,uLayer,uMorph,uAspect,uSiteProgress;uniform vec2 uPointer;uniform vec3 uRotation;
${vo} vec3 vNormal;${vo} vec3 vLocal;${vo} vec2 vUv;${vo} vec3 vBary;${vo} float vFacet;${vo} float vMorph;
mat3 rx(float a){float c=cos(a),s=sin(a);return mat3(1.,0.,0.,0.,c,-s,0.,s,c);}mat3 ry(float a){float c=cos(a),s=sin(a);return mat3(c,0.,s,0.,1.,0.,-s,0.,c);}mat3 rz(float a){float c=cos(a),s=sin(a);return mat3(c,-s,0.,s,c,0.,0.,0.,1.);}void main(){float m=uMorph*uMorph*(3.0-2.0*uMorph);vec3 n=normalize(mix(aCrystalNormal,aSphereNormal,m));vec3 base=mix(aCrystal,aSphere,m);vec3 local=(base+n*sin(uTime*.9+aUv.y*10.0+aUv.x*6.283)*.012)* (uLayer>.5?.50:1.0);mat3 r=rz(uRotation.z)*ry(.08+uRotation.y+uPointer.x*.22+uTime*.020)*rx(-.055+uRotation.x-uPointer.y*.15);vec3 w=r*local;vNormal=normalize(r*n);vLocal=w;vUv=aUv;vBary=aBary;vFacet=aFacet;vMorph=m;float camera=3.12-w.z*.70;vec2 projected=vec2(w.x/max(.56,uAspect),w.y)*vec2(mix(1.48,1.0,m),mix(1.30,1.0,m))*(2.76/max(1.72,camera));projected.y+=.010;gl_Position=vec4(projected,w.z*.13,1.0);}`;

    const fragmentSource=mobile?`${version}precision mediump float;
uniform float uTime,uEnergy,uBreath,uLayer,uMorph,uSiteProgress,uSurfacePulse;uniform vec2 uPointer;
${fi} vec3 vNormal;${fi} vec3 vLocal;${fi} vec2 vUv;${fi} float vFacet;${fi} float vMorph;${webgl2?'out vec4 outColor;':''}
float sat(float v){return clamp(v,0.,1.);}void main(){vec3 n=normalize(vNormal);float facing=sat(abs(n.z));float f=1.0-facing;f=f*f;float light=sat(dot(n,normalize(vec3(-.42,.73,.54))));float facet=.5+.5*sin(vFacet*8.0+uTime*.25);vec2 hp=vec2(vLocal.x,vLocal.y)-uPointer*.035;float r=length(hp);float h=sat(1.0-r/.42);h=h*h;float nu=sat(1.0-r/.15);nu=nu*nu*nu;float ring=1.0-smoothstep(.018,.045,abs(r-.20));float sweep=0.0;if(uSurfacePulse>=0.0){float pos=.5+(vLocal.y*.64+vLocal.x*.18)*.5;float head=-.15+1.30*sat(uSurfacePulse);sweep=sat(1.0-abs(pos-head)*12.0)*(.45+.55*f);}vec3 cyan=vec3(.03,1.05,1.55),violet=vec3(.65,.16,1.15),ice=vec3(.90,1.30,1.62),spectral=mix(cyan,violet,.18+.22*facet);float visual=sat(.48+uEnergy*.60);if(uLayer>.5){vec3 c=vec3(.025,.15,.38)+spectral*(.34+.30*visual)+ice*(h*.58+nu*1.70+ring*.48)+cyan*sweep*.90;float a=clamp(.18+.18*visual+.18*h+.28*nu+.12*ring+.10*sweep,.16,.84);${out}=vec4(c/(vec3(1.0)+c),a);return;}vec3 c=vec3(.035,.16,.38)+vec3(.04,.42,.72)*light+spectral*f*(.75+.55*visual)+ice*nu*.28+(ice+cyan*.35)*sweep*.70;float a=clamp(.33+.15*light+.24*f+.08*sweep,.30,.80);${out}=vec4(c/(vec3(1.0)+c),a);}`:`${version}precision highp float;
uniform float uTime,uEnergy,uBreath,uLayer,uMorph,uSiteProgress,uSurfacePulse;uniform vec2 uPointer;
${fi} vec3 vNormal;${fi} vec3 vLocal;${fi} vec2 vUv;${fi} vec3 vBary;${fi} float vFacet;${fi} float vMorph;${webgl2?'out vec4 outColor;':''}
float sat(float v){return clamp(v,0.,1.);}vec3 filmic(vec3 c){return 1.0-exp(-max(c,vec3(0.)));}void main(){vec3 n=normalize(vNormal);vec3 view=normalize(vec3(-vLocal.xy,2.9-vLocal.z));vec3 key=normalize(vec3(-.42,.73,.54));float ndl=max(dot(n,key),0.);float facing=sat(abs(dot(n,view)));float fresnel=pow(1.0-facing,1.68);float specular=pow(max(dot(n,normalize(key+view)),0.),20.0);float facet=.5+.5*sin(vFacet*11.0+uTime*.35+uSiteProgress*4.0);float edge=1.0-smoothstep(.010,.052,min(vBary.x,min(vBary.y,vBary.z)));vec2 heartLocal=vec2(vLocal.x,vLocal.y*1.025)-vec2(uPointer.x*.05,uPointer.y*.04);float radial=length(heartLocal);float heart=pow(sat(1.0-radial/.40),3.0);float nucleus=pow(sat(1.0-radial/.13),4.0);float ring=1.0-smoothstep(.012,.035,abs(radial-.19));float sweep=0.0;if(uSurfacePulse>=0.0){float coordinate=.5+(vLocal.y*.62+vLocal.x*.15+vLocal.z*.18)*.5;float head=mix(-.18,1.18,sat(uSurfacePulse));sweep=exp(-pow((coordinate-head)/.070,2.0))*(.38+.62*fresnel);}vec3 cyan=vec3(.03,1.14,1.66),violet=vec3(.74,.18,1.30),ice=vec3(.95,1.42,1.76),spectral=mix(cyan,violet,.16+.28*facet);float visual=sat(.48+uEnergy*.62);if(uLayer>.5){vec3 organ=mix(vec3(.035,.20,.50),spectral,.42+.22*visual);organ+=ice*heart;organ+=ice*nucleus*2.6;organ+=spectral*ring;organ+=(ice+cyan*.55)*sweep*1.55;${out}=vec4(filmic(organ*2.45),clamp(.18+.20*visual+.20*heart+.32*nucleus+.14*ring+.12*sweep,.16,.86));return;}vec3 glass=mix(vec3(.035,.17,.40),vec3(.08,.58,.96),.26+.34*ndl);glass+=spectral*fresnel*(1.05+.75*visual);glass+=ice*specular;glass+=spectral*edge*.55;glass+=(ice+cyan*.48)*sweep;${out}=vec4(filmic(glass*2.24),clamp(.34+.17*ndl+.28*fresnel+.07*edge+.12*specular+.10*sweep,.32,.82));}`;
    ...
    function upload(buffer,data,index,size){if(index<0)return;gl.bindBuffer(gl.ARRAY_BUFFER,buffer);gl.bufferData(gl.ARRAY_BUFFER,data,gl.STATIC_DRAW);gl.enableVertexAttribArray(index);gl.vertexAttribPointer(index,size,gl.FLOAT,false,0,0);}
    ...
    let disposed=false,contextLost=false,visible=true,raf=0,burstFrames=0,aspect=1,px=0,py=0,tx=0,ty=0,energy=IDLE_ENERGY,targetEnergy=IDLE_ENERGY,breath=.12,targetBreath=.12,morph=initialShape==='sphere'?1:0,targetMorph=morph,rotationX=-.035,rotationY=0,rotationZ=0,targetRotationX=rotationX,targetRotationY=0,targetRotationZ=0,angularVelocityY=0,siteProgress=0,targetSiteProgress=0,last=performance.now(),simulationTime=0,renderAverage=0,surfacePulseTimer=0,scrollFrame=0,tapCandidate=null,surfacePulseStart=-Infinity,lastSurfacePulseAt=-Infinity,surfacePulseCount=0,activeOrgan='hero',shapeLockUntil=0,birthAnimation=null;
    const cinematic=window.FormatXCoreCinematic=window.FormatXCoreCinematic||{};cinematic.version='r563';cinematic.corePosition=[0,0,.52];
    function resize(){const rect=stage.getBoundingClientRect();if(rect.width<2||rect.height<2)return false;const cap=mobile?1.40:1.65,dpr=Math.min(devicePixelRatio||1,cap),budget=mobile?620000:1150000;let w=Math.max(2,Math.round(rect.width*dpr)),h=Math.max(2,Math.round(rect.height*dpr));if(w*h>budget){const k=Math.sqrt(budget/(w*h));w=Math.round(w*k);h=Math.round(h*k);}if(canvas.width!==w||canvas.height!==h){canvas.width=w;canvas.height=h;}aspect=rect.width/Math.max(1,rect.height);gl.viewport(0,0,w,h);root.dataset.fxCoreReal3dResolution=`${w}x${h}`;root.dataset.fxCoreReal3dScale=(w/Math.max(1,rect.width)).toFixed(2);root.dataset.fxCoreViewportAspect=aspect.toFixed(4);return true;}
    function blocked(){return disposed||contextLost||document.hidden||!visible||root.dataset.fxRenderLifecycleSuspended==='true';}
    function schedule(frames=1){if(blocked())return;const frameCap=mobile?6:24;burstFrames=Math.max(burstFrames,Math.min(frameCap,Math.max(1,frames)));if(!raf){last=performance.now();raf=requestAnimationFrame(frame);}}
    ...
    function setMorph(value,source='api-morph',announce=true){const next=clamp(Number(value)||0,0,1),changed=Math.abs(next-targetMorph)>.001;targetMorph=next;if(/mag-button|api|keyboard|core-tap/.test(source))shapeLockUntil=performance.now()+7600;if(reduced.matches)morph=targetMorph;publishShape(source);boost(changed?1.04:.68,changed?6:2);if(changed&&announce)dispatchEvent(new CustomEvent('formatx:coreshapechange',{detail:{shape:shapeName(next),source,revision:'r413',renderer:VERSION,geometry:'closed-3d-volume'}}));return targetMorph;}
    function setShape(shape,source='api'){return setMorph(shape==='sphere'||shape===1||shape===true?1:0,source,true);}function toggleShape(source='interaction'){return setShape(targetMorph>=.5?'crystal':'sphere',source);}function rotateBy(x,y,source='api-rotate'){targetRotationX=clamp(targetRotationX+x,-1.02,1.02);targetRotationY+=y;root.dataset.fxCoreRotationSource=source;boost(.84,mobile?4:8);}
    ...
    function render(now){const begin=performance.now(),dt=Math.min(48,Math.max(1,now-last));last=now;if(!reduced.matches)simulationTime+=dt*.001;const ease=1-Math.exp(-dt*.018);px+=(tx-px)*ease;py+=(ty-py)*ease;rotationX+=(targetRotationX-rotationX)*ease;rotationY+=(targetRotationY-rotationY)*ease;rotationZ+=(targetRotationZ-rotationZ)*ease;energy+=(targetEnergy-energy)*ease;breath+=(targetBreath-breath)*ease;targetEnergy+=(IDLE_ENERGY-targetEnergy)*.04;targetBreath+=(.12-targetBreath)*.04;morph+=(targetMorph-morph)*(reduced.matches?1:.12);if(Math.abs(morph-targetMorph)<.001)morph=targetMorph;siteProgress+=(targetSiteProgress-siteProgress)*.08;publishShape();gl.clear(gl.COLOR_BUFFER_BIT|gl.DEPTH_BUFFER_BIT);gl.useProgram(program);gl.uniform1f(uniforms.uTime,simulationTime);gl.uniform1f(uniforms.uEnergy,energy);gl.uniform1f(uniforms.uBreath,breath);gl.uniform1f(uniforms.uMorph,morph);gl.uniform2f(uniforms.uPointer,px,py);gl.uniform3f(uniforms.uRotation,rotationX,rotationY,rotationZ);gl.uniform1f(uniforms.uAspect,aspect);gl.uniform1f(uniforms.uSiteProgress,siteProgress);const pe=(now-surfacePulseStart)/SURFACE_PULSE_MS;gl.uniform1f(uniforms.uSurfacePulse,pe>=0&&pe<=1?pe:-1);gl.depthMask(false);gl.blendFunc(gl.SRC_ALPHA,gl.ONE);if(!mobile){gl.cullFace(gl.FRONT);gl.uniform1f(uniforms.uLayer,0);gl.drawArrays(gl.TRIANGLES,0,geometry.count);}gl.cullFace(gl.BACK);gl.uniform1f(uniforms.uLayer,1);gl.drawArrays(gl.TRIANGLES,0,geometry.count);gl.depthMask(true);gl.blendFunc(gl.SRC_ALPHA,gl.ONE_MINUS_SRC_ALPHA);gl.uniform1f(uniforms.uLayer,0);gl.drawArrays(gl.TRIANGLES,0,geometry.count);const ms=performance.now()-begin;renderAverage=renderAverage?renderAverage*.82+ms*.18:ms;root.dataset.fxCoreRenderMs=renderAverage.toFixed(2);root.dataset.fxCoreFrameMs=dt.toFixed(2);root.dataset.fxCoreReal3dFps=String(Math.min(60,Math.round(1000/Math.max(16.67,renderAverage))));}
    ...
    function startMagBirth(){
      const duration=mobile?1180:1350;
      root.dataset.fxMagBirthDurationR845=String(duration);
      root.dataset.fxMagBirthOwnerR845='canonical-crystal-organism-r326-single-webgl';
      if(reduced.matches||typeof stage.animate!=='function'){
        stage.dataset.fxMagBirthR845='online';
        root.dataset.fxMagBirthR845='mag-online';
        boost(.72,1);schedule(1);scheduleSurfacePulse();
        dispatchEvent(new CustomEvent('formatx:magbirthcomplete',{detail:{duration:0,source:'reduced-or-no-waapi',renderer:VERSION}}));
        return;
      }
      stage.dataset.fxMagBirthR845='active';
      root.dataset.fxMagBirthR845='energy-field-awakening';
      dispatchEvent(new CustomEvent('formatx:magbirthstart',{detail:{duration,renderer:VERSION,revision:'r845'}}));
      birthAnimation=stage.animate([
        {offset:0,opacity:.08,transform:'scale(.08) rotate(-16deg)',filter:'blur(14px) brightness(.48) saturate(.65) drop-shadow(0 0 0 rgba(121,234,255,0))'},
        {offset:.16,opacity:.30,transform:'scale(.34) rotate(-10deg)',filter:'blur(8px) brightness(.72) saturate(.82) drop-shadow(0 0 18px rgba(121,234,255,.18))'},
        {offset:.34,opacity:.68,transform:'scale(.72) rotate(-4deg)',filter:'blur(3px) brightness(1.02) saturate(1.02) drop-shadow(0 0 34px rgba(121,234,255,.26))'},
        {offset:.58,opacity:1,transform:'scale(1.04) rotate(0deg)',filter:'blur(0) brightness(1.72) saturate(1.18) drop-shadow(0 0 58px rgba(121,234,255,.42))'},
        {offset:.77,opacity:1,transform:'scale(.985) rotate(0deg)',filter:'blur(0) brightness(1.22) saturate(1.12) drop-shadow(0 0 30px rgba(139,104,255,.28))'},
        {offset:.93,opacity:1,transform:'scale(1.01) rotate(0deg)',filter:'blur(0) brightness(1.12) saturate(1.08) drop-shadow(0 0 22px rgba(121,234,255,.18))'},
        {offset:1,opacity:1,transform:'scale(1) rotate(0deg)',filter:'blur(0) brightness(1) saturate(1) drop-shadow(0 0 0 rgba(121,234,255,0))'}
      ],{duration,easing:'linear',fill:'none'});
      const phases=[
        [.16,'signal-seeds-captured'],
        [.34,'crystal-structure-assembling'],
        [.58,'first-impulse'],
        [.77,'nervous-system-sync'],
        [.93,'mag-online']
      ];
      for(const [at,name] of phases)later(()=>{
        if(disposed||stage.dataset.fxMagBirthR845!=='active')return;
        root.dataset.fxMagBirthR845=name;
        if(name==='crystal-structure-assembling')boost(.90,mobile?3:6);
        if(name==='first-impulse'){boost(1.12,mobile?5:12);startSurfacePulse('birth-first-impulse');}
        if(name==='nervous-system-sync')boost(.82,mobile?3:6);
        dispatchEvent(new CustomEvent('formatx:magbirthphase',{detail:{phase:name,progress:at,renderer:VERSION}}));
      },Math.round(duration*at));
      birthAnimation.finished.then(()=>{
        if(disposed)return;
        birthAnimation=null;
        stage.dataset.fxMagBirthR845='online';
        root.dataset.fxMagBirthR845='mag-online';
        root.dataset.fxMagBirthSteadyR845='same-stage-same-canvas-continuous-life';
        boost(.72,2);schedule(1);scheduleSurfacePulse();
        dispatchEvent(new CustomEvent('formatx:magbirthcomplete',{detail:{duration,source:'canonical-webgl-stage',renderer:VERSION}}));
      }).catch(()=>{});
    }
    function destroy(){if(disposed)return;disposed=true;clearTimeout(surfacePulseTimer);try{birthAnimation?.cancel();}catch(_){}birthAnimation=null;delayed.forEach(clearTimeout);delayed.clear();if(raf)cancelAnimationFrame(raf);if(scrollFrame)cancelAnimationFrame(scrollFrame);controller.abort();ro.disconnect();io.disconnect();organObserver.disconnect();if(!contextLost){buffers.forEach(buffer=>gl.deleteBuffer(buffer));gl.deleteProgram(program);}stage.remove();if(window.FormatXCoreMobileV69?.destroy===destroy)delete window.FormatXCoreMobileV69;if(window.FormatXLivingCore?.destroy===destroy)delete window.FormatXLivingCore;}
    resize();
