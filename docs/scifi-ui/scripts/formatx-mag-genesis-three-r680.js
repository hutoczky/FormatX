(() => {
  ...
  class FormatXGenesisThree {
    ...
    makeTentacles(){
      ...
      for(let i=0;i<count;i++){
        ...
        const geo=this.createTaperedTube(curve,56,8,.074+r()*.014,.009+r()*.003);
    ...
    updateCore(t,time){
      ...
      this.glowSprite.material.opacity=(.004+eyeHold*.31)*pulse;
      this.glowSprite.scale.setScalar(1.12+eyeHold*.34);
      this.coreInner.material.opacity=.010+eyeHold*.46;
      this.coreLight.intensity=eyeHold*(5.4+Math.sin(time*.005)*.52);
    ...
    updateOrganic(t,time){
      ...
      this.organicHoodMaterial.opacity=.64*visible;
  ...
  async function attach(canvas,getTarget){
    ...
    try{
      ...
      if(!visualProof && isSoftwareWebGL()){
        document.documentElement.dataset.fxMagBirthR680='software-webgl-r649-fallback';
        return null;
      }
      loader ||= loadThree();
      ...
      return {
        ...
        engine,
        revision:'r680-reference-ratio-three-genesis'
      };
    }catch(error){
      console.error('FormatX R680 genesis renderer failed:',error);
      document.documentElement.dataset.fxMagBirthR680='fallback-r649';
      return null;
    }
  }
  ...
  window.FormatXMagGenesisThreeR680={
    attach,
    revision:'r680-reference-ratio-three-genesis-dna-cellular-living-architecture'
  };
})();