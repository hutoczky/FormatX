(() => {
  ...
  class FormatXGenesisThree {
    ...
    makeMechanicalLayer(){
      ...
      this.seams=[];
      const seamGeo=new T.BoxGeometry(.115,.024,.024);
      const seamDefs=[
        [-.55,.07,.34,0],
        [.55,.07,.34,0],
        [-.18,-.60,.33,Math.PI/2],
    ...
    updateMechanical(t,time){
      ...
      if(this.seamMaterial)this.seamMaterial.opacity=.54*grow;
  ...
  async function attach(canvas,getTarget){
    ...
    try{
      ...
      if(!visualProof && isSoftwareWebGL()){
        document.documentElement.dataset.fxMagBirthR666='software-webgl-r649-fallback';
        return null;
      }
      loader ||= loadThree();
      ...
      return {
        ...
        engine,
        revision:'r666-integrated-organic-three-genesis'
      };
    }catch(error){
      console.error('FormatX R666 genesis renderer failed:',error);
      document.documentElement.dataset.fxMagBirthR666='fallback-r649';
      return null;
    }
  }
  ...
  window.FormatXMagGenesisThreeR666={
    attach,
    revision:'r666-integrated-organic-three-genesis-dna-cellular-living-architecture'
  };
})();