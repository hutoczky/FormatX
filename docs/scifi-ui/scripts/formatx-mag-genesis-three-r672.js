(() => {
  ...
  class FormatXGenesisThree {
    ...
    makeOrganicLayer(){
      ...
      this.organicLobeMaterial=new T.MeshPhysicalMaterial({
        color:0x120b1b,roughness:.66,metalness:.03,
        clearcoat:.14,clearcoatRoughness:.50,
        transparent:true,opacity:0,
        emissive:0x170a22,emissiveIntensity:.22,
        depthWrite:true
      });
    ...
    updateCore(t,time){
      let sc=.001;
      if(t<2.55)sc=.48+ease(t/2.55)*.08;
      else if(t<3.35)sc=mix(.56,.52,smooth((t-2.55)/.80));
      else if(t<5.65)sc=.52+Math.sin((t-3.35)*1.02)*.003;
      else if(t<8.95)sc=mix(.52,.40,smooth((t-5.65)/3.30));
      else sc=.40;
      ...
      const surfaceZ=cellSurface*1.19;
      ...
      this.coreShell.material.opacity=early*(.52+.30*eyeHold);
      this.coreGlass.material.opacity=early*(.07+.12*eyeHold);
      this.coreEdges.material.opacity=early*(.09+.20*eyeHold);

      this.irisGroup.scale.setScalar((.08+eyeHold*.92)*pulse);
      this.irisRays.rotation.z=time*.000090;
      this.glowSprite.material.opacity=(.003+eyeHold*.48)*pulse;
      this.glowSprite.scale.setScalar(1.28+eyeHold*.44);
      this.coreInner.material.opacity=.012+eyeHold*.68;
      this.coreLight.intensity=eyeHold*(8.0+Math.sin(time*.005)*1.0);

      if(this.coreLabel)this.coreLabel.material.opacity=.32*early*(1-smooth((t-2.72)/.30));
    }
    ...
    render(r,time){
      ...
      this.renderer.toneMappingExposure=1.16+flash*.78+after*.05;
      this.coreLight.intensity+=flash*64+after*5;
      if(this.glowSprite){
        const g=1+flash*2.20;
        this.glowSprite.scale.multiplyScalar(g);
        this.glowSprite.material.opacity=Math.min(1,this.glowSprite.material.opacity+flash*.34);
      }
  ...
  async function attach(canvas,getTarget){
    ...
    try{
      ...
      if(!visualProof && isSoftwareWebGL()){
        document.documentElement.dataset.fxMagBirthR672='software-webgl-r649-fallback';
        return null;
      }
      loader ||= loadThree();
      ...
      return {
        ...
        engine,
        revision:'r672-video-scale-local-flash-three-genesis'
      };
    }catch(error){
      console.error('FormatX R672 genesis renderer failed:',error);
      document.documentElement.dataset.fxMagBirthR672='fallback-r649';
      return null;
    }
  }
  ...
  window.FormatXMagGenesisThreeR672={
    attach,
    revision:'r672-video-scale-local-flash-three-genesis-dna-cellular-living-architecture'
  };
})();