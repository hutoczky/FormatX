...
root.dataset.fxQuantumParticleHitPolicy='window-capture-coordinate-r345';
...
function mount(stage){
  ...
  for(let i=0;i<count;i++){
    ...
    field.append(node);
    particles.push({node,index:i,pair:Math.floor(i/2),phase:hash(i+4)*TAU,shell:i%4,polarity:i%2===0?1:-1,seed:hash(i+71),x:0,y:0});
  }

  function bounds(){
    const r=field.getBoundingClientRect();
    return {rect:r,rx:Math.max(72,r.width*.43),ry:Math.max(96,r.height*.39),cx:r.width*.5,cy:r.height*.5};
  }
  ...
  function shellRadius(shell){return [0.22,0.37,0.54,0.72][shell]||0.54;}
  ...
  function orbitalPoint(p,b,serial,interaction){
    ...
    const angle=p.phase+pairPhase*0.36+p.polarity*jitter;
    let x=Math.cos(angle)*b.rx*shell;
    let y=Math.sin(angle)*b.ry*shell*0.78;
    if(interaction){
      const dx=interaction.x-x,dy=interaction.y-y;
      ...
      x+=dx*response;y+=dy*response;
    }
    return{x,y};
  }
  function superpose(reason='idle',interaction=null){
    const b=bounds();interactionSerial++;
  ...
  function measure(clientX,clientY,inputSource='pointer',force=false){
    ...
    dispatchEvent(new CustomEvent('formatx:quantummeasurement',{detail:{revision:'r345',source:inputSource,measurements:Number(root.dataset.fxQuantumParticleMeasurements)}}));
  ...
  function tunnel(px,py){
    ...
    for(const p of particles){
      if(hash(p.index+interactionSerial*19.7)<.38){
        const x=-p.x*.92+px*.08,y=-p.y*.86+py*.06;
        p.shell=(p.shell+1+(p.pair%2))%4;
        setParticle(p,x,y,p.shell,.82+p.seed*.66,reduced?1:165);
      }
    }
    dispatchEvent(new CustomEvent('formatx:quantumtunnel',{detail:{revision:'r345',tunnels:Number(root.dataset.fxQuantumParticleTunnels)}}));
    clearTimeout(relaxTimer);
  ...
  // r345: capture at Window, before document-level controls can terminate
  // propagation. Geometry remains the authoritative hit test, so offscreen
  // or unrelated UI input cannot trigger a quantum measurement.
  ...
  dispatchEvent(new CustomEvent('formatx:quantumparticlesready',{detail:{revision:'r345',count,model:root.dataset.fxQuantumParticleModel}}));
