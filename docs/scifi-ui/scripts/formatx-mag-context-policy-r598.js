/* FormatX R598 — paint-safe OffscreenCanvas MAG transport owner.
   Navigation still owns the canonical MAG stage/canvas immediately. Worker creation,
   OffscreenCanvas transfer and WebGL init are deferred to the existing bounded init
   checkpoint, so compositor/GPU transport setup cannot race the first page paint.
   Mobile retains the existing absolute 2200 ms init checkpoint. Desktop retains the
   observed first-paint checkpoint with the existing 620 ms floor and a 1750 ms
   fallback only where paint observation is unavailable. No Lighthouse, CI, audit,
   intro-complete or user-input gate. */
(function(){
'use strict';
...
let destroyed=false,ready=false,fallbackStarted=false,transportCreated=false,initPosted=false;
let initTimer=0,readyTimer=0,paintFallbackTimer=0,paintObserver=null;
...
function fallback(reason){
  ...
  root.dataset.fxMagOffscreenR564=`fallback-${reason}`;root.dataset.fxMagOffscreenR571=`fallback-${reason}`;root.dataset.fxMagOffscreenTransportR598=`fallback-${reason}`;
  try{worker?.terminate();}catch(_){}worker=null;offscreen=null;
  ...
  delete root.dataset.fxCrystalOrganismR326;delete root.dataset.fxCoreReal3d;
...
function ensureWorkerTransport(){
  if(transportCreated)return Boolean(worker&&offscreen);if(destroyed||fallbackStarted||!canvas)return false;transportCreated=true;
  try{worker=new Worker(WORKER);offscreen=canvas.transferControlToOffscreen();worker.onmessage=event=>{const detail=event.data||{};if(detail.type==='ready'){publishReady(detail.count);return;}if(detail.type==='error')fallback('worker-error');};worker.onerror=()=>fallback('worker-event-error');root.dataset.fxMagOffscreenR564='worker-started-at-init-checkpoint';root.dataset.fxMagOffscreenR571=`worker-created-${profile}-at-init-checkpoint`;root.dataset.fxMagOffscreenTransportR598=`transport-created-${profile}-at-init-checkpoint`;return true;}catch(_){try{worker?.terminate();}catch(_){}worker=null;offscreen=null;fallback('transport-bootstrap-error');return false;}
}
try{
  ...
  stage=document.createElement('div');stage.className='fx-core-mobile-v55-stage fx-crystal-organism-r326-stage';stage.dataset.renderer='crystal-organism-r326';stage.dataset.revision=`r598-${profile}-offscreen-transport-deferred`;stage.dataset.active='true';stage.setAttribute('aria-hidden','true');host.prepend(stage);
  ...
  const initial=size();if(!initial){stage.remove();root.dataset.fxMagContextPolicyR561='geometry-unavailable-main-r326';return;}canvas.width=initial.width;canvas.height=initial.height;
  root.dataset.fxCrystalOrganismR326='booting';root.dataset.fxCoreReal3d='booting';root.dataset.fxMagStartupContractR530='living-core-autostart-navigation-owned';root.dataset.fxCurrentMagRequestR530=`navigation-owned-r598-${profile}-shell-created`;root.dataset.fxMagOffscreenR564='navigation-shell-created-transport-deferred';root.dataset.fxMagOffscreenR571=`navigation-shell-created-${profile}-transport-deferred`;root.dataset.fxMagWorkerInitAtR565=String(WORKER_INIT_AT_MS);root.dataset.fxMagDesktopDecontentionR578=mobile?'mobile-existing-2200':'desktop-paint-checkpoint-scheduled';root.dataset.fxMagDesktopDecontentionR590=mobile?'mobile-existing-2200':'await-first-paint-opportunity-floor-620';root.dataset.fxMagDesktopDecontentionR591=mobile?'mobile-existing-2200':'await-observed-first-paint-or-1750';root.dataset.fxMagDesktopDecontentionR595=mobile?'mobile-existing-2200':'await-observed-paint-no-competing-init';
  const beginInit=()=>{if(destroyed||fallbackStarted||initPosted)return;const start=size()||initial;if(!ensureWorkerTransport())return;initPosted=true;root.dataset.fxMagOffscreenR571=`worker-init-started-${profile}`;root.dataset.fxMagDesktopDecontentionR590=mobile?'mobile-init-2200':'desktop-init-after-paint-checkpoint';root.dataset.fxMagDesktopDecontentionR591=mobile?'mobile-init-2200':'desktop-init-after-observed-paint';root.dataset.fxMagDesktopDecontentionR595=mobile?'mobile-init-2200':'desktop-init-after-observed-paint-r595';root.dataset.fxMagOffscreenTransportR598=`init-posted-${profile}-after-checkpoint`;worker.postMessage({type:'init',canvas:offscreen,width:start.width,height:start.height},[offscreen]);offscreen=null;readyTimer=setTimeout(()=>{if(!ready)fallback('ready-timeout');},5500);};
  if(mobile){const initDelay=Math.max(0,WORKER_INIT_AT_MS-performance.now());initTimer=setTimeout(beginInit,initDelay);root.dataset.fxMagOffscreenTransportR598='mobile-transport-deferred-until-2200';}
  else{
    let paintArmed=false;
    const armAfterPaint=source=>{if(paintArmed||destroyed||fallbackStarted)return;paintArmed=true;cancelPaintSchedule();const floorDelay=Math.max(0,WORKER_INIT_AT_MS-performance.now());initTimer=setTimeout(beginInit,floorDelay);root.dataset.fxMagDesktopDecontentionR591=`${source}-init-armed-floor-620`;root.dataset.fxMagDesktopDecontentionR595=`${source}-init-armed-floor-620`;root.dataset.fxMagOffscreenTransportR598=`${source}-transport-armed-floor-620`;};
    ...
    if(alreadyPainted)armAfterPaint('buffered-first-paint');
      ...
      let paintObservationSupported=false;
      if(typeof PerformanceObserver==='function'){const supported=PerformanceObserver.supportedEntryTypes;if(!Array.isArray(supported)||supported.includes('paint')){try{paintObserver=new PerformanceObserver(list=>{if(list.getEntries().some(entry=>entry.name==='first-paint'))armAfterPaint('observed-first-paint');});paintObserver.observe({type:'paint',buffered:true});paintObservationSupported=true;}catch(_){paintObserver=null;}}}
      if(paintObservationSupported){root.dataset.fxMagDesktopDecontentionR595='paint-observer-supported-no-unpainted-init';root.dataset.fxMagOffscreenTransportR598='desktop-no-worker-transport-before-observed-paint';}
      else{root.dataset.fxMagDesktopDecontentionR595='paint-observer-unavailable-fallback-1750';root.dataset.fxMagOffscreenTransportR598='desktop-paint-api-unavailable-fallback-1750';paintFallbackTimer=setTimeout(()=>armAfterPaint('paint-api-unavailable-fallback-1750'),DESKTOP_INIT_FALLBACK_MS);}
    }
  }
