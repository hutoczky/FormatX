...
function install(){
  const p=path();
  if(NOINDEX.has(p)){meta('meta[name="robots"]','robots','noindex,nofollow,noarchive');R.dataset.fxSeo='ready-v5';return}
  const u=O+p,c=P[p]||P['/'],v=c[lang()];
  document.title=v[0];meta('meta[name="description"]','description',v[1]);meta('meta[property="og:title"]','og:title',v[0]);meta('meta[property="og:description"]','og:description',v[1]);meta('meta[property="og:type"]','og:type','website');meta('meta[property="og:url"]','og:url',u);
  link('canonical',u);link('alternate',u+'?lang=hu','hu');link('alternate',u+'?lang=en','en');link('alternate',u,'x-default');
  const rel=release();
  const software={'@type':'SoftwareApplication','@id':O+'/#software',name:'FormatX Suite Pro',applicationCategory:'UtilitiesApplication',operatingSystem:'Linux/Bazzite; Windows; Android',url:O+'/',downloadUrl:O+'/download/multiplatform',license:O+'/scifi-ui/license.html',author:{'@type':'Person',name:'Hutóczky József'},additionalProperty:[
