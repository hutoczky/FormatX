...
function explicitLang(){const x=new URLSearchParams(location.search).get('lang');return x==='hu'||x==='en'?x:null}
...
function install(){
  ...
  const p=path();
  if(NOINDEX.has(p)){meta('meta[name="robots"]','robots','noindex,nofollow,noarchive');R.dataset.fxSeo='ready-v8';return}
  const u=O+p,c=P[p]||P['/'],v=c[lang()],q=explicitLang(),canonical=q?u+'?lang='+q:u;
  document.title=v[0];
  ...
  meta('meta[property="og:url"]','og:url',canonical);
  ...
  link('canonical',canonical);link('alternate',u+'?lang=hu','hu');link('alternate',u+'?lang=en','en');link('alternate',u,'x-default');
  const rel=release();
  const software={
  ...
  s.textContent=JSON.stringify({'@context':'https://schema.org','@graph':[{'@type':'WebSite','@id':O+'/#website',url:O+'/',name:'FormatX Suite Pro',inLanguage:['hu-HU','en-GB']},software,{'@type':'WebPage','@id':canonical+'#webpage',url:canonical,name:v[0],description:v[1],isPartOf:{'@id':O+'/#website'},about:{'@id':O+'/#software'},inLanguage:lang()==='en'?'en-GB':'hu-HU'}]});
  R.dataset.fxSeo='ready-v8';
}
