(function () {
  ...
  function copy() {
    return language() === 'en' ? {
      ...
      note: 'Monthly licence · fixed amount · no payment-card data processing',
    ...
    return [
      ...
      PLANS.map(function (plan) {
        return [
          ...
          '<div class="fx-plan-qr-copy"><strong>', plan.name, '</strong><span></span><small></small></div>',
  ...
  function ensureDock() {
    ...
    if (dock) return dock;
    const payment = document.querySelector('#pricing .payment');
    if (!payment) return null;
    payment.insertAdjacentHTML('afterend', markup());
    return document.getElementById('formatx-plan-qr-dock');
  }

  function loadImage(card, image, source) {
    let retries = 0;
    card.classList.remove('is-qr-ready', 'is-qr-error');
    card.classList.add('is-qr-loading');

    function setSource(cacheBust) {
      image.src = source + (cacheBust ? '&retry=' + Date.now() : '');
    }
    ...
    image.onerror = function () {
      retries += 1;
      if (retries <= 2) {
        window.setTimeout(function () { setSource(true); }, retries * 900);
        return;
      }
  ...
  function refresh() {
    ...
    PLANS.forEach(function (plan) {
      ...
      if (amount) amount.textContent = money(plan[selectedCurrency], selectedCurrency) + (language() === 'en' ? ' / month' : ' / hónap');
