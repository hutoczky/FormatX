(() => {
  ...
  class FormatXGenesisThree {
    ...
    updateCore(t,time){
      let sc;
      if(t<2.70) sc=.88+ease(t/2.70)*.08;
      else if(t<3.25) sc=mix(.96,.46,smooth((t-2.70)/.55));
      else if(t<6.30) sc=.46;
      else sc=mix(.46,.54,smooth((t-6.30)/.75));
      ...
      this.coreShell.material.opacity=.97*stage1 + .22*(1-stage1)*(1-mech) + .020*mech;
      this.coreGlass.material.opacity=.10*stage1 + .040*(1-stage1)*(1-mech) + .006*mech;
      this.coreEdges.material.opacity=.095*stage1 + .040*(1-stage1)*(1-mech) + .006*mech;
      if(this.corePetalMaterial)this.corePetalMaterial.opacity=.05*stage1+.10*(1-stage1)*(1-mech)+.008*mech;

      this.irisGroup.scale.setScalar((.010+eyeHold*.59)*(1-mech*.96)*pulse);
      this.irisRays.rotation.z=time*.000060;
      if(this.irisCorona)this.irisCorona.material.opacity=.86*eyeHold*(1-mech*.92);
    ...
    updateOrganic(t,time){
      ...
      this.organicShellMaterial.opacity=.54*visible;
      ...
      this.organicHoodMaterial.opacity=.62*visible;
    ...
    updateMechanical(t,time){
      ...
      this.mechanicalGroup.scale.setScalar(.001+grow*1.14);
      ...
      if(this.mechLight)this.mechLight.intensity=10.2*grow;
    ...
    updateTentacles(t,time){
      ...
      if(this.tentacleGlowMaterial)this.tentacleGlowMaterial.opacity=.94*grow;
    ...
    updateCamera(t,time){
      ...
      if(t<2.70){
        ...
        z=4.62+Math.sin(time*.00018)*.010;
        y=.004;
      }
  ...
  async function attach(canvas,getTarget){
    ...
    try{
      ...
      if(!visualProof && isSoftwareWebGL()){
        document.documentElement.dataset.fxMagBirthR961='software-webgl-r649-fallback';
        return null;
      }
      loader ||= loadThree();
      ...
      return {
        ...
        engine,
        revision:'r961-reference-proportion-dark-depth-three-genesis'
      };
    }catch(error){
      console.error('FormatX R961 genesis renderer failed:',error);
      document.documentElement.dataset.fxMagBirthR961='fallback-r649';
      return null;
    }
  }
  ...
  window.FormatXMagGenesisThreeR961={
    attach,
    revision:'r961-reference-proportion-dark-depth-three-genesis-dna-cellular-living-architecture'
  };
})();