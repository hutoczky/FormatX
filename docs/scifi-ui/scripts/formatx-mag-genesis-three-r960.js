(() => {
  ...
  class FormatXGenesisThree {
    ...
    updateCore(t,time){
      ...
      this.coreShell.material.opacity=.92*stage1 + .24*(1-stage1)*(1-mech) + .025*mech;
      this.coreGlass.material.opacity=.15*stage1 + .045*(1-stage1)*(1-mech) + .008*mech;
      this.coreEdges.material.opacity=.18*stage1 + .045*(1-stage1)*(1-mech) + .008*mech;
      if(this.corePetalMaterial)this.corePetalMaterial.opacity=.05*stage1+.10*(1-stage1)*(1-mech)+.008*mech;

      this.irisGroup.scale.setScalar((.010+eyeHold*.54)*(1-mech*.96)*pulse);
  ...
  async function attach(canvas,getTarget){
    ...
    try{
      ...
      if(!visualProof && isSoftwareWebGL()){
        document.documentElement.dataset.fxMagBirthR960='software-webgl-r649-fallback';
        return null;
      }
      loader ||= loadThree();
      ...
      return {
        ...
        engine,
        revision:'r960-reference-proportion-three-genesis'
      };
    }catch(error){
      console.error('FormatX R960 genesis renderer failed:',error);
      document.documentElement.dataset.fxMagBirthR960='fallback-r649';
      return null;
    }
  }
  ...
  window.FormatXMagGenesisThreeR960={
    attach,
    revision:'r960-reference-proportion-three-genesis-dna-cellular-living-architecture'
  };
})();