(function () {
  ...
  const SPECS = [
    ...
    },
    {
      ...
      summaryEn: 'Six specialised modules appear only when they are actually needed.'
    },
    {
  ...
  function onClick(event) {
    ...
    const close = target.closest('[data-organism-close]');
    if (close) {
    ...
    const tab = target.closest('[data-organism-tab]');
    if (tab) {
      event.preventDefault();
    ...
    const trigger = target.closest('[data-organism-open]');
    if (trigger) {
      event.preventDefault();
  ...
  function initialise() {
    ...
    /* Never auto-open a stale URL hash during startup or bfcache restore. */
