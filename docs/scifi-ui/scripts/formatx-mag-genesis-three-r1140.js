(() => {
  ...
  class FormatXGenesisThree {
    ...
    makeDebris(){
      ...
      const mpa=new T.PointsMaterial({
        color:0xb7f3ff,size:.044,transparent:true,opacity:.68,
        depthWrite:false,blending:T.AdditiveBlending,sizeAttenuation:true
      });
    ...
    makeDNAField(){
      const placements=[
        [-2.76,1.48,.24,-.40,1.02],
        [2.70,1.50,.22,.42,1.00],
        [-2.70,-1.48,.18,.42,1.00],
        [2.66,-1.44,.20,-.40,.98],
        [-.66,2.30,-.42,1.54,.86],
        [.70,-2.24,-.38,1.56,.84],
        [-3.16,.05,-.70,1.52,.66],
    ...
    makeCore(){
      ...
      const shellMat=new T.MeshPhysicalMaterial({
        color:0x405560,metalness:.86,roughness:.19,
        emissive:0x0b2430,emissiveIntensity:.12,
        clearcoat:.92,clearcoatRoughness:.08,
        transparent:true,opacity:.92
      });
      const shellGlass=new T.MeshPhysicalMaterial({
        color:0x385965,metalness:.26,roughness:.18,
        transparent:true,opacity:.10,depthWrite:false,
        emissive:0x0b3340,emissiveIntensity:.10,
        clearcoat:.70,clearcoatRoughness:.16
      });
      ...
      const ring1=new T.Mesh(new T.TorusGeometry(.178,.012,10,80),cyan.clone());
      ring1.material.opacity=.22;
      const ring2=new T.Mesh(new T.TorusGeometry(.270,.006,8,80),cyan.clone());
      ring2.material.opacity=.07;
      const pupil=new T.Mesh(new T.CircleGeometry(.095,64),new T.MeshBasicMaterial({color:0x01090f,side:T.DoubleSide}));
      const pupilRing=new T.Mesh(new T.RingGeometry(.101,.132,72),cyan.clone());
    ...
    makeOrganicLayer(){
      ...
      this.organicLobeMaterial=new T.MeshPhysicalMaterial({
        color:0x32213f,roughness:.72,metalness:.008,
        clearcoat:.08,clearcoatRoughness:.62,
        transparent:true,opacity:0,
        emissive:0x271033,emissiveIntensity:.20,
        depthWrite:true
      });
      ...
      this.organicFoldMaterial=new T.MeshPhysicalMaterial({
        color:0x24172f,roughness:.76,metalness:.004,
        emissive:0x1e0d29,emissiveIntensity:.14,
        transparent:true,opacity:0,depthWrite:true
      });
      this.organicFoldGlowMaterial=new T.MeshBasicMaterial({
        color:0x57c9d8,transparent:true,opacity:0,
        depthWrite:false,blending:T.AdditiveBlending
      });
      this.organicFolds=new T.Group();
      const foldPairs=46;
      for(let pair=0;pair<foldPairs;pair++){
        const lane=pair/(foldPairs-1);
        const absA=.06+lane*1.28;
        for(const side of [-1,1]){
          const baseA=side*absA+(r()-.5)*.055;
          const baseP=.28+r()*2.34;
          const phase=r()*Math.PI*2;
          const pts=[];
          for(let j=0;j<10;j++){
            const u=(j-4.5)/9;
            const a=baseA+u*(.22+r()*.15)+Math.sin(j*.92+phase)*.040;
            const p=Math.max(.18,Math.min(2.96,
              baseP+u*(.34+r()*.18)+Math.sin(j*1.08+phase)*.050
            ));
            const rr=1.372+(r()-.5)*.016;
            pts.push(new T.Vector3(
              Math.sin(p)*Math.sin(a)*rr,
              Math.cos(p)*rr,
              Math.sin(p)*Math.cos(a)*rr*.735+.045
            ));
          }
          const curve=new T.CatmullRomCurve3(pts,false,'centripetal');
          const fold=new T.Mesh(
            new T.TubeGeometry(curve,40,.013+r()*.005,7,false),
            this.organicFoldMaterial
          );
          const glow=new T.Mesh(
            new T.TubeGeometry(curve,40,.0048,5,false),
      ...
      this.organicVeinMaterial=new T.MeshBasicMaterial({
        color:0x79e8f3,transparent:true,opacity:0,
      ...
      for(let i=0;i<46;i++){
        ...
        const vein=new T.Mesh(
          new T.TubeGeometry(curve,30,.011+r()*.004,7,false),
      ...
      for(let i=0;i<18;i++){
        ...
        const ez=Math.sqrt(Math.max(.04,1.38*1.38-outer*outer))*.58+.12;
        const end=new T.Vector3(ex,ey,ez);
        const bendA=a+(r()-.5)*.55;
        ...
        const branch=new T.Mesh(
          new T.TubeGeometry(curve,28,.0065+r()*.0025,6,false),
          this.organicVeinMaterial
        );
        branch.userData.phase=r()*Math.PI*2;
        this.organicGroup.add(branch);
      ...
      const hoodDefs=[
        [-.22,.34,.98,.72,1.18,.72,-.23,.13],
        [.22,.34,.98,.72,1.18,.72,.23,-.13],
        [0,.56,.92,.40,.72,.60,0,0]
      ];
    ...
    makeMechanicalLayer(){
      ...
      this.mechBody.scale.set(1.18,1.04,.94);
      ...
      crownL.scale.set(.44,.70,.82);
      crownL.position.set(-.17,.38,.34);
      ...
      crownR.scale.set(-.44,.70,.82);
      crownR.position.set(.17,.38,.34);
      ...
      const left=new T.Mesh(sideGeo,this.mechMidMaterial);
      left.scale.set(.80,.78,.80);
      left.position.set(-.42,.00,.30);
      left.rotation.z=.05;
      this.mechanicalGroup.add(left);
      this.plates.push(left);

      const right=new T.Mesh(sideGeo,this.mechMidMaterial);
      right.scale.set(-.80,.78,.80);
      right.position.set(.42,.00,.30);
      right.rotation.z=-.05;
      ...
      this.mechEyeCorona.scale.set(.72,.72,1);
    ...
    makeTentacles(){
      ...
      this.tentacleMaterial=new T.MeshStandardMaterial({
        color:0x071016,metalness:.48,roughness:.32,
        emissive:0x06212a,emissiveIntensity:.15,
        transparent:true,opacity:0
      });
      ...
      this.tentacleNodeMaterial=new T.PointsMaterial({
        color:0x82e9f7,size:.038,transparent:true,opacity:0,
        depthWrite:false,blending:T.AdditiveBlending,sizeAttenuation:true
      });
      this.tentacleGlowMaterial=new T.MeshBasicMaterial({
        color:0x40c7dc,transparent:true,opacity:0,
        depthWrite:false,blending:T.AdditiveBlending
      });
      this.tentacleDashMaterial=new T.LineDashedMaterial({
        color:0x76e4f0,dashSize:.080,gapSize:.070,
      ...
      for(let i=0;i<count;i++){
        ...
        const curve=new T.CatmullRomCurve3(pts,false,'centripetal');
        const geo=this.createTaperedTube(curve,62,8,.076+r()*.014,.010+r()*.0025);
        const glowGeo=this.createTaperedTube(curve,62,6,.015+r()*.0025,.004+r()*.001);
        ...
        const nodeGeo=new T.BufferGeometry().setFromPoints(curve.getPoints(42).filter((_,idx)=>idx%3===0));
        const nodes=new T.Points(nodeGeo,this.tentacleNodeMaterial);
        const dashGeo=new T.BufferGeometry().setFromPoints(curve.getPoints(72));
    ...
    updateDNA(t,time){
      ...
      this.dnas.forEach((h,i)=>{
        ...
        h.userData.materials.forEach((m,mi)=>{
          m.opacity=(bases[mi]??.5)*fade*intro*.90;
        });
      });
    }
    ...
    updateCore(t,time){
      let sc;
      if(t<2.70) sc=.78+ease(t/2.70)*.08;
      else if(t<3.30) sc=mix(.86,.52,smooth((t-2.70)/.60));
      else if(t<5.70) sc=.52;
      else if(t<6.55) sc=mix(.52,.50,smooth((t-5.70)/.85));
      else sc=.50;

      const endMove=smooth((t-9.78)/.20);
      const target=this.targetWorld();
      const tx=target.x*endMove,ty=target.y*endMove;
      const stage1=1-smooth((t-2.66)/.34);
      const cellular=smooth((t-2.58)/.50)*(1-smooth((t-6.22)/.76));
      const mechanical=smooth((t-6.18)/.82);
      const surfaceZ=stage1*.80+cellular*1.06+mechanical*.18;
      ...
      this.coreGroup.rotation.y=Math.sin(time*.00018)*.007;
      this.coreGroup.rotation.x=Math.sin(time*.00022)*.005;

      const eye=smooth((t-2.72)/.32);
      const eyeHold=eye*(1-smooth((t-6.30)/.68));
      const coreFade=1-smooth((t-6.18)/.78);
      const pulse=.982+.018*Math.sin(time*.0046);

      this.coreShell.material.opacity=(.92*stage1 + .52*(1-stage1))*coreFade;
      this.coreGlass.material.opacity=(.16*stage1 + .045*(1-stage1))*coreFade;
      this.coreEdges.material.opacity=(.18*stage1 + .055*(1-stage1))*coreFade;
      if(this.corePetalMaterial)this.corePetalMaterial.opacity=(.12*stage1+.16*(1-stage1))*coreFade;

      this.irisGroup.scale.setScalar((.015+eyeHold*.46)*pulse);
      this.irisRays.rotation.z=time*.000060;
      if(this.irisCorona)this.irisCorona.material.opacity=.70*eyeHold*coreFade;
      this.glowSprite.material.opacity=(.002+eyeHold*.035)*pulse*coreFade;
      this.glowSprite.scale.setScalar(1.00+eyeHold*.14);
      this.coreInner.material.opacity=(.004+eyeHold*.065)*coreFade;
      this.coreLight.intensity=eyeHold*(2.6+Math.sin(time*.0046)*.18)*coreFade;

      if(this.coreLabel)this.coreLabel.material.opacity=.44*stage1*coreFade;
    }

    updateOrganic(t,time){
      const grow=smooth((t-2.48)/.72);
      const fade=1-smooth((t-6.18)/.92)*.98;
      const visible=grow*fade;
      this.organicGroup.visible=visible>.002;
      const bodyScale=.001+visible*.999;
      this.organicGroup.scale.set(bodyScale*1.12,bodyScale*1.08,bodyScale);

      this.organicShellMaterial.opacity=.48*visible;
      this.organicLobeMaterial.opacity=.82*visible;
      this.organicWireMaterial.opacity=.0006*visible;
      this.organicVeinMaterial.opacity=.46*visible;
      this.organicHoodMaterial.opacity=.28*visible;
      if(this.organicFoldMaterial)this.organicFoldMaterial.opacity=.86*visible;
      if(this.organicFoldGlowMaterial)this.organicFoldGlowMaterial.opacity=.18*visible;

      if(this.organicHoodGroup){
        this.organicHoodGroup.scale.setScalar(.62);
        this.organicHoodGroup.rotation.y=Math.sin(time*.00022)*.016;
        this.organicHoodGroup.rotation.x=Math.sin(time*.00018)*.008;
      }
      this.organicShell.rotation.y=time*.000026;
      this.organicShell.rotation.x=Math.sin(time*.00020)*.009;

      this.organicLobes.forEach((lobe,i)=>{
        const q=1+Math.sin(time*.00096+lobe.userData.phase)*.014*visible;
        const b=lobe.userData.baseScale;
        if(b)lobe.scale.set(b.x*q,b.y*q,b.z*q);
        lobe.rotation.y+=.000075*(i%2?1:-1);
      });
      this.organicVeins.forEach(vein=>{
        vein.rotation.z=Math.sin(time*.00025+vein.userData.phase)*.008;
      });
      this.organicPrimaryVeins?.forEach((vein,i)=>{
        vein.rotation.z=Math.sin(time*.00019+(vein.userData.phase||i))*.0045;
      });
      if(this.organicFolds){
        this.organicFolds.rotation.y=Math.sin(time*.00015)*.011;
        this.organicFolds.rotation.x=Math.sin(time*.00012)*.006;
      }
    }

    updateCells(t,time){
      const grow=smooth((t-2.60)/.70);
      const fade=1-smooth((t-6.22)/.88)*.98;
      const visible=grow*fade;
      ...
      this.cellEdgeMaterial.opacity=.0008*visible;
      this.cellVeinMaterial.opacity=.20*visible;
      ...
      this.cellVeins?.forEach((line,i)=>{
        line.rotation.z=Math.sin(time*.00018+(line.userData.phase||i))*.004*visible;
      });
    }

    updateMechanical(t,time){
      const grow=smooth((t-6.18)/.82);
      this.mechanicalGroup.visible=grow>.002;
      this.mechanicalGroup.scale.setScalar(.001+grow*1.08);

      this.mechMaterial.opacity=.995*grow;
      this.mechMidMaterial.opacity=.985*grow;
      this.silverMaterial.opacity=.88*grow;
      this.mechEdgeMaterial.opacity=.08*grow;
      this.mechInnerMaterial.opacity=.52*grow;
      if(this.seamMaterial)this.seamMaterial.opacity=.56*grow;
      if(this.mechEyeCorona)this.mechEyeCorona.material.opacity=.94*grow;
      this.mechInnerRing.rotation.z=time*.00012;
      if(this.mechLight)this.mechLight.intensity=8.2*grow;
    ...
    updateTentacles(t,time){
      const grow=smooth((t-6.02)/.98);
      const flashFade=1-smooth((t-9.64)/.22)*.10;
      this.tentacleGroup.visible=grow>.002;
      this.tentacleMaterial.opacity=.90*grow*flashFade;
      this.tentacleEdgeMaterial.opacity=.0025*grow;
      this.tentacleNodeMaterial.opacity=.66*grow;
      if(this.tentacleGlowMaterial)this.tentacleGlowMaterial.opacity=.34*grow;
      if(this.tentacleDashMaterial)this.tentacleDashMaterial.opacity=.72*grow;
      this.tentacles.forEach((g,i)=>{
        const wave=1+Math.sin(time*.00066+g.userData.phase)*.006*grow;
        const v=.001+grow*.999;
        g.scale.set(v*wave,v*wave,v*wave);
        g.rotation.z=Math.sin(time*.00032+g.userData.phase)*.012*grow;
        g.rotation.x=Math.sin(time*.00025+g.userData.phase)*.008*grow;
      });
    }

    updateCamera(t,time){
      let z=5.00,y=.014,x=0;
      if(t<2.70){
        const k=smooth(t/2.70);
        z=mix(5.02,4.84,k);
        x=Math.sin(time*.00027)*.055*(1-k*.35);
        y=.012+Math.sin(time*.00024)*.020;
      }else if(t<3.28){
        const k=smooth((t-2.70)/.58);
        z=mix(4.84,3.94,k);
        x=mix(.024,0,k); y=mix(.016,0,k);
      }else if(t<5.55){
        z=3.94+Math.sin(time*.00020)*.010;
        x=Math.sin(time*.00015)*.006;
        y=Math.cos(time*.00018)*.005;
      }else if(t<7.18){
        const k=smooth((t-5.55)/1.63);
        z=mix(3.94,4.58,k);
      }else{
        z=4.58+Math.sin(time*.00017)*.008;
        y=.004;
      }
      if(this.width<this.height)z+=.94;
    ...
    render(r,time){
      ...
      if(this.flashBurst){
        ...
        const burstScale=3.8+flash*3.4;
        this.flashBurst.scale.set(burstScale,burstScale,1);
      }
      if(this.flashBeam){
        this.flashBeam.material.opacity=flash*.86;
        this.flashBeam.scale.x=1+flash*1.55;
      }
  ...
  async function attach(canvas,getTarget){
    ...
    try{
      ...
      if(!visualProof && isSoftwareWebGL()){
        document.documentElement.dataset.fxMagBirthR1140='software-webgl-r649-fallback';
        return null;
      }
      loader ||= loadThree();
      ...
      return {
        ...
        engine,
        revision:'r1140-proof-corrected-reference'
      };
    }catch(error){
      console.error('FormatX R1140 genesis renderer failed:',error);
      document.documentElement.dataset.fxMagBirthR1140='fallback-r649';
      return null;
    }
  }
  ...
  window.FormatXMagGenesisThreeR1140={
    attach,
    revision:'r1140-proof-corrected-reference-dna-cellular-living-architecture'
  };
})();