(() => {
  ...
  class FormatXGenesisThree {
    ...
    updateCore(t,time){
      let sc=.001;
      if(t>=1.35 && t<2.55)sc=.05+ease((t-1.35)/1.20)*.45;
      else if(t>=2.55 && t<3.35)sc=mix(.50,.84,smooth((t-2.55)/.80));
      else if(t>=3.35 && t<5.65)sc=.84+Math.sin((t-3.35)*1.02)*.004;
      else if(t>=5.65 && t<8.95)sc=mix(.84,.58,smooth((t-5.65)/3.30));
      else if(t>=8.95)sc=.58;

      const endMove=smooth((t-9.76)/.22);
      const target=this.targetWorld();
      const tx=target.x*endMove,ty=target.y*endMove;

      // During the cellular film the optical diamond sits on the front surface.
      ...
      this.irisGroup.scale.setScalar((.10+eyeHold*.92)*pulse);
      this.irisRays.rotation.z=time*.000090;

      this.glowSprite.material.opacity=(.015+eyeHold*.50)*pulse;
      this.glowSprite.scale.setScalar(1.34+eyeHold*.42);
      this.coreInner.material.opacity=.04+eyeHold*.68;
      this.coreEdges.material.opacity=.18+.16*eyeHold;
      this.coreLight.intensity=eyeHold*(8.4+Math.sin(time*.005)*1.1);

      if(this.coreLabel)this.coreLabel.material.opacity=.34*(1-smooth((t-2.72)/.30));
    }
  ...
  async function attach(canvas,getTarget){
    ...
    try{
      ...
      if(!visualProof && isSoftwareWebGL()){
        document.documentElement.dataset.fxMagBirthR670='software-webgl-r649-fallback';
        return null;
      }
      loader ||= loadThree();
      ...
      return {
        ...
        engine,
        revision:'r670-video-accurate-organic-handoff-three-genesis'
      };
    }catch(error){
      console.error('FormatX R670 genesis renderer failed:',error);
      document.documentElement.dataset.fxMagBirthR670='fallback-r649';
      return null;
    }
  }
  ...
  window.FormatXMagGenesisThreeR670={
    attach,
    revision:'r670-video-accurate-organic-handoff-three-genesis-dna-cellular-living-architecture'
  };
})();