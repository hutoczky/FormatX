...
cleanup(){ rm -f -- "$TMP_CONFIG"; unset CLOUDFLARE_API_TOKEN; }
trap cleanup EXIT
fail(){ printf '\nHIBA: %s\n' "$*" >&2; exit 1; }
...
json_value(){ python3 -c "import json,sys; o=json.load(sys.stdin); $1"; }
...
DB_ID="$(python3 - "$DB_NAME" <<<"$DB_LIST" <<'PY'
import json,sys
name=sys.argv[1]; data=json.load(sys.stdin)
print(next((item.get('uuid') or item.get('id') or '' for item in data.get('result',[]) if item.get('name')==name),''))
PY
)"
if [[ -z "$DB_ID" ]]; then
  ...
  DB_ID="$(printf '%s' "$DB_CREATE" | json_value 'r=o.get("result") or {}; print(r.get("uuid") or r.get("id") or "")')"
fi
...
if [[ -z "$AUTH_DOMAIN" ]]; then
  ...
  ORG_BODY="$(python3 - "$AUTH_DOMAIN" <<'PY'
import json,sys
print(json.dumps({'name':'FormatX','auth_domain':sys.argv[1],'auto_redirect_to_identity':False,'deny_unmatched_requests':False}))
PY
)"
...
OTP_ID="$(python3 - <<<"$IDPS" <<'PY'
import json,sys
data=json.load(sys.stdin)
print(next((item.get('id','') for item in data.get('result',[]) if item.get('type')=='onetimepin'),''))
PY
)"
if [[ -z "$OTP_ID" ]]; then
  ...
  OTP_ID="$(printf '%s' "$OTP_CREATE" | json_value 'r=o.get("result") or {}; print(r.get("id") or "")')"
fi
...
APP_ID="$(python3 - "$ADMIN_PATH" <<<"$APPS" <<'PY'
import json,sys
suffix=sys.argv[1]; data=json.load(sys.stdin)
for app in data.get('result',[]):
    uris=[d.get('uri','') for d in app.get('destinations',[]) if isinstance(d,dict)]
    if any(uri.endswith(suffix) for uri in uris) or str(app.get('domain','')).endswith(suffix):
        print(app.get('id','')); break
else: print('')
PY
)"
APP_BODY="$(python3 - "$OTP_ID" <<'PY'
import json,sys
otp=sys.argv[1]
print(json.dumps({
 'name':'FormatX tulajdonosi licenckezelő',
 'type':'self_hosted',
 'domain':'www.formatxsuite.com/fx-owner-license/*',
 'destinations':[{'type':'public','uri':'www.formatxsuite.com/fx-owner-license/*'},{'type':'public','uri':'formatxsuite.com/fx-owner-license/*'}],
 'session_duration':'8h','app_launcher_visible':False,'auto_redirect_to_identity':True,
 'allowed_idps':[otp], 'options_preflight_bypass':False
}))
PY
)"
if [[ -n "$APP_ID" ]]; then APP_RESULT="$(api PUT "/accounts/$ACCOUNT_ID/access/apps/$APP_ID" "$APP_BODY")"; else APP_RESULT="$(api POST "/accounts/$ACCOUNT_ID/access/apps" "$APP_BODY")"; fi
printf '%s' "$APP_RESULT" | check_success || fail 'A Cloudflare Access alkalmazás nem állítható be.'
APP_ID="$(printf '%s' "$APP_RESULT" | json_value 'r=o.get("result") or {}; print(r.get("id") or "")')"
ACCESS_AUD="$(printf '%s' "$APP_RESULT" | json_value 'r=o.get("result") or {}; print(r.get("aud") or "")')"
...
POLICY_ID="$(python3 - <<<"$POLICIES" <<'PY'
import json,sys
data=json.load(sys.stdin)
print(next((item.get('id','') for item in data.get('result',[]) if item.get('name')=='FormatX owner only'),''))
PY
)"
POLICY_BODY="$(python3 - "$ADMIN_EMAIL" "$OTP_ID" <<'PY'
import json,sys
print(json.dumps({'name':'FormatX owner only','decision':'allow','precedence':1,'include':[{'email':{'email':sys.argv[1]}}],'require':[{'login_method':{'id':sys.argv[2]}}],'session_duration':'8h'}))
PY
)"
if [[ -n "$POLICY_ID" ]]; then POLICY_RESULT="$(api PUT "/accounts/$ACCOUNT_ID/access/apps/$APP_ID/policies/$POLICY_ID" "$POLICY_BODY")"; else POLICY_RESULT="$(api POST "/accounts/$ACCOUNT_ID/access/apps/$APP_ID/policies" "$POLICY_BODY")"; fi
...
source=Path('wrangler.jsonc'); data=json.loads(source.read_text(encoding='utf-8'))
...
set_secret_if_missing(){ local name="$1" value="$2"; if ! grep -q "\"name\"[[:space:]]*:[[:space:]]*\"$name\"" <<<"$SECRET_LIST"; then printf '%s' "$value" | npx wrangler secret put "$name" --config "$TMP_CONFIG" >/dev/null; fi; }
...
if ! grep -q '"ADMIN_PASSWORD_RECORD"' <<<"$SECRET_LIST" || [[ "$RESET_PASSWORD" == "1" ]]; then
  ...
  PASSWORD_RECORD="$(python3 - "$FALLBACK_PASSWORD" <<'PY'
import base64,hashlib,os,sys
pw=sys.argv[1].encode(); salt=os.urandom(24); iterations=600000; derived=hashlib.pbkdf2_hmac('sha256',pw,salt,iterations,dklen=32)
b64=lambda b: base64.urlsafe_b64encode(b).decode().rstrip('=')
print(f'v1:{iterations}:{b64(salt)}:{b64(derived)}')
PY
)"
...
for attempt in $(seq 1 24); do
  ...
  if [[ "$status" == '200' ]] && python3 -c 'import json; o=json.load(open("/tmp/formatx-license-health.json")); assert o.get("ok") is True'; then break; fi
  sleep 5
done
